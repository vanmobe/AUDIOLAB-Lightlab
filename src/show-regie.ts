...
export function defaultShowRegie(show: Pick<ShowDocument, 'groups'>): ShowRegie {
  return {
    ...
    colorRoles: ['primary', 'accent'],
    safetyGroupIds: show.groups.some((group) => group.id === 'front') ? ['front'] : [],
  }
}
export function assertShowRegie(value: unknown, groupIds: string[]): asserts value is ShowRegie {
  const invalid = () => {
    throw new Error('Ongeldige showregie: controleer lichtdekking, kleurrollen en veiligheidslichtgroepen.')
  }
  if (!value || typeof value !== 'object' || Array.isArray(value)) return invalid()
  const regie = value as Record<string, unknown>
  if (
    Object.keys(regie).some((key) => !['minimumCoverage', 'colorRoles', 'safetyGroupIds', 'transition'].includes(key))
  )
    return invalid()
  if (regie.transition !== undefined) {
    ...
    if (
      ...
      Object.keys(transition).some((key) => !['quantizeBeats', 'fadeBeats'].includes(key)) ||
      ![0, 1, 2, 4, 8].includes(transition.quantizeBeats as number) ||
      typeof transition.fadeBeats !== 'number' ||
      !Number.isFinite(transition.fadeBeats) ||
      transition.fadeBeats < 0 ||
      transition.fadeBeats > 32
    )
      return invalid()
  }
  ...
  if (
    ...
    Object.keys(coverage).some((key) => !['percent', 'threshold'].includes(key))
  )
    return invalid()
  if (
    ...
    !Number.isFinite(coverage.threshold) ||
    coverage.threshold < 0.01 ||
    coverage.threshold > 1
  )
    return invalid()
  if (
    !Array.isArray(regie.colorRoles) ||
    !regie.colorRoles.length ||
    regie.colorRoles.length > 4 ||
    new Set(regie.colorRoles).size !== regie.colorRoles.length ||
    regie.colorRoles.some((role) => !colorRoles.includes(role))
  )
    return invalid()
  if (
    !Array.isArray(regie.safetyGroupIds) ||
    regie.safetyGroupIds.length > groupIds.length ||
    new Set(regie.safetyGroupIds).size !== regie.safetyGroupIds.length ||
    regie.safetyGroupIds.some((id) => !groupIds.includes(id))
  )
    return invalid()
}
...
export function regieColor(profile: ColorProfile | undefined, index: number, regie?: ShowRegie): string {
  ...
  const role =
    roles.length === 2 && roles[0] === 'primary' && roles[1] === 'accent'
      ? index % 3 === 0
        ? 'accent'
        : 'primary'
...
function points(
  ...
  return frame.fixtures.flatMap((light, fixture) => {
    const deployment = deployments.get(light.fixtureId),
      definition = deployment && definitions.get(deployment.profileId)
...
function visible(point: LightPoint, threshold: number) {
  return point.intensity + 1e-9 >= threshold && point.color.toLowerCase() !== '#000000'
}
export function coverageReport(show: ShowDocument, profiles: FixtureProfile[], frame: EvaluatedFrame) {
  const settings = show.regie?.minimumCoverage ?? { percent: 0, threshold: 0.1 }
  ...
  return {
    ...
    lit,
    required,
    percent: all.length ? (lit / all.length) * 100 : 0,
    unmet: Math.max(0, required - lit),
    applied: settings.percent > 0 && (frame.mode === 'automation' || frame.mode === 'static'),
  }
}

/** Raise only the minimum number of eligible points, never operator-off or level caps. */
export function applyMinimumCoverage(
  show: ShowDocument,
  profiles: FixtureProfile[],
  frame: EvaluatedFrame,
  caps: Map<string, number>,
  ...
  let missing =
    Math.ceil((all.length * settings.percent) / 100) - all.filter((point) => visible(point, settings.threshold)).length
  if (missing <= 0) return frame
  // Stable fixture/segment order breaks ties; no random flicker from this correction itself.
  const candidates = all
    .filter(
      (point) =>
        !visible(point, settings.threshold) &&
        point.cap + 1e-9 >= settings.threshold &&
        point.color.toLowerCase() !== '#000000',
    )
  ...
  const fixtures = frame.fixtures.map((fixture, index) => ({
    ...fixture,
    ...(fixture.segments
      ? { segments: fixture.segments.map((segment) => ({ ...segment })) }
      : headCounts.has(index)
        ? {
            segments: Array.from({ length: headCounts.get(index)! }, () => ({
