;async (page) => {
  ...
  assert(
    normal.universes[0].fixtures.find((f) => f.fixtureId === 'hazer-1').channels.length === 2,
    'New hazer uses two channels',
  )
  ...
  assert(
    black.universes.every((universe) => universe.channels.every((value) => value === 0)),
    'Actual runtime zeroes every blackout channel',
  )
  ...
  assert(
    legacy.universes.length === 0 && legacy.issues.some((issue) => issue.code === 'unsupported-mode'),
    'Unsupported saved mode fails closed',
  )
  ...
  assert(
    await page.getByRole('button', { name: 'Patch opslaan', exact: true }).isDisabled(),
    'New fan channel overlap blocks patch save',
  )
  assert(
    await page.getByRole('button', { name: 'Bereken DMX-proef', exact: true }).isDisabled(),
    'Dirty patch cannot be inspected as current',
  )
  ...
  assert(
    repaired.universes[0].fixtures.find((f) => f.fixtureId === 'hazer-1').address === 100,
    'Explicit repaired patch is used',
  )
  ...
  assert(
    outputCalls.length > 0 && outputCalls.every((url) => url.endsWith('/output/inspect')),
    'No arm or raw-frame calls occurred',
  )
  assert(errors.length === 0, `Browser errors: ${errors.join('; ')}`)
  await page.evaluate(() => {
    window.__dmxInspectorQa = 'passed'
  })
