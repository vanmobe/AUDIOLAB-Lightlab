      ...
      for (const row of rows) {
        ...
        for (const field of fields) {
          for (const other of fields.filter((other) => Math.abs(other.label.top - field.label.top) < 1)) {
            if (
              Math.abs(other.control.top - field.control.top) > 1 ||
              Math.abs(other.control.bottom - field.control.bottom) > 1
            )
              errors.push('Controls in the same row are misaligned')
          }
        }
        ...
        if (
          ...
          fields.some(
            (field) =>
              Math.abs(field.control.top - button.top) > 1 || Math.abs(field.control.bottom - button.bottom) > 1,
          )
        )
          errors.push('Action button is misaligned')
      }
