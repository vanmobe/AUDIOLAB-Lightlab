...
async function fixture(t) {
  ...
  const root = join(temp, 'package'); await mkdir(root)
  const suffix = process.platform === 'win32' ? '.exe' : ''
  const names = [`runtime/Lightflow.Runtime${suffix}`, `bin/node${suffix}`, 'dist/index.html', 'runtime-worker/dist/engine.mjs', 'scripts/start-local.mjs', 'scripts/local-launcher.mjs', 'scripts/runtime-manager.mjs', 'scripts/distribution.mjs']
  for (const name of names) { await mkdir(dirname(join(root, name)), { recursive: true }); await writeFile(join(root, name), `test fixture ${name}`) }
...
test('verifies a complete host-native package and its required runtime path', async t => {
  ...
  assert.equal(result.rid, hostRuntime()); assert.equal(result.files.length, manifest.files.length)
...
test('detects changed bytes, extra files and missing shipped files', async t => {
  const { root } = await fixture(t), index = join(root, 'dist/index.html'), original = await readFile(index)
  await writeFile(index, 'tampered')
  await assert.rejects(verifyDistribution(root), /Distributiecontrole mislukt/)
  await writeFile(index, original); await writeFile(join(root, 'extra.txt'), 'unexpected')
  await assert.rejects(verifyDistribution(root), /Distributiecontrole mislukt/)
  await rm(join(root, 'extra.txt')); await rm(index)
  await assert.rejects(verifyDistribution(root), /Distributiecontrole mislukt/)
})
test('rejects omitted required components, duplicate entries and path traversal in a manifest', async t => {
  const { root, manifest, publish } = await fixture(t), original = structuredClone(manifest.files)
  manifest.files = manifest.files.filter(file => file.path !== 'dist/index.html'); await publish()
  await assert.rejects(verifyDistribution(root), /mist een vereist onderdeel/)
  manifest.files = [...original, original[0]]; await publish()
  await assert.rejects(verifyDistribution(root), /Ongeldige distributiecontrole/)
  manifest.files = [{ ...original[0], path: '../outside' }, ...original.slice(1)]; await publish()
...
test('rejects unsupported or foreign runtime targets', async t => {
  ...
  manifest.rid = 'unsupported-x64'; await publish()
  await assert.rejects(verifyDistribution(root), /niet geschikt/)
})
test('rejects symlinked manifest and symlinked contents before following their targets', { skip: process.platform === 'win32' ? 'Windows symlink creation requires elevated/developer-mode permissions.' : false }, async t => {
  const { root, temp, manifestPath } = await fixture(t)
  const outside = join(temp, 'outside.json'); await writeFile(outside, await readFile(manifestPath)); await rm(manifestPath)
  await symlink(outside, manifestPath)
  await assert.rejects(verifyDistribution(root), /manifest ongeldig/)
  await rm(manifestPath); await writeFile(manifestPath, await readFile(outside))
...
test('bounds manifest bytes and claimed total bytes', async t => {
  ...
  manifest.files[0].bytes = 600_000_001; await publish()
...
test('preflights actual oversized files before hashing despite small claimed manifest sizes', async t => {
  ...
  try { await file.truncate(600_000_001) } finally { await file.close() }
  await assert.rejects(verifyDistribution(root), /verificatielimiet/)
})
test('bounds manifest entry count, real directory count and nesting', async t => {
  const { root, manifest, publish } = await fixture(t)
  manifest.files = Array.from({ length: 2049 }, () => manifest.files[0]); await publish()
  await assert.rejects(verifyDistribution(root), /manifest is ongeldig/)
  const countRoot = join(root, 'count'); await mkdir(countRoot)
  ...
  const deepRoot = join(root, 'deep'); await mkdir(join(deepRoot, ...Array.from({ length: 13 }, () => 'child')), { recursive: true })
...
test('rejects nonportable and escaped paths without resolving outside the root', () => {
  for (const path of ['', '/etc/passwd', '../outside', 'a/../b', 'a//b', 'a\\b', './x']) assert.throws(() => packagePath('/temporary-root', path), /Ongeldig pad/)
})
