...
interface Props {
  ...
  onDragStart?: (event: { dataTransfer: { setData: (type: string, value: string) => void; effectAllowed: string } }) => void
  onDragOver?: (event: { preventDefault: () => void; dataTransfer: { types: string[]; effectAllowed: string; dropEffect: string } }) => void
...
function session(start: ShowDocument = initialShow) {
  ...
  const preview = vi.fn()
  const changed = vi.fn((update: SetStateAction<ShowDocument>) => { show = typeof update === 'function' ? update(show) : update })
  const render = () => {
    hooks.cursor = 0
  ...
  const source = (name: string) => {
    const item = find(render(), props => props.className === 'surface-source' && text(props.children).startsWith(name))
    if (!item) throw new Error(`Missing source: ${name}`)
    return item
  }
  return { render, label, button, source, preview, changed, show: () => show, replace: (value: ShowDocument) => { show = value } }
}
...
describe('control surface editor workflow', () => {
  it('keeps the auto-fill draft mounted across closing and bank changes, resetting only for another profile', () => {
    const view = session()
    const editor = () => find(view.render(), props => props.initialBank !== undefined && props.hideCloseButton === true)
  ...
  it('matches copy and move drop effects to the source drag permission', () => {
    ...
    const transfer = { effectAllowed: 'none', dropEffect: 'none', types: ['application/x-lightlab-control'], setData: (key: string, value: string) => { payload.set(key, value) } }
    ...
    view.label('Knop 1: Niet toegewezen').onDrop!({ preventDefault: vi.fn(), dataTransfer: { getData: key => payload.get(key) ?? '' } })
    ...
    view.label('Knop 2: Niet toegewezen').onDrop!({ preventDefault: vi.fn(), dataTransfer: { getData: key => payload.get(key) ?? '' } })
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 1 })).toBeUndefined()
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 2 })?.targetId).toBe('warm-static')
    expect(view.preview).not.toHaveBeenCalled()
  })
  ...
  it('moves or swaps using the click/keyboard flow without native drag', () => {
    ...
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 1 })?.targetId).toBe('neon-chorus')
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 2 })?.targetId).toBe('warm-static')
    expect(view.preview).not.toHaveBeenCalled()
  })
  ...
  it('releases an unsupported old slot before assigning to an occupied Compact button', () => {
    let start = assignControlBinding(initialShow, initialShow.controlSurface.bindings[0], { bank: 16, kind: 'button', index: 8 })
    ...
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 1 })?.id).toBe(initialShow.controlSurface.bindings[0].id)
    expect(view.show().controlSurface.bindings.find(binding => binding.id === initialShow.controlSurface.bindings[1].id)?.slot).toBeUndefined()
    expect(view.preview).not.toHaveBeenCalled()
  })
  ...
  it('deletes only an unplaced binding and supports undo without deleting its Look', () => {
    ...
    expect(view.show().controlSurface.bindings.some(binding => binding.id === initialShow.controlSurface.bindings[0].id)).toBe(false)
  ...
  it('clicks a source then assigns a button, and previews only through the explicit action', () => {
    ...
    expect(view.changed).not.toHaveBeenCalled()
    view.label('Knop 1: Niet toegewezen').onClick!()
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 1 })).toMatchObject({ action: 'look', targetId: 'warm-static' })
    expect(view.preview).not.toHaveBeenCalled()
    view.label('Knop 1: Warm static').onClick!()
    expect(view.preview).not.toHaveBeenCalled()
  ...
  it('assigns a group only to a rotary without changing the group master', () => {
    ...
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'rotary', index: 4 })).toMatchObject({ action: 'group-intensity', targetId: 'wash' })
  ...
  it('keeps source selection through a bank switch and places only in that bank', () => {
    ...
    expect(bindingAtSlot(view.show().controlSurface, { bank: 16, kind: 'button', index: 8 })?.targetId).toBe('neon-chorus')
  ...
  it('switches to Compact while preserving unsupported Rack slots in the unplaced library', () => {
    const start = assignControlBinding(initialShow, initialShow.controlSurface.bindings[0], { bank: 16, kind: 'button', index: 8 })
    const view = session(start)
    view.label('Type bedieningspaneel').onChange!({ target: { value: 'wing-compact' } })
    expect(bindingAtSlot(view.show().controlSurface, { bank: 16, kind: 'button', index: 8 })?.id).toBe(initialShow.controlSurface.bindings[0].id)
    const html = renderToStaticMarkup(view.render())
    ...
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 16 })?.id).toBe(initialShow.controlSurface.bindings[0].id)
    expect(view.preview).not.toHaveBeenCalled()
  })

  it('accepts drag keys only from current show data, not arbitrary serialized controls', () => {
    const view = session()
    view.label('Knop 1: Niet toegewezen').onDrop!({ preventDefault: vi.fn(), dataTransfer: { getData: () => '{"action":"mode","targetId":"blackout"}' } })
    expect(view.changed).not.toHaveBeenCalled()
    expect(text(view.render())).toContain('Kies eerst een beschikbare functie')
    view.label('Knop 1: Niet toegewezen').onDrop!({ preventDefault: vi.fn(), dataTransfer: { getData: () => 'look:warm-static' } })
    expect(bindingAtSlot(view.show().controlSurface, { bank: 1, kind: 'button', index: 1 })?.targetId).toBe('warm-static')
    expect(view.preview).not.toHaveBeenCalled()
  })
})
