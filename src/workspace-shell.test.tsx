...
vi.mock('./show-library', async original => ({ ...await original<typeof import('./show-library')>(), saveLibraryRecovery: vi.fn(async () => ({ id: 'recovery' })) }))

// Exercise shell transitions without WebGL effects; native focus/dialog/layout gets browser QA.
const hooks = vi.hoisted(() => ({ values: [] as unknown[], cursor: 0, effects: [] as { effect: () => void | (() => void); deps: unknown[] }[] }))
vi.mock('react', async original => ({
  ...
  useMemo: (factory: () => unknown) => factory(), useEffect: (effect: () => void | (() => void), deps: unknown[]) => { hooks.effects.push({ effect, deps }) }, useCallback: (callback: unknown) => callback,
}))
interface Props { children?: ReactNode; 'data-workspace'?: string; 'aria-label'?: string; 'aria-pressed'?: boolean; onClick?: () => void; onDirtyChange?: (dirty: boolean) => void; onChange?: (event: { target: { value: string } }) => void; onOpen?: (bundle: ShowPackage) => Promise<boolean>; onRestoreVersion?: (id: string) => Promise<boolean>; versions?: ShowPackage['versions'] }
...
function find(node: ReactNode, match: (props: Props) => boolean): Props {
  for (const child of Children.toArray(node)) {
    ...
    try { return find(child.props.children, match) } catch { /* search siblings */ }
  }
  throw new Error('Control not found')
}
function buttonLabels(node: ReactNode): string[] {
  return Children.toArray(node).flatMap(child => !isValidElement<Props>(child) ? [] : [
    ...(child.props.onClick ? [text(child.props.children)] : []),
    ...buttonLabels(child.props.children),
  ])
}
function setup() {
  hooks.values = []; hooks.cursor = 0
  vi.mocked(saveLibraryRecovery).mockReset(); vi.mocked(saveLibraryRecovery).mockResolvedValue({ id: 'recovery', name: 'Recovery', byteLength: 1, versionCount: 0, updatedAt: new Date().toISOString() })
  ...
  const render = () => { hooks.cursor = 0; hooks.effects = []; return App() }
  const button = (name: string) => find(render(), props => !!props.onClick && text(props.children) === name).onClick!()
...
describe('professional workspace shell', () => {
  it('opens DMX runtime controls without implicitly starting or transmitting a show', () => {
    const view = setup()
    view.button('Live'); view.button('Naar DMX-bediening')
    expect(find(view.render(), props => !!props.onClick && text(props.children) === 'Livesessie')['aria-pressed']).toBe(true)
  ...
  it('preserves runtime warning after successful library opening and explains WAV detach', async () => {
    const view = setup()
    view.button('Live'); view.button('Livesessie'); view.button('Bibliotheek & versies')
  ...
  it('warns before unload immediately after autosave fails and clears only after successful persistence', () => {
    const view = setup(); view.render()
    const save = hooks.effects.find(({ deps }) => deps.length === 3 && Array.isArray(deps[1]) && typeof deps[2] === 'boolean')!.effect
    const cleanup = hooks.effects.find(({ deps }) => deps.length === 3 && deps.every(value => !!value && typeof value === 'object' && 'current' in value))!.effect()
    const warn = view.addEventListener.mock.calls.find(([name]) => name === 'beforeunload')![1] as (event: { preventDefault: () => void; returnValue?: string }) => void
    const before = { preventDefault: vi.fn() }; warn(before); expect(before.preventDefault).not.toHaveBeenCalled()
    view.setItem.mockImplementationOnce(() => { throw new Error('Quota') }); save()
    const failed = { preventDefault: vi.fn(), returnValue: undefined as string | undefined }; warn(failed)
    expect(failed.preventDefault).toHaveBeenCalledOnce(); expect(failed.returnValue).toBe('')
    expect(text(view.render())).toContain('Niet opgeslagen:')
    // Other messages must not erase the independent unsaved-state warning.
    view.button('Bibliotheek & versies'); expect(text(view.render())).toContain('Niet opgeslagen:')
    save(); const saved = { preventDefault: vi.fn() }; warn(saved)
    expect(saved.preventDefault).not.toHaveBeenCalled(); expect(text(view.render())).not.toContain('Niet opgeslagen:')
    cleanup!(); expect(view.removeEventListener).toHaveBeenCalledWith('beforeunload', warn)
  })
  it('also warns before unload when canonical storage was fenced at startup', () => {
    const view = setup(); view.stored.set(activePackageKey, '{corrupt'); view.render()
    hooks.effects.find(({ deps }) => deps.length === 3 && deps.every(value => !!value && typeof value === 'object' && 'current' in value))!.effect()
    const warn = view.addEventListener.mock.calls.find(([name]) => name === 'beforeunload')![1]
    const event = { preventDefault: vi.fn() }; warn(event)
    expect(event.preventDefault).toHaveBeenCalledOnce(); expect(view.setItem).not.toHaveBeenCalled()
  })
  it('migrates legacy show/history once during initialization, not on subsequent renders', () => {
    const view = setup(); view.stored.delete(activePackageKey)
    view.stored.set('lightflow-show-v1', JSON.stringify(view.show)); view.stored.set('lightflow-versions-v1', '[]')
    ...
    expect(view.setItem).toHaveBeenCalledOnce(); expect(view.setItem.mock.calls[0][0]).toBe(activePackageKey)
  ...
  it('awaits durable recovery before any active writes or show replacement', async () => {
    const view = setup(); view.button('Bibliotheek & versies')
    let finish!: (value: Awaited<ReturnType<typeof saveLibraryRecovery>>) => void
    vi.mocked(saveLibraryRecovery).mockImplementationOnce(() => new Promise(resolve => { finish = resolve }))
    const opening = find(view.render(), props => !!props.onOpen).onOpen!(createShowPackage(initialShow, []))
    expect(view.setItem).not.toHaveBeenCalled(); expect(text(view.render())).toContain('Tourshow behouden')
  ...
  it('retains active show and visible library when recovery backup rejects', async () => {
    ...
    await expect(find(view.render(), props => !!props.onOpen).onOpen!(createShowPackage(initialShow, []))).rejects.toThrow('Backup quota')
    expect(view.setItem).not.toHaveBeenCalled(); expect(text(view.render())).toContain('Tourshow behouden')
  ...
  it('retains the entire canonical package and visible history when its atomic write fails', async () => {
    const view = setup(), history = [createVersion(view.show, 'Kept')]
    view.stored.set(activePackageKey, JSON.stringify(createShowPackage(view.show, history))); view.button('Bibliotheek & versies')
    const before = new Map(view.stored)
    view.setItem.mockImplementationOnce(() => { throw new Error('Quota') })
    await expect(find(view.render(), props => !!props.onOpen).onOpen!(createShowPackage(initialShow, []))).rejects.toThrow('huidige show blijft intact')
    expect(view.stored).toEqual(before); expect(text(view.render())).toContain('Tourshow behouden')
  ...
  it('restores a selected version only after full backup and adds the previous editor as recovery version', async () => {
    const view = setup(), version = createVersion(initialShow, 'Target')
    view.stored.set(activePackageKey, JSON.stringify(createShowPackage(view.show, [version]))); view.button('Bibliotheek & versies')
    ...
    expect(saved).toHaveLength(2); expect(saved[0].show.name).toBe('Tourshow behouden'); expect(saved[1].id).toBe(version.id)
    expect(JSON.parse(view.stored.get(activePackageKey)!).show.name).toBe(initialShow.name)
  })
  it('blocks opening the library while a patch draft is dirty', () => {
    const view = setup(); view.button('Setup'); view.button('Patch & netwerk')
    find(view.render(), props => !!props.onDirtyChange).onDirtyChange!(true)
    view.button('Bibliotheek & versies')
    expect(() => find(view.render(), props => !!props.onOpen)).toThrow()
    expect(saveLibraryRecovery).not.toHaveBeenCalled(); expect(view.setItem).not.toHaveBeenCalled()
  })
  ...
  it('keeps the current show if the reset recovery cannot be stored', () => {
    const view = setup()
    view.setItem.mockImplementation(() => { throw new Error('Quota exceeded') })
    view.button('Herstel startshow…')
    expect(text(view.render())).toContain('Tourshow behouden')
  ...
  it('guards leaving a dirty patch and supports cancel then confirm', () => {
    const view = setup()
    view.button('Setup'); view.button('Patch & netwerk')
    find(view.render(), props => !!props.onDirtyChange).onDirtyChange!(true)
    view.confirm.mockReturnValueOnce(false)
    view.button('Live')
  ...
  it('keeps runtime controls on cancelled departure and warns after switching to simulation', () => {
    const view = setup()
    view.button('Live'); view.button('Livesessie')
    view.confirm.mockReturnValueOnce(false)
  ...
  it('does not discard a dirty patch when returning to a possibly active session', () => {
    const view = setup()
    view.button('Live'); view.button('Livesessie'); view.button('Setup'); view.button('Patch & netwerk')
    find(view.render(), props => !!props.onDirtyChange).onDirtyChange!(true)
    view.confirm.mockReturnValueOnce(false)
    view.button('Terug naar livesessie')
    expect(find(view.render(), props => !!props['data-workspace'])['data-workspace']).toBe('patch')
  ...
  it('derives dashboard inventory and library totals from the current show', () => {
    ...
    const html = renderToStaticMarkup(<ShowDashboard show={show} versionCount={4} onSetup={vi.fn()} onDesign={vi.fn()} onLive={vi.fn()} onImport={vi.fn()} onExport={vi.fn()} />)
  ...
  it('orders design navigation from AI to Testlab and opens the safe test surface', () => {
    ...
    expect(expected.map(label => labels.indexOf(label))).toEqual([...expected.keys()].map(index => labels.indexOf(expected[0]) + index))
    view.button('Testlab')
  ...
  it('searches large Live libraries and selects the exact filtered Look', () => {
    hooks.values = []; hooks.cursor = 0
    const show = { ...initialShow, looks: Array.from({ length: 32 }, (_, i) => ({ ...initialShow.looks[0], id: `look-${i}`, name: `Nummer ${i}` })) }
    const onSelect = vi.fn()
    const render = () => { hooks.cursor = 0; return LiveLookLibrary({ show, onSelect, activeLookId: 'look-31' }) }
