...
try {
  ...
  let packaged = false
  try {
    await access(resolve(root, 'lightlab-distribution.json'))
    packaged = true
  } catch (error) {
  ...
  if (!distribution) {
    ...
    if (runtimes.error || !/Microsoft\.AspNetCore\.App 10\./.test(runtimes.stdout ?? ''))
      throw new Error('Installeer de ASP.NET Core 10-runtime of de .NET 10 SDK.')
  }
  try {
    await Promise.all([
      access(resolve(root, 'dist/index.html')),
      access(worker),
      access(distribution?.runtime ?? assembly),
    ])
  } catch {
    throw new Error('Lokale build ontbreekt. Voer eerst npm run build:local uit.')
  }
  if (process.argv.includes('--check'))
    console.log(
      'Lightlab klaar: Node, ASP.NET Core, webapp, patroonengine en runtime gevonden. Geen processen gestart.',
    )
  else {
    await Promise.all([available(5173), available(5188)])
    manager = createRuntimeManager({
      root,
      command: distribution?.runtime ?? 'dotnet',
      args: distribution ? [] : [assembly],
      ...
      env: {
        ...
        LIGHTLAB_WORKER_PATH: worker,
      },
    })
    web = staticApplication(resolve(root, 'dist'), 5173, manager)
    await new Promise((accept, reject) => {
      web.once('error', reject)
      web.listen(5173, '127.0.0.1', accept)
    })
    ...
    if (process.argv.includes('--browser')) {
      ...
      while (!ready && !stopping && Date.now() < deadline) {
        try {
          ready = (await fetch('http://127.0.0.1:5188/health', { signal: AbortSignal.timeout(1000) })).ok
        } catch {
          /* Owned child may still be starting. */
        }
        if (!ready) await new Promise((resolve) => setTimeout(resolve, 200))
      }
      if (!stopping && !ready)
        console.error('Runtime nog niet gereed. Bekijk Lokale runtime in de interface voor status en herstel.')
      if (!stopping) {
        ...
        browser.on('error', () => console.log(`Open de browser zelf op ${url}`))
        browser.unref()
      }
    }
  }
  ...
  console.error(
    error?.code === 'EADDRINUSE'
      ? 'Poort 5173 of 5188 is al bezet. Sluit de eerdere Lightlab/devsessie eerst; bestaande processen zijn niet gestopt.'
      : error instanceof Error
        ? error.message
        : 'Lightlab kon niet starten.',
  )
  shutdown(1)
}
