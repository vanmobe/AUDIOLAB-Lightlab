...
assert.ok(
  ...
  'Active session: refusing to interfere',
)
...
show.regie = {
  ...
  colorRoles: ['primary'],
  safetyGroupIds: ['wash'],
  transition: { quantizeBeats: 1, fadeBeats: 4 },
}
show.groups.forEach((group) => {
  group.intensity = group.id === 'effects' ? 0 : 1
})
show.colorProfiles.forEach((profile, index) => {
  profile.primary = index ? '#ff0000' : '#0000ff'
...
function colorDistance(a, b) {
  return Math.max(
    ...[1, 3, 5].map((index) =>
      Math.abs(parseInt(a.slice(index, index + 2), 16) - parseInt(b.slice(index, index + 2), 16)),
    ),
  )
}
...
function sampleMemory(elapsed) {
  ...
  const rows = result.stdout
    ...
    .filter(Boolean)
    .map((match) => ({ pid: Number(match[1]), parent: Number(match[2]), rssKiB: Number(match[3]) }))
  const runtime = rows.find((row) => row.pid === runtimePid),
    workers = rows.filter((row) => row.parent === runtimePid)
  if (runtime)
    memory.push({
      elapsedSeconds: Math.round(elapsed / 1000),
      runtimeRssKiB: runtime.rssKiB,
      workerRssKiB: workers.reduce((sum, row) => sum + row.rssKiB, 0),
...
try {
  ...
  const fading = await until((value) => value.transition?.phase === 'fading' && value.transition.progress > 0.1)
  ...
  assert.ok(
    colorDistance(fading.frame.fixtures[0].color, interrupted.frame.fixtures[0].color) < 32,
    'Interrupt cue should not jump to a fresh endpoint',
  )
  await until((value) => value.transition?.phase === 'fading' && value.transition.progress > 0.1)
  await command({ command: 'mode', mode: 'static' })
  const held = await preview()
  ...
  assert.equal(black.transition, null)
  assert.ok(black.frame.fixtures.every((light) => light.intensity === 0 && light.haze === 0))
  await command({ command: 'look', lookId: 'neon-chorus' })
  const beforeOff = await preview()
  await command({
    command: 'live',
    expectedRevision: beforeOff.revision,
    controls: { overrides: { back: { mode: 'off' } }, links: [] },
    groupIntensities: beforeOff.groupIntensities,
    colorLockId: null,
  })
  const off = await preview()
  assert.equal(off.transition, null)
  assert.ok(
    off.frame.fixtures
      .filter((light) => show.fixtures.find((fixture) => fixture.id === light.fixtureId).groupId === 'back')
      .every((light) => light.intensity === 0),
  )
  await command({ command: 'look', lookId: 'warm-static' })
  const startTime = performance.now(),
    startFrame = (await preview()).status.frameCount
  let nextCue = 5000,
    nextMemory = 0,
    nextReport = 60_000,
    cueCount = 0,
  ...
  while (performance.now() - startTime < duration) {
    ...
    if (elapsed >= nextCue) {
      await command({ command: 'look', lookId: ++cueCount % 2 ? 'neon-chorus' : 'warm-static' })
      nextCue += 5000
    }
    const value = await preview(),
      now = performance.now()
    assert.equal(value.sessionId, sessionId)
    assert.equal(value.status.outputSent, false)
    ...
    if (elapsed >= nextReport) {
      console.log(
        `Soak ${Math.round(elapsed / 1000)}s: ${value.status.frameCount - startFrame} frames; session healthy, output disabled.`,
      )
      nextReport += 60_000
    }
    await delay(100)
    ...
    JSON.stringify(
      {
        ...
        frames: last.status.frameCount - startFrame,
        cueCount,
        httpRequests: latencies.length,
        httpLatencyMs: {
          median: quantile(latencies, 0.5),
          p95: quantile(latencies, 0.95),
          max: Math.max(...latencies),
        },
        observedCadenceHz: { median: quantile(cadence, 0.5), p05: quantile(cadence, 0.05) },
        memory,
