...
describe('Look group editor', () => {
  it('embeds the selected Look directly without a collapsed card or navigation away', () => {
    const html = renderToStaticMarkup(<LookEditor embedded show={initialShow} look={initialShow.looks[0]} onChange={() => {}} />)
  ...
  it('materializes legacy group behavior without changing untouched groups or input', () => {
    ...
    expect(edited.layers?.filter(layer => layer.groupId !== 'front')).toEqual(before.filter(layer => layer.groupId !== 'front'))
  })

  it('keeps fixed palettes and levels when changing another group', () => {
    const original = editLookLayer(initialShow, initialShow.looks[0], 'front', { colorProfileId: 'warm', intensity: 0.4 })
    const edited = editLookLayer(initialShow, original, 'wash', { mode: 'off', programId: null })
    expect(edited.layers?.find(layer => layer.groupId === 'front')).toEqual(original.layers?.find(layer => layer.groupId === 'front'))
  })
  ...
  it('chooses actual motion for animation mode while preserving an existing moving pattern', () => {
    ...
    expect(animationForLayer({ ...initialShow, programs: initialShow.programs.filter(program => program.effect === 'static') }, layer)).toBeUndefined()
  })

  it('shows fixed group palettes separately from the overall Look palette', () => {
    const look = editLookLayer(initialShow, initialShow.looks[1], 'front', { mode: 'static', colorProfileId: 'warm', intensity: 0.4 })
  ...
  it('renders a compact accessible group selector and contextual inspector', () => {
    const html = renderToStaticMarkup(<LookEditor show={initialShow} look={initialShow.looks[0]} onChange={() => {}} onPreview={() => {}} />)
