...
;async (page) => {
  ...
  page.on('request', (request) => {
    if (request.url().includes('/output/')) outputRequests.push(request.url())
  })
  ...
  if (!(await inspector.getByRole('button', { name: 'Patch opslaan' }).isDisabled()))
    throw new Error('Overlap is not blocked')
  await inspector.getByLabel('Startadres', { exact: true }).fill('510')
  if (!(await inspector.getByRole('button', { name: 'Patch opslaan' }).isDisabled()))
    throw new Error('Channel overflow is not blocked')
  await inspector.getByLabel('Startadres', { exact: true }).fill('300')
  page.once('dialog', (dialog) => dialog.dismiss())
  await page.locator('.patch-fixture').nth(1).click()
  if ((await inspector.getByLabel('Startadres', { exact: true }).inputValue()) !== '300')
    throw new Error('Cancelled switch lost draft')
  ...
  await page.locator('.patch-fixture').first().click()
  if ((await inspector.getByLabel('Startadres', { exact: true }).inputValue()) !== '300')
    throw new Error('Patch not persisted')
  await page.getByRole('button', { name: 'Universes & netwerk', exact: true }).click()
  if (
    (await page
      .locator('.patch-route')
    ...
    throw new Error('Route not persisted')
  for (const width of [1024, 390]) {
    await page.setViewportSize({ width, height: 900 })
    if (await page.evaluate(() => document.documentElement.scrollWidth > innerWidth))
      throw new Error('Network overflows')
