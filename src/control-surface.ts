...
export interface ControlSurfaceProfile {
  id: string; name: string; rotaryMode: 'absolute' | 'relative'
  banks: number; buttons: number; rotaries: number; buttonColumns: number
}
export const controlSurfaceProfiles: readonly ControlSurfaceProfile[] = wingProfiles
export function getControlSurfaceProfile(id: string) { return controlSurfaceProfiles.find(profile => profile.id === id) }
...
export function isControlSlot(value: unknown): value is ControlSlot {
  ...
  return Object.keys(slot).length === 3 && ['bank', 'kind', 'index'].every(key => Object.hasOwn(slot, key)) && (slot.kind === 'button' || slot.kind === 'rotary')
    && typeof slot.bank === 'number' && Number.isInteger(slot.bank) && slot.bank >= 1 && slot.bank <= 64
    && typeof slot.index === 'number' && Number.isInteger(slot.index) && slot.index >= 1 && slot.index <= 64
}
...
export function canAssignBinding(binding: ControlBinding, slot: ControlSlot, profile?: ControlSurfaceProfile): boolean {
  ...
  const compatible = binding.action === 'group-intensity' ? slot.kind === 'rotary'
    : ['look', 'mode', 'color-lock'].includes(binding.action) && slot.kind === 'button'
  return compatible && (!profile || (slot.bank <= profile.banks && slot.index <= (slot.kind === 'button' ? profile.buttons : profile.rotaries)))
}

function validateBindingTarget(show: ShowDocument, binding: ControlBinding) {
  if (typeof binding.id !== 'string' || !binding.id.trim() || binding.id.length > 1024 || typeof binding.label !== 'string' || !binding.label.trim() || binding.label.length > 1024) throw new Error('Kies een geldige naam en ID voor de bediening.')
  ...
  if (binding.action === 'mode' && ['automation', 'static', 'safety', 'blackout'].includes(binding.targetId ?? '')) return
  if (binding.action === 'color-lock' && (binding.targetId === undefined || show.colorProfiles.some(profile => profile.id === binding.targetId))) return
...
export function assignControlBinding(show: ShowDocument, binding: ControlBinding, slot: ControlSlot): ShowDocument {
  ...
  if (!canAssignBinding(binding, slot, profile)) throw new Error('Deze actie past niet op deze knop of draaiknop in de gekozen bank.')
  const previous = surface.bindings.find(item => item.id === binding.id)
  if (!previous && surface.bindings.length >= 512) throw new Error('Maximum 512 bedieningen. Verwijder eerst een ongebruikte bediening.')
  const occupied = bindingAtSlot(surface, slot)
  if (occupied && occupied.id !== binding.id && previous?.slot && !canAssignBinding(occupied, previous.slot, profile)) throw new Error('Deze bedieningen kunnen niet wisselen: de oude positie is niet geschikt voor de andere actie.')
...
export function clearControlSlot(show: ShowDocument, slot: ControlSlot): ShowDocument {
  ...
  return { ...show, controlSurface: { ...show.controlSurface, bindings: show.controlSurface.bindings.map(item => item.id === binding.id ? unassigned(item) : item) } }
}

export function renameControlBank(show: ShowDocument, bank: number, name: string): ShowDocument {
  if (!Number.isInteger(bank) || bank < 1 || bank > 64 || typeof name !== 'string' || name.length > 80) throw new Error('Kies een geldige banknaam van maximaal 80 tekens.')
