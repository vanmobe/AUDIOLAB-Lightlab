...
const invalid = () =>
  new Error('De lokale runtime geeft een ongeldig statusantwoord. Open de webapp opnieuw met de Lightlab-starter.')
export const launcherUnavailable =
  'De lokale starter is niet bereikbaar. Open de webapp opnieuw met de Lightlab-starter om de runtime hier te starten.'
...
export function parseLocalRuntimeStatus(value: unknown): LocalRuntimeStatus {
  ...
  if (
    item.version !== 1 ||
    !['stopped', 'starting', 'running', 'external', 'error'].includes(item.state) ||
    typeof item.canStart !== 'boolean' ||
    !safeText(item.message, 2048) ||
    !Array.isArray(item.logs) ||
    item.logs.length > 200 ||
    (item.canStart && !['stopped', 'error'].includes(item.state))
  )
    throw invalid()
  if (
    (item.canStop !== undefined && typeof item.canStop !== 'boolean') ||
    ...
    (item.startedAt !== undefined &&
      item.startedAt !== null &&
      (!safeText(item.startedAt, 40) || !Number.isFinite(Date.parse(item.startedAt)))) ||
    (item.launchMode !== undefined && !['development', 'prebuilt'].includes(item.launchMode)) ||
    (item.buildMessage !== undefined && !safeText(item.buildMessage, 2048))
  )
    throw invalid()
  ...
  for (const log of item.logs) {
    if (
      ...
      log.time.length > 40 ||
      !Number.isFinite(Date.parse(log.time)) ||
      !['info', 'warning', 'error'].includes(log.level) ||
      !safeText(log.message, 2048)
    )
      throw invalid()
    ids.add(log.id)
  }
...
export async function requestLocalRuntime(
  action: boolean | 'status' | 'start' | 'stop',
  signal: AbortSignal,
  ...
  try {
    ...
    if (response.status === 404 || !response.headers.get('content-type')?.includes('application/json'))
      throw new Error(launcherUnavailable)
    if (!response.ok || !response.body)
      throw new Error('De starter kon de aanvraag niet uitvoeren. Controleer de status opnieuw.')
    ...
    try {
      return parseLocalRuntimeStatus(JSON.parse(content))
    } catch {
      throw invalid()
    }
  } catch (error) {
    if (controller.signal.aborted)
      throw new Error(
        operation === 'start'
          ...
          : operation === 'stop'
            ? 'Stopaanvraag niet bevestigd. De runtime kan nog draaien; controleer de status opnieuw.'
