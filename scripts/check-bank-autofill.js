async (page) => {
  ...
  page.on('request', request => { if (request.method() !== 'GET' && request.url().includes(':5188/')) writes.push(request.url()) })
  ...
  for (const [name, value] of [['Warm static', 'calm'], ['Neon chorus', 'movement'], ['Neon pulse', 'energetic']]) {
  ...
  assert(await panel.getByRole('button', { name: 'Pas bankindeling toe', exact: true }).count() === 0, 'Capacity error has no apply')
  ...
  await page.waitForFunction(() => JSON.parse(localStorage.getItem('lightlab-active-package-v1')).show.controlSurface.bindings.some(binding => binding.slot?.bank === 6))
  const applied = await readShow()
  const surface = applied.controlSurface
  assert(surface.bankNames['1'] === 'Rustig' && surface.bankNames['3'] === 'In beweging' && surface.bankNames['6'] === 'Energiek', 'Named category banks')
  for (const bank of [1, 3, 6]) {
    assert(surface.bindings.some(binding => binding.slot?.bank === bank && binding.slot.kind === 'rotary' && binding.slot.index === 1 && binding.targetId === 'wash'), 'Same R1 across banks')
    assert(surface.bindings.some(binding => binding.slot?.bank === bank && binding.slot.kind === 'rotary' && binding.slot.index === 2 && binding.targetId === 'front'), 'Same R2 across banks')
  }
  const stripSurface = show => { const { controlSurface, ...rest } = show; return rest }
  assert(JSON.stringify(stripSurface(applied)) === JSON.stringify(stripSurface(original)), 'Show/patch/masters remain identical')
  await page.getByRole('button', { name: 'Ongedaan maken', exact: true }).click()
  await page.waitForFunction(surface => JSON.stringify(JSON.parse(localStorage.getItem('lightlab-active-package-v1')).show.controlSurface) === surface, JSON.stringify(original.controlSurface))
  ...
  await panel.getByRole('button', { name: 'Bekijk bankindeling', exact: true }).click()
  await panel.getByRole('button', { name: 'Pas bankindeling toe', exact: true }).click()
  await page.waitForFunction(() => JSON.parse(localStorage.getItem('lightlab-active-package-v1')).show.controlSurface.bindings.filter(b => b.slot?.kind === 'button').length === 3)
  await page.reload()
  ...
  return { status: 'Capacity, review-before-apply, character overrides, noncontiguous banks, fixed rotaries, undo, persistence, live mapping, mobile and no runtime writes passed' }
}
