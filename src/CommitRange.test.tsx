...
function setup() {
  ...
  const render = () => { hooks.cursor = 0; hooks.effects = []; return CommitRange({ value, onCommit, min: 0, max: 1, step: .01 }) }
  return { render, onCommit, update: (next: number) => { value = next; render(); hooks.effects.forEach(effect => effect()) } }
}
...
it('commits once after a full pointer gesture, not while dragging or on subsequent blur', () => {
  ...
  expect(view.render().props.value).toBe(.2)
  expect(view.onCommit).not.toHaveBeenCalled()
  view.render().props.onPointerUp(pointer); view.render().props.onBlur({})
  expect(view.onCommit).toHaveBeenCalledExactlyOnceWith(.2)
})
it('commits held keyboard changes once on keyup and supports blur-only edits', () => {
  ...
  view.render().props.onChange({ target: { value: '0.6' } }); view.render().props.onChange({ target: { value: '0.7' } })
  view.render().props.onKeyUp(keyboard('ArrowRight')); view.render().props.onBlur({})
  expect(view.onCommit).toHaveBeenCalledExactlyOnceWith(.7)
  view.render().props.onChange({ target: { value: '0.9' } }); view.render().props.onBlur({})
  expect(view.onCommit).toHaveBeenLastCalledWith(.9)
})
it.each(['pointer', 'escape'])('cancels %s without committing and restores latest server value', kind => {
  const view = setup(); view.render().props.onPointerDown(pointer)
  view.render().props.onChange({ target: { value: '0.1' } })
  view.update(.8)
  expect(view.render().props.value).toBe(.1)
  ...
  expect(view.render().props.value).toBe(.8); expect(view.onCommit).not.toHaveBeenCalled()
  view.update(.3); expect(view.render().props.value).toBe(.3)
})

interface InputProps { children?: ReactNode; 'aria-label'?: string; onChange?: (event: { target: { value: string } }) => void; onBlur?: () => void; onKeyDown?: (event: ReturnType<typeof keyboard>) => void }
function offset(node: ReactNode): InputProps {
  for (const child of Children.toArray(node)) {
    ...
    try { return offset(child.props.children) } catch { /* next */ }
  }
  throw new Error('Offset not found')
}
it('defers runtime exact offsets until Enter/blur and avoids duplicate commit on blur', () => {
  const onChange = vi.fn(), layer = { ...resolveLookLayers(initialShow, initialShow.looks[0])[0], offsetBeats: 0 }
  const render = () => { hooks.cursor = 0; return GroupTimingControls({ show: initialShow, layers: [layer], onChange, deferCommit: true }) }
  offset(render()).onChange!({ target: { value: '1' } }); offset(render()).onChange!({ target: { value: '1.25' } })
  expect(onChange).not.toHaveBeenCalled()
  offset(render()).onKeyDown!(keyboard('Enter')); offset(render()).onBlur!()
