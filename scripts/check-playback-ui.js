;async (page) => {
  ...
  try {
    ...
    assert(
      (await command(() => page.getByRole('button', { name: 'Tempo toepassen op runtime', exact: true }).click()))
        .bpm === 90,
      'BPM reaches runtime',
    )
    assert(
      (await command(() => page.getByRole('button', { name: 'Beeld vasthouden', exact: true }).click())).mode ===
        'static',
      'Static reaches runtime',
    )
    assert(
      (await command(() => page.getByRole('button', { name: 'Blackout', exact: true }).click())).mode === 'blackout',
      'Blackout reaches runtime',
    )
    ...
    assert(
      resumed.sessionId === ownId && resumed.frameCount > first.frameCount,
      'Session advances across browser reload',
    )
    assert(
      (await page.getByLabel('Tempo runtimeproef', { exact: true }).inputValue()) === '90',
      'Reconnect shows the actual runtime BPM',
    )
    ...
    assert(
      (await command(() => page.getByRole('button', { name: 'Stop runtimeproef', exact: true }).click())).status ===
        'stopped',
      'Stop works with invalid BPM',
    )
    ...
    if (ownId)
      await page.request.post('http://127.0.0.1:5188/playback/command', {
        data: { version: 1, sessionId: ownId, command: 'stop' },
      })
  }
}
