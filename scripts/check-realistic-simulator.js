...
;async (page) => {
  ...
  await page.evaluate((value) => {
    document.body.dataset.renderTiming = JSON.stringify(value)
  }, timing)
  ...
  await page.evaluate(async () => {
    ...
    const { fixtureProfiles } = await import('/src/fixtures.ts')
    const { evaluateFrame } = await import('/src/domain.ts')
    const frame = evaluateFrame(show, fixtureProfiles, { mode: 'automation', activeLookId: show.looks[1].id }, 0.25)
    view.update(frame)
  })
  await page.locator('#render-check').screenshot({ path: 'output/playwright/realistic-rgb.png' })
  await page.evaluate(() => {
    const { view, host, update } = window.renderCheck
    update('automation', 0.8)
    view.setCamera({ position: [0, 4, 9], target: [0, 1.4, 0], fov: 48 })
    host.style.width = '600px'
    host.style.height = '350px'
  })
  await page.locator('#render-check').screenshot({ path: 'output/playwright/realistic-haze.png' })
  await page.evaluate(() => {
    const { host, view } = window.renderCheck
    view.setCamera({ position: [0, 13, 8], target: [0, 1.4, 0], fov: 48 })
  ...
  const cleanup = await page.evaluate(() => {
    ...
    for (let i = 0; i < 3; i++) {
      const next = new StageSimulator(host, show.fixtures, show.camera, show.bandMembers)
      next.dispose()
    }
    const canvases = host.querySelectorAll('canvas').length
    host.remove()
    delete window.renderCheck
    return { textures, canvases }
  })
  if (cleanup.textures !== 0 || cleanup.canvases !== 0)
    throw new Error('Renderer resources retained: ' + JSON.stringify(cleanup))
