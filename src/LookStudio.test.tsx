...
describe('combined Look studio', () => {
  it('places the saved Look editor beside a preview with one Look picker and temporary tempo', () => {
    const html = renderToStaticMarkup(<LookStudio {...defaults} show={initialShow} selectedLookId={initialShow.looks[0].id} />)
  ...
  it('provides a concise desktop Look list with search for larger libraries and the same selected Look', () => {
    const looks = Array.from({ length: 7 }, (_, index) => ({ ...initialShow.looks[0], id: `look-${index}`, name: `Look ${index + 1}` }))
    const html = renderToStaticMarkup(<LookStudio {...defaults} show={{ ...initialShow, looks }} selectedLookId="look-3" />)
  ...
  it('omits search for a short Look list', () => {
    const html = renderToStaticMarkup(<LookStudio {...defaults} show={{ ...initialShow, looks: initialShow.looks.slice(0, 6) }} selectedLookId={initialShow.looks[0].id} />)
    expect(html).not.toContain('Zoek studio-Look')
  })
  it('offers a clear empty state without tempo or dangling group controls', () => {
    const html = renderToStaticMarkup(<LookStudio {...defaults} show={{ ...initialShow, looks: [] }} selectedLookId="removed" />)
  ...
  it('keeps a Look without groups editable and explains how to populate it', () => {
    const html = renderToStaticMarkup(<LookStudio {...defaults} show={{ ...initialShow, groups: [] }} selectedLookId={initialShow.looks[0].id} />)
