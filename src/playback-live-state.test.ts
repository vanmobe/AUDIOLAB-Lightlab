...
const makeLive = (): PlaybackLiveState => ({
  controls: emptyLiveControls(),
  groupIntensities: Object.fromEntries(initialShow.groups.map((g) => [g.id, g.intensity])),
  colorLockId: null,
})
describe('runtime live transport', () => {
  it('uses existing linked overrides, masters, color lock and timing without modifying loaded show', () => {
    ...
    live.controls = updateLiveGroups(initialShow, live.controls, 'wash', {
      mode: 'animation',
    ...
    live.groupIntensities.wash = 0.2
    live.groupIntensities.back = 0.2
    ...
    for (const mode of ['automation', 'static', 'safety', 'blackout'] as const) {
      const state: RuntimeState = { mode, activeLookId: initialShow.activeLookId, heldAtBeats: 0.25 }
      ...
      const direct = livePreview(
        { ...initialShow, groups: initialShow.groups.map((g) => ({ ...g, intensity: live.groupIntensities[g.id] })) },
        { ...state, colorLockId: live.colorLockId! },
        live.controls,
      )
  ...
  it('keeps native property-looking group IDs as own data', () => {
    const show = { ...initialShow, groups: [{ id: '__proto__', name: 'Group', intensity: 0.8 }] }
    const live = {
      controls: { overrides: JSON.parse('{"__proto__":{"intensity":0.2}}'), links: [] },
      groupIntensities: JSON.parse('{"__proto__":0.4}'),
      colorLockId: null,
    }
    expect(() => assertPlaybackLiveState(show, live)).not.toThrow()
    expect(
      playbackLivePreview(show, { mode: 'automation', activeLookId: show.activeLookId }, live).show.groups[0].intensity,
    ).toBe(0.4)
  })
  it('accepts bounded full-state collections larger than the old 16KiB command limit', () => {
    const groups = Array.from({ length: 256 }, (_, i) => ({
      id: `group-${i}-${'x'.repeat(70)}`,
      name: `Group ${i}`,
      intensity: 0.5,
    }))
    ...
    const live: PlaybackLiveState = {
      controls: {
        overrides: Object.fromEntries(
          groups.map((g) => [g.id, { mode: 'static', programId: null, rateBeats: 8, offsetBeats: -2 }]),
        ),
        links: [],
      },
      groupIntensities: Object.fromEntries(groups.map((g) => [g.id, 0.5])),
      colorLockId: null,
    }
