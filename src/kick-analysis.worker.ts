...
self.onmessage = (
  event: MessageEvent<{ samples: Float32Array; sampleRate: number; sensitivity: number; minIntervalMs: number }>,
) => {
  try {
    ...
    self.postMessage({ error: error instanceof Error ? error.message : 'Audioanalyse mislukt.' })
  }
}
