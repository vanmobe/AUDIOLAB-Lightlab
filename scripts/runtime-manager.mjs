        ...
        try {
          ...
          accept(
            response.statusCode === 200 &&
              ...
              value.output === 'disabled-by-default'
              ? 'ready'
              : 'occupied',
          )
        } catch {
...
export function createRuntimeManager({
  root,
  command = 'dotnet',
  args = ['run', '--project', resolve(root, 'runtime/Lightflow.Runtime.csproj'), '--launch-profile', 'Ollama'],
  cwd = resolve(root, 'runtime'),
  env = {},
  spawn = spawnProcess,
  probe = probeRuntime,
  prepare = () => access(resolve(root, 'runtime-worker/dist/engine.mjs')),
  ...
  let state = 'stopped',
    message = 'De lokale runtime is gestopt.',
    child,
    starting,
    checking,
    timer,
  ...
  let deadline = 0,
    ...
    startedAt = null
  const launchMode =
    command === 'dotnet' && args[0] === 'run' && !args.includes('--no-build') ? 'development' : 'prebuilt'
  const buildMessage =
  ...
  function snapshot() {
    return {
      ...
      state,
      canStart:
        !disposed && !child && !starting && !stopping && !portBlocked && (state === 'stopped' || state === 'error'),
      canStop: !disposed && !stopping && Boolean(child || starting),
      stopping,
      startedAt: state === 'external' ? null : startedAt,
      launchMode,
      buildMessage,
      message,
      ...
      try {
        ...
        if (process.platform !== 'win32' && owned.pid) killProcess(-owned.pid, value)
          ...
          const killer = spawnProcess(
            'taskkill.exe',
            ['/PID', String(owned.pid), '/T', ...(value === 'SIGKILL' ? ['/F'] : [])],
        ...
        /* Already exited. Never target a discovered external process. */
      }
    }
    signal('SIGTERM')
    ...
    checking = (async () => {
      ...
      if (child) {
        if (result === 'ready' && !stopping) transition('running', 'De lokale runtime is klaar.')
          ...
          stopOwned()
        } else if (state === 'running' && result !== 'ready')
          transition('error', 'De runtime reageert niet. Sluit deze Lightlab-sessie en start opnieuw.', 'error')
      } else if (result === 'ready')
        transition('external', 'Een bestaande lokale runtime is verbonden. Deze sessie beheert dat proces niet.')
      else if (result === 'occupied')
        transition('error', 'Poort 5188 is bezet door een proces dat niet als Lightlab-runtime reageert.', 'error')
      else if (state === 'external' || message.startsWith('Poort 5188'))
        transition('stopped', 'De lokale runtime is gestopt.')
      return result
    })()
    try {
    ...
    starting = (async () => {
      ...
      try {
        const owned = spawn(command, args, {
          ...
          env: {
            ...process.env,
            LIGHTFLOW_AI_PROVIDER: process.env.LIGHTFLOW_AI_PROVIDER ?? 'ollama',
            ...env,
            LIGHTLAB_RUNTIME_PORT: '5188',
            LIGHTLAB_NODE_PATH: process.execPath,
            LIGHTLAB_WORKER_PATH: resolve(root, 'runtime-worker/dist/engine.mjs'),
          },
        })
        child = owned
        ...
        const output = (chunk, stream) => {
          ...
          const category = /error (CS|MSB|NETSDK)\d/.test(prefix)
            ? 'build'
            : /address already in use|EADDRINUSE/i.test(prefix)
              ? 'port'
              : stream
          ...
          const text =
            category === 'build'
              ? 'De .NET-build meldt een fout. Controleer de lokale build in de terminal.'
              : category === 'port'
                ? 'De runtime meldt dat de luisterpoort bezet is.'
                : `De runtime heeft ${stream === 'stderr' ? 'diagnostische' : 'proces'}uitvoer geschreven. Ruwe inhoud wordt om privacyredenen niet getoond.`
        ...
        owned.once('error', (error) => {
          ...
          if (!disposed)
            transition(
              'error',
              error.code === 'ENOENT'
                ? 'De runtime kon niet starten: .NET of het runtimeprogramma ontbreekt.'
                : 'De runtime kon niet worden gestart.',
              'error',
            )
        })
        owned.once('exit', (code) => {
          ...
          child = undefined
          clearInterval(timer)
          if (!disposed && state !== 'error' && !wasStopping)
            transition(
              code === 0 ? 'stopped' : 'error',
              code === 0 ? 'De lokale runtime is gestopt.' : 'De lokale runtime is onverwacht gestopt.',
              code === 0 ? 'info' : 'error',
            )
        })
        timer = setInterval(() => {
        ...
        transition('error', 'De runtime kon niet worden gestart.', 'error')
      }
    })()
      .finally(() => {
        starting = undefined
  ...
  async function middleware(
    ...
    if (!path.startsWith('/__lightlab/runtime')) {
      next()
      return
    }
    ...
    if (!['127.0.0.1:5173', 'localhost:5173'].includes(host) || (origin !== undefined && origin !== `http://${host}`)) {
      send(403, { error: 'Alleen lokale toegang vanuit Lightlab is toegestaan.' })
      return
    }
    const action = path === '/__lightlab/runtime/start' ? 'start' : path === '/__lightlab/runtime/stop' ? 'stop' : null
    if (path !== '/__lightlab/runtime' && !action) {
      send(404, { error: 'Onbekende runtimeactie.' })
      return
    }
    if (path === '/__lightlab/runtime' && request.method === 'GET') {
      send(200, await status())
      return
    }
    if (!action || request.method !== 'POST') {
      send(405, { error: 'Deze methode is niet toegestaan.' })
      return
    }
    if (origin !== `http://${host}` || request.headers['x-lightlab-action'] !== action) {
      send(403, { error: 'De runtimeactie moet vanuit Lightlab worden aangevraagd.' })
      return
    }
    if (
      ...
      send(400, { error: 'De runtimeactie accepteert geen inhoud.' })
      return
    }
