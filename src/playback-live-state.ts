...
function keys(value: Record<string, unknown>, allowed: string[], required: string[] = []) {
  if (Object.keys(value).some(key => !allowed.includes(key)) || required.some(key => !Object.hasOwn(value, key))) throw new Error('Ongeldige live-instellingen.')
}
...
export function assertPlaybackLiveState(show: ShowDocument, value: unknown): asserts value is PlaybackLiveState {
  ...
  if (Object.keys(masters).length !== groups.size || Object.entries(masters).some(([id, level]) => !groups.has(id) || !finite(level, 0, 1))) throw new Error('Ongeldige groepsmasters.')
  if (live.colorLockId !== null && (typeof live.colorLockId !== 'string' || !colors.has(live.colorLockId))) throw new Error('Onbekend kleurprofiel.')
  ...
  for (const [id, raw] of Object.entries(overrides)) {
    ...
    if (Object.hasOwn(layer, 'mode') && !['animation', 'static', 'off'].includes(layer.mode as string)) throw new Error('Ongeldige groepsmodus.')
    if (Object.hasOwn(layer, 'programId') && layer.programId !== null && (typeof layer.programId !== 'string' || !programs.has(layer.programId))) throw new Error('Onbekend patroon.')
    if (layer.mode === 'animation' && layer.programId === null) throw new Error('Animatie mist een patroon.')
    if (Object.hasOwn(layer, 'colorProfileId') && layer.colorProfileId !== null && (typeof layer.colorProfileId !== 'string' || !colors.has(layer.colorProfileId))) throw new Error('Onbekend kleurprofiel.')
    if (Object.hasOwn(layer, 'intensity') && !finite(layer.intensity, 0, 1)) throw new Error('Ongeldige intensiteit.')
    if (Object.hasOwn(layer, 'rateBeats') && layer.rateBeats !== null && !finite(layer.rateBeats, .01, 1024)) throw new Error('Ongeldige beatduur.')
    if (Object.hasOwn(layer, 'offsetBeats') && !finite(layer.offsetBeats, -64, 64)) throw new Error('Ongeldige fase.')
  }
  if (!Array.isArray(controls.links) || controls.links.length > groups.size) throw new Error('Ongeldige groepskoppelingen.')
  const linked = new Set<string>()
  for (const link of controls.links) {
    if (!Array.isArray(link) || link.length < 2 || link.length > groups.size) throw new Error('Ongeldige groepskoppeling.')
    for (const id of link) {
      if (typeof id !== 'string' || !groups.has(id) || linked.has(id)) throw new Error('Dubbele of onbekende groepskoppeling.')
      linked.add(id)
    }
  }
}
...
export function playbackLivePreview(show: ShowDocument, state: RuntimeState, live: PlaybackLiveState) {
  const mastered = { ...show, groups: show.groups.map(group => ({ ...group, intensity: live.groupIntensities[group.id] })) }
