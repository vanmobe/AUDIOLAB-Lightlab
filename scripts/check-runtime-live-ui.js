;async (page) => {
  ...
  page.on('request', (request) => {
    if (request.url().includes('/output/')) output.push(request.url())
  })
  const read = async (path) => {
    const reply = await page.request.get(base + path)
    assert(reply.ok(), `GET ${path}: ${reply.status()}`)
    return reply.json()
  }
  ...
  let id,
    commandCount = 0
  page.on('request', (request) => {
    if (request.url().endsWith('/playback/command') && request.method() === 'POST') commandCount++
  })
  try {
    await page.evaluate(async () => {
      ...
      show.controlSurface.bindings = [
        ...
        },
        {
          ...
          action: 'mode',
          targetId: 'blackout',
          slot: { bank: 1, kind: 'button', index: 2 },
        },
        {
          id: 'test-color',
          label: 'Kleur',
          action: 'color-lock',
          targetId: 'neon',
          slot: { bank: 1, kind: 'button', index: 3 },
        },
        {
          id: 'test-master',
          label: 'Wash',
    ...
    const open = async () => {
      ...
      await page.getByRole('button', { name: 'Lokale runtime', exact: true }).click()
    }
    const ready = () =>
      page.waitForFunction(() => {
        const el = document.querySelector('.runtime-live-controls')
    ...
    const command = async (action) => {
      ...
      assert(response.status() === 200, `Command ${response.status()}`)
      await ready()
    ...
    assert(started.status() === 200, 'Runtime starts')
    id = (await started.json()).sessionId
    await ready()
    await page.locator('.runtime-live .stage canvas').waitFor()
    ...
    assert(
      (await command(() => page.getByRole('button', { name: 'Tempo toepassen', exact: true }).click())).status.bpm ===
        97,
      'Runtime owns BPM',
    )
    assert(
      (await command(() => page.getByLabel('Live knop 1: Refrein', { exact: true }).click())).status.lookId ===
        'neon-chorus',
      'WING look command',
    )
    ...
    await page
      .locator('.runtime-live .live-group-links')
      .getByRole('checkbox', { name: 'Front spots', exact: true })
      .check()
    assert(
      (
        await command(() => page.getByRole('button', { name: 'Koppel selectie', exact: true }).click())
      ).controls.links[0].includes('front'),
      'Link stored runtime-side',
    )
    const mastered = await command(async () => {
      await page.getByLabel('Live draaiknop 1: Wash', { exact: true }).fill('37')
      await page.getByLabel('Live draaiknop 1: Wash', { exact: true }).press('Tab')
    })
    assert(
      mastered.groupIntensities.wash === 0.37 && mastered.groupIntensities.front === 0.37,
      'Linked rotary changes both runtime masters',
    )
    const slider = page.getByLabel('Live draaiknop 1: Wash', { exact: true })
    await slider.scrollIntoViewIfNeeded()
    const box = await slider.boundingBox(),
      beforeDrag = commandCount
    await page.mouse.move(box.x + box.width * 0.37, box.y + box.height / 2)
    await page.mouse.down()
    await page.mouse.move(box.x + box.width * 0.82, box.y + box.height / 2, { steps: 14 })
    assert(
      commandCount === beforeDrag && !(await slider.isDisabled()),
      'Continuous drag stays interactive without intermediate commands',
    )
    ...
    assert(
      commandCount === beforeDrag + 1 &&
        dragged.groupIntensities.wash > 0.75 &&
        dragged.groupIntensities.front === dragged.groupIntensities.wash,
      'Final linked drag value reaches runtime',
    )
    await command(async () => {
    ...
    assert(
      timed.controls.overrides.wash.rateBeats === 8 && timed.controls.overrides.front.offsetBeats === 2,
      'Linked timing/offset reach runtime',
    )
    ...
    const parity = await page.evaluate(async (id) => {
      const [{ evaluateFrame }, { playbackLivePreview }, { fixtureProfiles }] = await Promise.all([
        import('/src/domain.ts'),
        import('/src/playback-live-state.ts'),
        import('/src/fixtures.ts'),
      ])
    ...
    assert(
      black.inspection.universes.length === 2 &&
        black.inspection.universes.every((u) => u.channels.every((x) => x === 0)),
      'Both universes remain black',
    )
    ...
    assert(
      reset.colorLockId === null &&
        Object.keys(reset.controls.overrides).length === 0 &&
        reset.controls.links.length === 1 &&
        reset.groupIntensities.wash === 0.37,
      'Look resets overrides/color but keeps masters/links',
    )
    const conflict = await page.request.post(base + '/playback/command', {
      data: {
        version: 1,
        sessionId: id,
        command: 'live',
        ...
        groupIntensities: old.groupIntensities,
        colorLockId: old.colorLockId,
      },
    })
    assert(conflict.status() === 409, 'Stale control update cannot undo new Look')
    const invalid = await page.request.post(base + '/playback/command', {
      data: {
        version: 1,
        sessionId: id,
        command: 'live',
        ...
        groupIntensities: reset.groupIntensities,
        colorLockId: 'unknown-palette',
      },
    })
    assert(
      invalid.status() === 400 && (await read(`/playback/preview?sessionId=${id}`)).revision === reset.revision,
      'Invalid reference is rejected without fault or state mutation',
    )
    ...
    assert(
      (await page.evaluate(() => localStorage.getItem('lightlab-active-package-v1'))) === initialStored,
      'Runtime edits never change saved show',
    )
    await page.locator('.runtime-live .live-group-panel > summary').click()
    const lastCanvas = await page.locator('.runtime-live .stage canvas').boundingBox()
    assert(
      Math.abs(firstCanvas.width - lastCanvas.width) < 1 && Math.abs(firstCanvas.height - lastCanvas.height) < 1,
      'Control changes keep preview geometry stable',
    )
    ...
    await page.getByRole('button', { name: 'Verbind met runtimesessie', exact: true }).click()
    await ready()
    await page.reload()
    await open()
    await page.getByRole('button', { name: 'Verbind met runtimesessie', exact: true }).click()
    ...
    assert(
      (await stopped).status() === 200 && (await read('/playback/status')).status === 'stopped',
      'Stop remains reachable with invalid tempo',
    )
    ...
    await page.unroute('**/playback/preview?*')
    if (id)
      await page.request.post(base + '/playback/command', { data: { version: 1, sessionId: id, command: 'stop' } })
  }
}
