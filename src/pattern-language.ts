...
export interface Pattern { version: 1; floor: number; steps: PatternStep[] }
...
function record(value: unknown, keys: string[]): asserts value is Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    || Object.keys(value).length !== keys.length || keys.some(key => !Object.hasOwn(value, key))) throw new Error('Patroon bevat ontbrekende of onbekende velden.')
}
...
export function validatePattern(value: unknown): asserts value is Pattern {
  ...
  if (value.version !== 1 || !bounded(value.floor, 0, 1) || !Array.isArray(value.steps) || value.steps.length < 1 || value.steps.length > 16) throw new Error('Ongeldige patroonversie, basisintensiteit of aantal stappen (1–16).')
  for (const step of value.steps) {
    record(step, ['selection', 'direction', 'envelope', 'width', 'trail', 'level', 'weight'])
    if (!selections.includes(step.selection as string) || !directions.includes(step.direction as string) || !envelopes.includes(step.envelope as string)
      || !bounded(step.width, 1, 8, true) || !bounded(step.weight, 1, 8, true) || !bounded(step.trail, 0, 1) || !bounded(step.level, 0, 1)) throw new Error('Ongeldige patroonstap: selectie, richting, verloop, breedte, lichtstaart, niveau of gewicht.')
  }
}
...
export function patternSignature(program: { effect: string; pattern?: Pattern }): string {
  ...
  const constants = steps.map(step => step.level <= floor ? floor : step.selection === 'all' && step.envelope === 'hold' ? step.level : undefined)
  const constant = constants[0]
  if (constant !== undefined && constants.every(value => value === constant)) {
    return 'recipe:' + JSON.stringify({ version: 1, floor: constant, steps: [{ selection: 'all', direction: 'forward', envelope: 'hold', width: 1, trail: 0, level: constant, weight: 1 }] })
  }
  const divisor = steps.reduce((value, step) => gcd(value, step.weight), 0)
  return 'recipe:' + JSON.stringify({ version: 1, floor, steps: steps.map(step => ({
...
export function patternDescription(pattern: Pattern): string {
  const selection = { all: 'Alle lampen', alternate: 'Afwisselende sets', moving: 'Lopend venster', random: 'Willekeurige lampen' }
  const direction = { forward: 'vooruit', reverse: 'achteruit', bounce: 'heen en weer', inward: 'naar het midden', outward: 'naar buiten' }
  const envelope = { hold: 'vast niveau', 'fade-in': 'opkomend', 'fade-out': 'uitdovend', pulse: 'ademend' }
  return pattern.steps.map(step => `${selection[step.selection]}${step.selection === 'moving' ? ` ${direction[step.direction]} (breedte ${step.width}${step.trail > 0 ? `, lichtstaart ${Math.round(step.trail * 100)}%` : ''})` : step.selection === 'random' ? ` (${step.width})` : ''}, ${envelope[step.envelope]}${step.level < 1 ? ` op ${Math.round(step.level * 100)}%` : ''}`)
...
function moving(step: PatternStep, progress: number, point: number, count: number) {
  ...
  if (step.direction === 'bounce') {
    backwards = progress >= .5
    progress = progress < .5 ? progress * 2 : (1 - progress) * 2
  }
...
export function evaluatePattern(pattern: Pattern, phase: number, point: number, count: number): number {
  ...
  let start = 0, index = 0
  while (index < pattern.steps.length - 1 && position >= start + pattern.steps[index].weight) start += pattern.steps[index++].weight
  const step = pattern.steps[index]
  const progress = (position - start) / step.weight
  const envelope = step.envelope === 'hold' ? 1 : step.envelope === 'fade-in' ? progress : step.envelope === 'fade-out' ? 1 - progress : Math.sin(progress * Math.PI) ** 2
  let selected = 1
  if (step.selection === 'moving') selected = moving(step, progress, point, count)
    ...
    for (let slot = 0; slot < Math.min(step.width, count); slot++) {
      if ((seed % count + slot * stride) % count === point) { selected = 1; break }
    }
  }
