...
const evaluate = (atBeats = 2, state: RuntimeState = { mode: 'automation', activeLookId: initialShow.activeLookId }) => ({ version: 1, requestId: 'frame', op: 'evaluate', atBeats, state })
...
describe('autonomous evaluator protocol', () => {
  ...
  it('rejects oversized rigs and unknown personalities without modifying the snapshot', () => {
    const handle = createEngineProtocol(); handle(load())
  ...
  it('rejects extra fields, invalid references, unsupported versions and invalid time', () => {
    const handle = createEngineProtocol(); handle(load())
    for (const request of [null, { ...load(), version: 2 }, { ...evaluate(), requestId: 'a'.repeat(121) },
      { ...evaluate(), script: 'untrusted' }, { ...evaluate(), atBeats: -1 }, { ...evaluate(), atBeats: Infinity },
...
describe('bundled Node process parity', () => {
  beforeAll(async () => { await build({ configFile: 'vite.engine.config.ts', logLevel: 'silent' }) })
  const run = (input: string) => spawnSync(process.execPath, [resolve('runtime-worker/dist/engine.mjs')], { input, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024, timeout: 10_000 })
  it('carries individual-kick fades through the real bundled process without changing the audio wire clock', () => {
    ...
    const analysis = { duration: 10, kicks: [{ time: .7, strength: 1 }, { time: 1.4, strength: 1 }], bpm: 90, confidence: 1 }
    const player = createAudioLivePlayer(), sharedAnalysis = { ...analysis, waveform: [] }
    const requests: unknown[] = [load(show), { version: 1, requestId: 'audio', op: 'audio', analysis }]
    const expected: unknown[] = [{ version: 1, requestId: 'load', ok: true }, { version: 1, requestId: 'audio', ok: true }]
    for (const [seconds, look, mode] of [[0, 0, 'automation'], [.7, 1, 'automation'], [1, 1, 'automation'],
      [1.4, 1, 'automation'], [1.4, 0, 'automation'], [1.5, 0, 'static'], [2, 0, 'blackout']] as const) {
      const state: RuntimeState = { activeLookId: show.looks[look].id, mode }
      const position = { seconds, playing: true, mode: 'kicks' as const, bpm: 90, decayMs: 100, floor: .2,
        reactions: Object.fromEntries(show.groups.map(group => [group.id, 'pulse' as const])) }
      const beat = seconds * position.bpm / 60
      const result = player.evaluate(show, state, beat, { ...position, analysis: sharedAnalysis } satisfies AudioLiveSnapshot)
      result.frame.atBeats = beat
      if (seconds === 1) expect(result.transition?.progress).toBeCloseTo(.225)
  ...
  it('matches the browser transition controller through queued, interrupted, held and blackout frames', () => {
    ...
    const samples = [
      ...
      { at: .5, state: { mode: 'automation', activeLookId: 'neon-chorus' } },
    ...
    const expected = [{ version: 1, requestId: 'load', ok: true }, ...samples.map(sample => ({ version: 1, requestId: 'frame', ok: true, ...player.evaluate(show, fixtureProfiles, sample.state, sample.at) }))]
    const child = run(requests.map(request => JSON.stringify(request)).join('\n') + '\n')
    expect(child.status, child.stderr).toBe(0)
    expect(child.stdout.trim().split('\n').map(line => JSON.parse(line))).toEqual(expected)
  })
  it('matches direct shared-source playback for every legacy effect and layered recipes at exact beats', () => {
    const requests: unknown[] = [], expected: unknown[] = []
    ...
    recipe.programs[0].pattern = { version: 1, floor: .2, steps: [
      { selection: 'moving', direction: 'inward', envelope: 'fade-out', width: 1, trail: .5, level: 1, weight: 2 },
      { selection: 'random', direction: 'forward', envelope: 'pulse', width: 2, trail: 0, level: .8, weight: 1 },
    ] }
    recipe.looks[0].layers = recipe.looks[0].layers!.map((layer, index) => ({ ...layer,
      mode: layer.mode === 'off' ? 'off' : 'animation', programId: layer.mode === 'off' ? null : recipe.programs[0].id,
      rateBeats: index + .5, offsetBeats: index % 2 ? -.75 : .25,
    }))
    expect(evaluateFrame(recipe, fixtureProfiles, { mode: 'automation', activeLookId: recipe.activeLookId }, .375)
      .fixtures.some(fixture => fixture.segments?.length === 4)).toBe(true)
    shows.push(recipe)
    // Additive show settings must cross the real process boundary with exact browser parity.
    shows.push({ ...recipe, regie: { minimumCoverage: { percent: 80, threshold: .1 }, colorRoles: ['primary', 'secondary', 'accent', 'white'], safetyGroupIds: ['wash'] } })
    for (const show of shows) {
      requests.push(load(show)); expected.push({ version: 1, requestId: 'load', ok: true })
      for (const at of [0, .375, 128.7]) for (const mode of ['automation', 'static', 'safety', 'blackout'] as const) {
        const state = { mode, activeLookId: show.activeLookId, heldAtBeats: .375, colorLockId: 'neon' }
        requests.push(evaluate(at, state))
        expected.push({ version: 1, requestId: 'frame', ok: true, frame: evaluateFrame(show, fixtureProfiles, state, at) })
      }
    }
    const child = run(requests.map(request => JSON.stringify(request)).join('\n') + '\n')
    expect(child.status, child.stderr).toBe(0)
  ...
  it('recovers from malformed lines and accepts a final complete message at EOF', () => {
    ...
    expect(child.status, child.stderr).toBe(0)
    expect(child.stdout.trim().split('\n').map(line => JSON.parse(line).ok)).toEqual([false, true, true])
  })
  it('carries live overrides, linked groups, masters and lock through the actual process protocol', () => {
    const live: PlaybackLiveState = {
      controls: { overrides: {
        wash: { mode: 'animation', programId: initialShow.programs[1].id, rateBeats: 8, offsetBeats: -2, intensity: .7 },
        back: { mode: 'animation', programId: initialShow.programs[1].id, rateBeats: 8, offsetBeats: -2, intensity: .7 },
      }, links: [['wash', 'back']] },
      groupIntensities: Object.fromEntries(initialShow.groups.map(g => [g.id, .35])),
      colorLockId: initialShow.colorProfiles[1].id,
    }
    const requests: unknown[] = [load(), { version: 1, requestId: 'bad', op: 'validate-live', live: { ...live, colorLockId: 'missing' } },
      { version: 1, requestId: 'valid', op: 'validate-live', live }]
    const expectedFrames = []
    for (const mode of ['automation', 'static', 'safety', 'blackout'] as const) {
      const state = { mode, activeLookId: initialShow.activeLookId, heldAtBeats: .5 }
