  ...
  while (changed) {
    ...
    for (const link of controls.links) {
      ...
      for (const id of link)
        if (valid.has(id) && !connected.has(id)) {
          connected.add(id)
          changed = true
        }
    }
...
export function unlinkLiveGroup(controls: LiveControls, groupId: string): LiveControls {
  return {
    ...controls,
    links: controls.links.map((link) => link.filter((id) => id !== groupId)).filter((link) => link.length >= 2),
  }
}

export function updateLiveGroups(
  show: ShowDocument,
  ...
  for (const id of targets)
    overrides = { ...overrides, [id]: { ...(Object.hasOwn(overrides, id) ? overrides[id] : {}), ...change } }
  return { ...controls, overrides }
}

export function applyLiveGroupLook(
  show: ShowDocument,
  controls: LiveControls,
  groupId: string,
  lookId: string,
  ...
  for (const { groupId: id, ...layer } of resolveLookLayers(show, look)) {
    ...
    if (targets.has(id))
      overrides = {
        ...
        [id]: {
          ...layer,
          colorProfileId: layer.colorProfileId ?? look.colorProfileId,
          // Explicit resets prevent timing from leaking in from the currently active base Look.
          rateBeats: layer.rateBeats ?? 1,
...
function mergedLayer(show: ShowDocument, base: LookLayer, override: LayerChange | undefined): LookLayer {
  ...
  const layer = { ...base }
  if (override.colorProfileId === null || show.colorProfiles.some((profile) => profile.id === override.colorProfileId))
    layer.colorProfileId = override.colorProfileId!
  if (
    typeof override.intensity === 'number' &&
    Number.isFinite(override.intensity) &&
    override.intensity >= 0 &&
    override.intensity <= 1
  )
    layer.intensity = override.intensity
  if (override.rateBeats === null) layer.rateBeats = 1
  else if (
    typeof override.rateBeats === 'number' &&
    Number.isFinite(override.rateBeats) &&
    override.rateBeats >= 0.01 &&
    override.rateBeats <= 1024
  )
    layer.rateBeats = override.rateBeats
  if (
    typeof override.offsetBeats === 'number' &&
    Number.isFinite(override.offsetBeats) &&
    override.offsetBeats >= -64 &&
    override.offsetBeats <= 64
  )
    layer.offsetBeats = override.offsetBeats
...
export function livePreview(
  ...
  state: RuntimeState,
  controls: LiveControls,
): { show: ShowDocument; state: RuntimeState } {
  ...
  const layers = base.map((layer) =>
    mergedLayer(
      show,
      layer,
      Object.hasOwn(controls.overrides, layer.groupId) ? controls.overrides[layer.groupId] : undefined,
    ),
  )
  if (layers.every((layer, index) => JSON.stringify(layer) === JSON.stringify(base[index]))) return { show, state }
  return {
    show: { ...show, looks: show.looks.map((item) => (item.id === look.id ? { ...item, layers } : item)) },
    state,
  }
}
