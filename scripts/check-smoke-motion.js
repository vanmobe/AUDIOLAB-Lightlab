async (page) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    Object.assign(host.style, { position: 'fixed', left: '0', top: '0', width: '900px', height: '600px', zIndex: '9999' })
    ...
    const sample = (time, haze, currentFrame = frame) => {
      ...
      const ctx = canvas.getContext('2d'); ctx.drawImage(sim.renderer.domElement, 0, 0, 900, 600)
    ...
    const a = sample(0, 80), b = sample(8, 80)
    const clearDelta = delta(sample(0, 0), sample(8, 0))
    const black = { ...frame, fixtures: frame.fixtures.map(f => ({ ...f, intensity: 0, segments: f.segments?.map(s => ({ ...s, intensity: 0 })) })) }
    const blackDelta = delta(sample(0, 80, black), sample(8, 80, black))
    sim.smokeTime.value = 0; sim.previousRenderTime = 1000
    sim.render(1050); cancelAnimationFrame(sim.animationFrame)
    const advanced = sim.smokeTime.value
    sim.reducedMotion = { matches: true }
    sim.render(1100); cancelAnimationFrame(sim.animationFrame)
    const frozen = sim.smokeTime.value === advanced
    sim.reducedMotion = { matches: false }
    sim.render(100000); cancelAnimationFrame(sim.animationFrame)
    const bounded = sim.smokeTime.value - advanced <= .101
    sim.dispose(); host.remove()
    return { motionDelta: delta(a, b), clearDelta, blackDelta, advanced, frozen, bounded, immutable: JSON.stringify(frame) === source, images: [a.url, b.url] }
  })
  for (let i = 0; i < result.images.length; i++) {
    await page.evaluate(url => { const img = document.createElement('img'); img.id = 'smoke-proof'; img.src = url; document.body.append(img) }, result.images[i])
  ...
  if (!(result.motionDelta > 10000 && result.clearDelta === 0 && result.blackDelta === 0 && result.advanced > 0 && result.frozen && result.bounded && result.immutable) || errors.length) throw new Error(JSON.stringify({ result, errors }))
  return result
}
