...
export interface ActivePackageLoad {
  bundle: ShowPackage
  blocked: boolean
  message: string
...
export function persistActivePackage(storage: PackageStorage, bundle: ShowPackage) {
  ...
  try {
    storage.setItem(activePackageKey, raw)
  } catch {
    throw new Error(
      'Actieve show kon niet worden opgeslagen. Je huidige show blijft intact; exporteer je werk als back-up.',
    )
  }
}
...
export function loadActivePackage(storage: PackageStorage, fallbackShow: ShowDocument): ActivePackageLoad {
  ...
  try {
    ...
    source = 'legacy'
    const show = storage.getItem('lightflow-show-v1'),
      versions = storage.getItem('lightflow-versions-v1')
    ...
    const bundle = parseShowPackage(
      JSON.stringify({
        format: 'lightflow-show',
        version: 1,
        show: show === null ? fallbackShow : JSON.parse(show),
        versions: versions === null ? [] : JSON.parse(versions),
      }),
    )
    try {
      ...
      return {
        ...
        source,
        message:
          'Migratie naar atomaire showopslag is mislukt. Oude gegevens zijn behouden; exporteer je werk. Automatisch bewaren is geblokkeerd.',
      }
    }
    ...
    return {
      ...
      source,
      message:
        'Lokale showopslag is ongeldig of ontoegankelijk. Oorspronkelijke gegevens blijven onaangeroerd; automatisch bewaren is geblokkeerd. Exporteer tijdelijk werk en herstel de opgeslagen gegevens voordat je verder bewaart.',
    }
  }
}
