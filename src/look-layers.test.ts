...
const layer = (
  ...
  colorProfileId: string | null = null,
  intensity = 1,
): LookLayer => ({ groupId, mode, programId, colorProfileId, intensity })
...
describe('group layers in Looks', () => {
  it('keeps fronts steady while wash pulses, without using legacy program targets', () => {
    ...
    const first = frame(show, 0),
      later = frame(show, 0.25)
    const front = first.fixtures.find((f) => f.fixtureId === 'front-1')!
    expect(front.intensity).toBeCloseTo(0.8 * 0.85)
  ...
  it('fixed palettes bypass color lock; following layers inherit it', () => {
    ...
    expect(locked.fixtures[0].intensity).toBeCloseTo(0.75 * 0.85)
    expect(locked.fixtures[4].intensity).toBeCloseTo(0.8 * 0.9)
  })
  it('multiplies layer intensity, group master and profile limit, including zero masters', () => {
    const show = layeredShow()
    show.looks[0].layers = [layer('wash', 'static', null, null, 0.5)]
    expect(frame(show).fixtures[0].intensity).toBeCloseTo(0.5 * 0.75 * 0.85)
  ...
  it('safety and blackout override off layers, palettes and layer intensity', () => {
    ...
    expect(safety.fixtures.find((f) => f.fixtureId === 'front-1')!.intensity).toBeCloseTo(0.8 * 0.8)
    expect(safety.fixtures[0].intensity).toBe(0)
    expect(
      frame(show, 0, { ...state, mode: 'blackout' }).fixtures.every((f) => f.intensity === 0 && f.haze === 0),
    ).toBe(true)
  })
  it('freezes all animation layers on the same held beat', () => {
    const show = layeredShow()
    show.looks[0].layers!.push(layer('back', 'animation', 'chorus'))
    const frozen = { ...state, mode: 'static' as const, heldAtBeats: 0.37 }
  ...
  it.each(animationEffects)('converting legacy $id retains frames, ordering and per-head behavior', (effect) => {
    ...
    for (const at of [0, 0.17, 0.51, 2.35]) {
...
describe('persisted Look layer validation', () => {
  it.each([
    ...
    { programId: 'missing' },
    { programId: null },
    { colorProfileId: 'missing' },
    { colorProfileId: undefined },
    { mode: 'static', programId: 'pulse' },
    { mode: 'off', programId: 'pulse' },
