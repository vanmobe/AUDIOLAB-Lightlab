...
function runtime() {
  return { show: undefined, preview: undefined, status: { version: 1, sessionId: 'test', status: 'stopped', mode: 'blackout', lookId: null, bpm: 120, atBeats: 0, frameCount: 0, universeCount: 0, outputSent: false, error: null }, pending: false, connected: false, error: '', start: vi.fn(), loadEditorShow: vi.fn(), attach: vi.fn(), stop: vi.fn(), command: vi.fn(), setLive: vi.fn(), refreshStatus: vi.fn() } as ReturnType<typeof useRuntimeLive>
}
function find(node: ReactNode, type: unknown): ReactNode {
  for (const item of Children.toArray(node)) if (isValidElement<{ children?: ReactNode }>(item)) {
    if (item.type === type) return item
    const child = find(item.props.children, type); if (child) return child
  }
  return null
}
it('exposes runtime, show, DMX and WING without starting or commanding anything on render', () => {
  ...
  for (const label of ['Start runtime', 'Stop runtime', 'Livesessie', 'DMX-uitvoer', 'Synchroniseren met WING', 'Logging']) expect(html).toContain(label)
  expect(state.start).not.toHaveBeenCalled(); expect(state.command).not.toHaveBeenCalled(); expect(state.stop).not.toHaveBeenCalled()
})
it('keeps the same WAV session component when switching Live to Runtime management', () => {
  ...
  function Capture({ management }: { management: boolean }) { const tree = RuntimeLiveView({ ...props, management }); views.push(tree); return null }
  renderToStaticMarkup(<Capture management={false} />); renderToStaticMarkup(<Capture management />)
