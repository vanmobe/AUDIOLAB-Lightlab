...
function session() {
  ...
  show.controlSurface.bindings = [
    ...
    { id: 'b', label: 'Donker', action: 'mode', targetId: 'blackout', slot: { bank: 1, kind: 'button', index: 2 } },
    {
      ...
      label: 'Kleur',
      action: 'color-lock',
      targetId: show.colorProfiles[0].id,
    ...
    { id: 'd', label: 'Volg Look', action: 'color-lock', slot: { bank: 1, kind: 'button', index: 4 } },
    {
      ...
      targetId: show.groups[0].id,
      slot: { bank: 1, kind: 'rotary', index: 1 },
    },
  ]
  const props = {
    show,
    state: { mode: 'automation' as const, activeLookId: show.looks[0].id },
    modified: false,
    linkedGroups: [],
    onLook: vi.fn(),
    onMode: vi.fn(),
...
describe('Live WING surface', () => {
  it('dispatches assigned actions without changing assignments or inventing mappings', () => {
    const view = session(),
      before = structuredClone(view.props.show)
  ...
  it('uses stored master values and dispatches normalized rotary levels including zero', () => {
    const view = session()
    view.props.show.groups[0].intensity = 0.37
    ...
    expect(view.props.onIntensity.mock.calls).toEqual([
      [view.props.show.groups[0].id, 0],
      [view.props.show.groups[0].id, 1],
    ])
  })
