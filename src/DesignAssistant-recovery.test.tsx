...
interface Props { children?: ReactNode; disabled?: boolean; onClick?: () => Promise<void> | void; onChange?: (event: { target: { value: string } }) => void }
...
function find(node: ReactNode, match: (props: Props) => boolean): Props {
  for (const child of Children.toArray(node)) {
    ...
    try { return find(child.props.children, match) } catch { /* next sibling */ }
  }
...
describe('AI duplicate rejection recovery', () => {
  it('offers a corrective retry with the same settings and an unchanged show', async () => {
    ...
    const show = structuredClone(initialShow), saved = structuredClone(show), onAccept = vi.fn()
    const fetch = vi.fn().mockImplementation(async () => Response.json({ error: 'Voorstel afgewezen: Dubbel animatierecept: naam, snelheid, kleur of groep maken geen nieuw patroon.' }, { status: 502 }))
    vi.stubGlobal('fetch', fetch)
    const render = () => { hooks.cursor = 0; return DesignAssistant({ show, onAccept }) }
