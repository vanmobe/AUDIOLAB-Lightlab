...
export function layerAnimationLabel(program: AutomationProgram, layer: LookLayer) {
  ...
  return (
    ...
    (program.pattern || program.effect !== 'static' ? ` · ${duration} ${duration === 1 ? 'beat' : 'beats'}` : '') +
    ((program.pattern || program.effect !== 'static') && offset !== 0
      ? ` · offset ${offset > 0 ? '+' : ''}${offset} beats`
      : '')
  )
}
...
export function GroupTimingControls({
  layers,
  show,
  onChange,
  deferCommit = false,
  ...
  layers: LookLayer[]
  show: ShowDocument
  onChange: (change: TimingChange) => void
  deferCommit?: boolean
}) {
  const rate = commonTiming(
    layers.map((layer) => ({
      ...layer,
  ...
  const options =
    typeof rate === 'number' && !durations.includes(rate) ? [...durations, rate].sort((a, b) => a - b) : durations
  const inactive = layers.some((layer) => {
    const program = show.programs.find((program) => program.id === layer.programId)
    return layer.mode !== 'animation' || (!program?.pattern && program?.effect === 'static')
  })
  const summary =
    rate === mixedTiming || offset === mixedTiming
      ? ' · Gemengd'
      : `${rate === null ? '' : ` · ${rate} ${rate === 1 ? 'beat' : 'beats'}`}${offset === 0 ? '' : ` · offset ${offset > 0 ? '+' : ''}${offset} beats`}`
  function commitDraft() {
    const value = validOffset(draft)
    if (value === undefined) {
      if (offset === mixedTiming && !draft.trim()) return
      setError('Kies een offset van −64 tot 64 beats.')
      return
    }
  ...
  return (
    <details className="group-timing-controls">
      <summary>Timing{summary}</summary>
      <p>
        Duur bepaalt hoe snel het patroon loopt. Offset verschuift de fase: positief loopt achter, negatief loopt
        vooruit. Geen startvertraging.
      </p>
      {inactive && (
        <p className="timing-note">
          Bij vast licht of uit is timing niet actief. De instellingen blijven bewaard; wijzigen schakelt geen animatie
          in.
        </p>
      )}
      <label>
        ...
        <select
          aria-label="Groepsduur in beats"
          value={rate ?? 1}
          onChange={(event) => onChange({ rateBeats: Number(event.target.value) })}
        >
          {rate === mixedTiming && (
            <option value={mixedTiming} disabled>
              Gemengd
            </option>
          )}
          {options.map((value) => (
            <option key={value} value={value}>
              ...
              {!durations.includes(value) ? ' (aangepast)' : ''}
            </option>
          ))}
        </select>
      </label>
      <label>
        Offset kiezen
        <select
          aria-label="Groepsoffset preset"
          value={offset === mixedTiming ? mixedTiming : offsets.includes(offset) ? offset : ''}
          onChange={(event) => {
        ...
        >
          ...
          {offset !== mixedTiming && !offsets.includes(offset) && (
            <option value="" disabled>
              {offset} beats (aangepast)
            </option>
          )}
          {offsets.map((value) => (
            <option key={value} value={value}>
              {value > 0 ? '+' : ''}
              {value} {Math.abs(value) === 1 ? 'beat' : 'beats'}
              {value === 0 ? ' · gelijk' : value > 0 ? ' · achter' : ' · vooruit'}
            </option>
          ))}
        </select>
      </label>
      <label>
        Exacte offset (beats)
        <input
          aria-label="Exacte groepsoffset in beats"
          type="number"
          min="-64"
          max="64"
          step="any"
          placeholder={offset === mixedTiming ? 'Gemengd' : undefined}
          value={draft}
          onChange={(event) => {
          ...
          onBlur={commitDraft}
          onKeyDown={(event) => {
            if (deferCommit && event.key === 'Enter') {
              event.preventDefault()
              commitDraft()
            }
            if (deferCommit && event.key === 'Escape') {
              setDraft(offset === mixedTiming ? '' : String(offset))
              setError('')
            }
          }}
      ...
      <small>
        {deferCommit
          ? 'Exacte offset toepassen met Enter of bij het verlaten van het veld.'
          : 'Geldige invoer wordt meteen toegepast.'}{' '}
        {layers.length > 1 ? 'Wijzigingen gelden voor alle gekoppelde groepen.' : ''}
      </small>
      <p className="timing-effective">
        {layers
          .map((layer) => {
      ...
      <button
        onClick={() => {
          setDraft('0')
          setError('')
          onChange({ rateBeats: 1, offsetBeats: 0 })
        }}
      >
        Timing naar 1 beat / offset 0
      </button>
    </details>
  )
}
