...
describe('show persistence validation', () => {
  it('preserves optional band profiles in packages and snapshots, with old shows unchanged', () => {
    const show = { ...initialShow, bandProfile: { ...defaultBandProfile, name: 'Band', genres: 'Soul', preferredColors: ['#ff6600'] } }
  ...
  it.each([null, {}, { ...defaultBandProfile, bpm: 120 }, { ...defaultBandProfile, preferredColors: ['red'] }, { ...defaultBandProfile, energy: 'extreme' }])('rejects invalid persisted band profile %j', bandProfile => {
  ...
  it.each([
    null, {}, { bank: 0, kind: 'button', index: 1 }, { bank: 65, kind: 'button', index: 1 },
    { bank: 1.5, kind: 'button', index: 1 }, { bank: 1, kind: 'button', index: 0 },
    { bank: 1, kind: 'button', index: 65 }, { bank: 1, kind: 'button', index: '1' },
    { bank: 1, kind: 'fader', index: 1 }, { bank: 1, kind: 'rotary', index: 1 },
  ...
  it.each([null, [], { '0': 'Test' }, { '65': 'Test' }, { '01': 'Test' }, { '1.5': 'Test' }, { '1': '' }, { '1': ' ' }, { '1': 3 }, { '1': 'x'.repeat(81) }])('rejects malformed control bank names %j', bankNames => {
    const show = structuredClone(initialShow)
  ...
  it.each([
    ['missing groups', (show: any) => { delete show.groups }],
    ['malformed fixture', (show: any) => { show.fixtures[0] = null }],
    ['invalid position', (show: any) => { show.fixtures[0].position = [0, '1', 2] }],
    ['nonfinite aim', (show: any) => { show.fixtures[0].aim[0] = Infinity }],
    ['invalid camera', (show: any) => { show.camera.position = {} }],
    ['invalid fov', (show: any) => { show.camera.fov = 180 }],
    ['unknown group', (show: any) => { show.fixtures[0].groupId = 'missing' }],
    ['duplicate group', (show: any) => { show.groups.push(show.groups[0]) }],
    ['unknown palette', (show: any) => { show.programs[0].defaultColorProfileId = 'missing' }],
    ['unknown program', (show: any) => { show.looks[0].programId = 'missing' }],
    ['unknown active look', (show: any) => { show.activeLookId = 'missing' }],
    ['unknown control target', (show: any) => { show.controlSurface.bindings[0].targetId = 'missing' }],
    ['invalid color', (show: any) => { show.colorProfiles[0].primary = 'red' }],
    ['fractional patch', (show: any) => { show.fixtures[0].patch.address = 1.5 }],
    ['oversized name', (show: any) => { show.name = 'x'.repeat(1025) }],
    ['too many palettes', (show: any) => { show.colorProfiles = Array.from({ length: 33 }, (_, i) => ({ ...show.colorProfiles[0], id: String(i) })) }],
  ...
  it('accepts the supported 32-item boundary and does not mutate snapshots', () => {
    const show = structuredClone(initialShow)
    while (show.colorProfiles.length < 32) show.colorProfiles.push({ ...show.colorProfiles[0], id: `extra-${show.colorProfiles.length}` })
    expect(() => assertShowDocument(show)).not.toThrow()
