...
function rig() {
  ...
  program.pattern = { version: 1, floor: 0, steps: [{ selection: 'all', direction: 'forward', envelope: 'hold', width: 1, trail: 0, level: 0, weight: 1 }] }
  show.looks[0].layers = show.groups.map(group => ({ groupId: group.id, mode: 'animation', programId: program.id, colorProfileId: null, intensity: 1, rateBeats: 1, offsetBeats: 0 }))
  show.activeLookId = show.looks[0].id
  show.groups.forEach(group => { group.intensity = 1 })
  show.colorProfiles.forEach(profile => { profile.intensityLimit = 1; profile.primary = '#ff0000'; profile.accent = '#00ff00'; profile.secondary = '#0000ff'; profile.white = '#ffffff' })
  return show
}
function frame(show: ReturnType<typeof rig>, mode: 'automation' | 'static' | 'safety' | 'blackout' = 'automation') { return evaluateFrame(show, fixtureProfiles, { activeLookId: show.activeLookId, mode, heldAtBeats: .5 }, 2) }
describe('show regie', () => {
  ...
  it('raises exactly the required number of dark controllable heads, excluding haze', () => {
    const show = rig(); show.regie!.minimumCoverage = { percent: 80, threshold: .1 }
    const before = JSON.stringify(show), result = frame(show), report = coverageReport(show, fixtureProfiles, result)
  ...
  it('never defeats blackout, safety, off, black colors, zero masters or palette caps', () => {
    const show = rig(); show.regie!.minimumCoverage.percent = 100
    ...
    for (const fixture of show.fixtures.filter(fixture => ['wash', 'back'].includes(fixture.groupId))) expect(result.fixtures.find(light => light.fixtureId === fixture.id)!.intensity).toBe(0)
    ...
    expect(safety.fixtures.every(light => show.fixtures.find(fixture => fixture.id === light.fixtureId)!.groupId === 'back' || light.intensity === 0)).toBe(true)
    show.groups.forEach(group => { group.intensity = 1 })
    show.colorProfiles.forEach(profile => { profile.intensityLimit = .05 })
    expect(coverageReport(show, fixtureProfiles, frame(show)).lit).toBe(0)
    show.colorProfiles.forEach(profile => { profile.intensityLimit = 1; profile.primary = '#000000'; profile.accent = '#000000' })
  ...
  it('counts and corrects independent bar heads even for legacy aggregate effects', () => {
    ...
    show.regie!.minimumCoverage = { percent: 80, threshold: .8 }
    const result = frame(show)
  ...
  it('uses all selected RGB roles while fixed-white spots retain their physical color', () => {
    const show = rig(); show.regie!.colorRoles = ['secondary']
    const result = frame(show)
    ...
    const colors = new Set(frame(show).fixtures.flatMap(light => light.segments?.map(segment => segment.color) ?? [light.color]))
  ...
  it('validates persisted settings and rejects unknown, duplicate or dangling choices', () => {
    const show = rig(); expect(() => assertShowDocument(show)).not.toThrow()
    for (const regie of [{ ...show.regie, colorRoles: [] }, { ...show.regie, colorRoles: ['primary', 'primary'] }, { ...show.regie, safetyGroupIds: ['missing'] }, { ...show.regie, minimumCoverage: { percent: 101, threshold: .1 } }, { ...show.regie, minimumCoverage: { percent: 80, threshold: 0 } }, { ...show.regie, code: 'x' }]) expect(() => assertShowDocument({ ...show, regie })).toThrow()
  })
})
