...
it('hydrates a discovered session tempo once and preserves later user drafts', async () => {
  ...
  view.button('Status ophalen').onClick!()
  await tick()
...
it('starts explicitly, preserves snapshot identity, and keeps stop available with dirty patch and bad tempo', async () => {
  hooks.get.mockResolvedValue(idle)
  hooks.start.mockResolvedValue(running)
  hooks.command.mockResolvedValue({ ...running, status: 'stopped' })
  const view = setup()
  view.render()
  hooks.effects[0]()
  await tick()
  view.button('Start opgeslagen snapshot').onClick!()
  await tick()
  expect(hooks.start.mock.calls[0].slice(0, 3)).toEqual([initialShow, initialShow.activeLookId, 120])
  view.change({ ...initialShow, name: 'Edited snapshot' })
  view.dirty()
  ...
  view.button('Stop runtimeproef').onClick!()
  await tick()
...
it('does not retry a conflicting command and reads the actual session instead', async () => {
  hooks.get
    ...
    .mockResolvedValueOnce({ ...running, sessionId: 'new-session', lookId: 'other-look' })
...
it('polls only while expanded and suspends automatic requests for an old runtime', async () => {
  ...
  const view = setup()
  view.render()
  const unmount = hooks.effects[0]() as () => void
  hooks.effects[1]()
  await tick()
  ...
  view.render()
  const close = hooks.effects[1]() as () => void
