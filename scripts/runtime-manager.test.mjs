...
async function request(manager, { url = '/__lightlab/runtime', method = 'GET', headers = {} } = {}) {
  ...
  await manager.middleware(
    { url, method, headers: { host: '127.0.0.1:5173', ...headers } },
    {
      writeHead(value, supplied) {
        code = value
        resultHeaders = supplied
      },
      end(value) {
...
test('singleflight start returns consistent status and fixed safe process configuration', async (t) => {
  const f = fixture({ env: { LIGHTLAB_RUNTIME_PORT: '6000' } })
  ...
  assert.deepEqual(args, [
    'run',
    '--project',
    '/lightlab/runtime/Lightflow.Runtime.csproj',
    '--launch-profile',
    'Ollama',
  ])
...
test('stop route rejects unsafe requests and accepts only the explicit same-origin stop header', async (t) => {
  ...
  for (const [headers, code] of [
    [startHeaders, 403],
    [{ origin: 'http://evil.example', 'x-lightlab-action': 'stop' }, 403],
  ...
  const result = await request(f.manager, {
    url: '/__lightlab/runtime/stop',
    method: 'POST',
    headers: { ...startHeaders, 'x-lightlab-action': 'stop' },
  })
  assert.equal(result.code, 202)
  assert.equal(result.body.stopping, true)
  assert.equal(f.signals.length, 1)
  f.children[0].emit('exit', 0)
})
...
test('start time and build mode distinguish source build from prebuilt Release without freshness claims', async (t) => {
  const dev = fixture(),
    release = fixture({ args: ['/lightlab/runtime/bin/Release/net10.0/Lightflow.Runtime.dll'] })
  t.after(dev.manager.dispose)
  t.after(release.manager.dispose)
  const initial = await dev.manager.status()
  assert.equal(initial.startedAt, null)
  assert.equal(initial.launchMode, 'development')
  ...
  assert.equal(prebuilt.launchMode, 'prebuilt')
  assert.match(prebuilt.buildMessage, /bouwt geen broncode/)
})
...
test('HTTP start requires exact local Host, matching Origin, explicit header and empty body', async (t) => {
  ...
  for (const extra of [{ 'content-length': '1' }, { 'transfer-encoding': 'chunked' }]) {
    assert.equal(
      (
        await request(f.manager, {
          url: '/__lightlab/runtime/start',
          method: 'POST',
          headers: { ...startHeaders, ...extra },
        })
      ).code,
      400,
    )
  ...
  assert.equal(result.code, 202)
  assert.equal(result.body.state, 'starting')
...
test('HTTP reads reject foreign origins and API never accepts arguments, queries or other methods', async (t) => {
  ...
  assert.equal(
    (await request(f.manager, { url: '/__lightlab/runtime/start?args=evil', method: 'POST', headers: startHeaders }))
      .code,
    404,
  )
  ...
  assert.equal(result.body.state, 'stopped')
