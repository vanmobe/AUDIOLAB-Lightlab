...
export interface WavInfo { duration: number; channels: number; sampleRate: number }
export interface Kick { time: number; strength: number }
...
export interface KickAnalysisOptions { sensitivity?: number; minIntervalMs?: number }
...
export function inspectWav(buffer: ArrayBuffer): WavInfo {
  ...
  while (offset < buffer.byteLength) {
    ...
    if (chunk === 'fmt ') {
      ...
      if (channels < 1 || channels > 8 || sampleRate < 8000 || sampleRate > 96000
        || blockAlign !== channels * bits / 8 || byteRate !== sampleRate * blockAlign) throw invalid()
  ...
  if (duration > MAX_AUDIO_DURATION_SECONDS) throw new Error(`WAV duurt ${(duration / 60).toFixed(1)} minuten; maximaal ${MAX_AUDIO_DURATION_SECONDS / 60} minuten toegestaan.`)
...
export function analyzeKickAudio(samples: Float32Array, sampleRate: number, options: KickAnalysisOptions = {}): KickAnalysis {
  ...
  if (samples.length > sampleRate * MAX_AUDIO_DURATION_SECONDS) throw new Error(`Audio mag maximaal ${MAX_AUDIO_DURATION_SECONDS / 60} minuten duren.`)
  ...
  if (!Number.isFinite(sensitivity) || sensitivity < 0 || sensitivity > 1
    || !Number.isFinite(minIntervalMs) || minIntervalMs < 80 || minIntervalMs > 500) throw new Error('Ongeldige kickdetectie-instellingen.')
  ...
  let low = 0, high = 0, previousLow = 0
  ...
  const kicks: Kick[] = []
  let baseline = 0, previousEnergy = 0, maxStrength = 0
  ...
  for (let frame = 0; frame < frames; frame++) {
    ...
    const onset = current > floor && current > (baseline + floor) * ratioThreshold
      && current > previousEnergy * 1.2 && current / Math.max(fullEnergy[frame], 1e-12) > 0.22
  ...
  let bpm: number | null = null, confidence = 0
