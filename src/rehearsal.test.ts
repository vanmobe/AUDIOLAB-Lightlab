...
describe('independent rehearsal selection', () => {
  it('renders every program/profile pair, including items with no Look', () => {
    ...
    for (const program of show.programs) for (const profile of show.colorProfiles) {
      const chosen = chooseRehearsalItem(show, chooseRehearsalItem(show, state, 'program', program.id), 'profile', profile.id)
      ...
      const allLightGroups = show.groups.filter(group => show.fixtures.some(fixture => fixture.groupId === group.id && fixtureProfiles.find(p => p.id === fixture.profileId)?.kind !== 'hazer')).map(group => group.id)
      expect(evaluateFrame(preview.show, fixtureProfiles, preview.state, 0)).toEqual(evaluateFrame({ ...show, programs: show.programs.map(p => p.id === program.id ? { ...p, targetGroupIds: allLightGroups } : p), looks: [{ id: 'expected', name: '', programId: program.id, colorProfileId: profile.id }] }, fixtureProfiles, { mode: 'automation', activeLookId: 'expected' }, 0))
    }
  })
  ...
  it('preserves the other choice and resumes from blackout or a frozen frame', () => {
    ...
    const chosen = chooseRehearsalItem(initialShow, { ...state, mode: 'blackout', heldAtBeats: 3, colorLockId: profileId }, 'program', programId)
    expect(chosen).toMatchObject({ mode: 'automation', programId, colorLockId: profileId, heldAtBeats: undefined })
    expect(chooseRehearsalItem(initialShow, { ...chosen, mode: 'static' }, 'profile', initialShow.colorProfiles[0].id).programId).toBe(programId)
  })
  ...
  it('falls back to current show entries when prior selections no longer exist', () => {
    const preview = rehearsalPreview(initialShow, { ...state, activeLookId: 'removed', programId: 'removed', colorLockId: 'removed' })
  ...
  it('preserves complete Look layers and fixed palettes when only auditioning a color', () => {
    ...
    look.layers = [
      { groupId: 'front', mode: 'static', programId: null, colorProfileId: show.colorProfiles[0].id, intensity: .8 },
      { groupId: 'wash', mode: 'animation', programId: show.programs[1].id, colorProfileId: null, intensity: .9 },
    ]
    const followed = followRehearsalLook(state, look.id)
    const preview = rehearsalPreview(show, followed)
    expect(evaluateFrame(preview.show, fixtureProfiles, preview.state, 2)).toEqual(evaluateFrame(show, fixtureProfiles, followed, 2))
    ...
    expect(evaluateFrame(colorPreview.show, fixtureProfiles, colorPreview.state, 2)).toEqual(evaluateFrame(show, fixtureProfiles, chosen, 2))
  })
  it('can audition a color statically even when every original layer has a fixed palette', () => {
    const show = structuredClone(initialShow)
    show.looks[0].layers = show.groups.map(group => ({ groupId: group.id, mode: 'static', programId: null, colorProfileId: show.colorProfiles[0].id, intensity: .4 }))
    const preview = rehearsalPreview(show, { mode: 'automation', activeLookId: show.looks[0].id, colorLockId: show.colorProfiles[1].id, auditionStatic: true })
  ...
  it('retains edited group timing through persistence and renders it in rehearsal', () => {
    ...
    show.looks[0] = editLookLayer(show, show.looks[0], 'wash', {
      mode: 'animation', programId: 'pulse', rateBeats: 8, offsetBeats: 2,
    })
    ...
    expect(preview.show.looks[0].layers?.find(layer => layer.groupId === 'wash')).toMatchObject({ rateBeats: 8, offsetBeats: 2 })
    ...
    for (const beat of [0, 1, 3.5]) {
      ...
      expect(frame.fixtures.find(fixture => fixture.fixtureId === 'adj-1')?.intensity)
        .not.toBeCloseTo(evaluateFrame(unshifted, fixtureProfiles, followed, beat).fixtures.find(fixture => fixture.fixtureId === 'adj-1')!.intensity)
    }
  })
  it('restores saved timing when following a Look after standalone animation audition', () => {
    const show = structuredClone(initialShow)
    show.looks[0] = editLookLayer(show, show.looks[0], 'wash', {
      mode: 'animation', programId: 'pulse', rateBeats: 8, offsetBeats: -1.25,
    })
    ...
    expect(restored.show.looks[0].layers?.find(layer => layer.groupId === 'wash')).toMatchObject({ programId: 'pulse', rateBeats: 8, offsetBeats: -1.25 })
    expect(evaluateFrame(restored.show, fixtureProfiles, restored.state, 2))
      .toEqual(evaluateFrame(show, fixtureProfiles, followed, 2))
