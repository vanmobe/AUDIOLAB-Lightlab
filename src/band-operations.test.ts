...
const member: BandMember = { id: 'singer', name: 'Zang', position: [1, .5, 2] }
const lamp: FixtureDeployment = { id: 'front', name: 'Front', profileId: 'test', modeId: 'test', groupId: 'front', position: [0, 4, 3], aim: [0, 0, 0], aimMode: 'direction' }
describe('band placement and fixed aiming', () => {
  ...
  it('clamps member movement and leaves existing fixture target fixed', () => {
    ...
    expect(moved[0].position).toEqual([6.5, .5, -4.5])
    expect(member.position).toEqual([1, .5, 2])
    expect(result[0].aim).toEqual([1, 1.8, 2])
  })
})
