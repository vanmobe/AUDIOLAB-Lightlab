...
const audio: AudioLiveSnapshot = { seconds: 1, playing: true, analysis: { duration: 3, kicks: [{ time: 1, strength: 1 }], waveform: [.2], bpm: 120, confidence: 1 }, mode: 'kicks', bpm: 120, reactions: { wash: 'pulse' }, decayMs: 300, floor: .2 }
const status = (audioId: string | null, sequence = 0, state = 'following') => ({ version: 1, sessionId: 'session', audioId, state, sequence, error: null })
afterEach(() => vi.unstubAllGlobals())
it('sends only analysis once and small media state thereafter; never sends DMX or audio bytes', async () => {
  const fetch = vi.fn().mockImplementation(async (_url, init) => { const body = JSON.parse(init.body); return Response.json(status(body.audioId, body.sequence ?? 0, body.command === 'detach' ? 'detached' : 'following')) })
  vi.stubGlobal('fetch', fetch)
  const id = 'a'.repeat(32), signal = new AbortController().signal
...
it('rejects malformed status and stale session responses', () => {
  for (const value of [null, status(null), { ...status('a'.repeat(32)), sequence: -1 }, { ...status('a'.repeat(32)), sessionId: 'other' }, status('bad')]) {
...
it('does not connect automatically and disconnects when the loaded analysis changes', async () => {
  const send = vi.fn<typeof sendRuntimeAudio>().mockImplementation(async (_session, id, command, _signal, _input, sequence) => parseRuntimeAudioStatus(status(id, sequence ?? 0, command === 'detach' ? 'detached' : 'following'), 'session'))
