...
export function kickIndexAt(analysis: KickAnalysis | undefined, seconds: number): number {
  ...
  let low = 0, high = kicks.length
  while (low < high) { const middle = (low + high) >>> 1; if (kicks[middle].time <= seconds) low = middle + 1; else high = middle }
  return low - 1
}

/** Session-only adaptation; the saved Look, palette limits and off groups remain intact. */
export function audioPreviewFrame(show: ShowDocument, lookId: string, seconds: number, analysis: KickAnalysis | undefined,
  reactions: Record<string, AudioReaction>, bpm: number, decayMs: number, floor: number,
  ...
  const layers = resolveLookLayers(show, look).map(layer => {
    ...
    if (reaction === 'pulse' || reaction === 'static') return { ...layer, mode: 'static' as const,
    ...
    if (reaction === 'step') return { ...layer, offsetBeats: (layer.offsetBeats ?? 0) + beats - (index + 1) * (layer.rateBeats ?? 1) / 8 }
    return layer
  })
