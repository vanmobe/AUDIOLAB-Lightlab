...
const status = { version: 1, sessionId: 'session', state: 'disarmed', routes: [{ universe: 1, protocol: 'artnet', host: '127.0.0.1' }], framesSent: 0, lastError: null, armError: null }
...
it('bounds output status and rejects malformed or stale state instead of showing output off', () => {
  ...
  for (const change of [{ state: 'off' }, { framesSent: -.1 }, { framesSent: Infinity }, { lastError: {} }, { armError: 'x'.repeat(4097) }, { routes: [...status.routes, ...status.routes] }, { routes: [{ ...status.routes[0], universe: 32769 }] }, { state: 'armed', routes: [] }]) expect(() => parseOutputStatus({ ...status, ...change }, 'session')).toThrow()
...
it('never arms implicitly or without confirmation; explicit commands are session-fenced', async () => {
  const fetch = vi.fn().mockImplementation(async () => Response.json(status)); vi.stubGlobal('fetch', fetch)
  ...
  expect(JSON.parse(fetch.mock.calls[1][1].body)).toEqual({ version: 1, sessionId: 'session', command: 'arm', confirmed: true })
