...
const hooks = vi.hoisted(() => ({ values: [] as unknown[], cursor: 0, effects: [] as (() => unknown)[], inspect: vi.fn() }))
vi.mock('./dmx-inspection', async original => ({ ...await original<typeof import('./dmx-inspection')>(), inspectDmx: hooks.inspect }))
...
interface Props { children?: ReactNode; disabled?: boolean; 'aria-label'?: string; onClick?: () => unknown; onChange?: (event: { target: { value: string } }) => void }
...
const result = { version: 1, requestId: 'test', catalogVersion: 'inspection-test', dryRun: true, outputSent: false, issues: [], universes: [] }
beforeEach(() => { hooks.values = []; hooks.cursor = 0; hooks.effects = []; hooks.inspect.mockReset() })
...
it('mounts fixture channel rows only while its disclosure is open', () => {
  ...
  const render = () => { hooks.cursor = 0; return DmxFixtureDisclosure({ fixture, name: 'Test PAR' }) }
...
it('blocks dirty/empty/invalid snapshots without a runtime call', () => {
  const render = (dirty = false, show = initialShow) => { hooks.cursor = 0; return DmxInspector({ show, dirty }) }
  const calculate = (node: ReactNode) => find(node, props => !!props.onClick && text(props.children) === 'Bereken DMX-proef')
...
it.each(['show', 'beat', 'cancel'])('discards late replies after %s change and preserves a newer request', async change => {
  let resolve!: (value: unknown) => void
  hooks.inspect.mockImplementationOnce(() => new Promise(done => { resolve = done })).mockResolvedValueOnce(result)
  let show = initialShow
  const render = () => { hooks.cursor = 0; hooks.effects = []; return DmxInspector({ show }) }
  ...
  if (change === 'cancel') button('Annuleer DMX-proef').onClick!()
    ...
    else find(render(), props => props['aria-label'] === 'Moment voor DMX-proef').onChange!({ target: { value: '2' } })
...
it('shows actionable fixture names and collapses repeated verification warnings', async () => {
  hooks.inspect.mockResolvedValue({ ...result, issues: [
    ...initialShow.fixtures.map(fixture => ({ severity: 'warning', code: 'unverified-personality', message: 'repeated', fixtureId: fixture.id })),
    { severity: 'error', code: 'legacy-mode', message: 'Kies 2ch.', fixtureId: 'hazer-1' },
  ] })
  const render = () => { hooks.cursor = 0; return DmxInspector({ show: initialShow }) }
