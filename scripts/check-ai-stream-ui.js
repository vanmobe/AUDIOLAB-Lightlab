...
async page => {
  ...
  await panel.waitFor()
  await page.waitForFunction(() => /Model denkt na|Voorstel wordt geschreven/.test(document.querySelector('.ai-progress [role="status"]')?.textContent || ''), null, { timeout: 60000 })
  ...
  assert(before === await page.evaluate(() => localStorage.getItem('lightlab-active-package-v1')), 'Provisional output must not change show')
