;async (page) => {
  ...
  const nav = (name) =>
    page
      .getByRole('navigation', { name: 'Showworkflow', exact: true })
  ...
  for (const width of [1440, 390]) {
    ...
    await nav('1 · Setup')
    await check(`stage-${width}`)
    assert(
      (await page
        .locator('.map-fixture')
        .first()
        .evaluate((element) => getComputedStyle(element).borderRadius)) === '50%',
      'Stage fixture markers retain their circular shape',
    )
    await page.getByRole('button', { name: 'Patch & netwerk', exact: true }).click()
    await check(`patch-${width}`)
    await page.getByRole('button', { name: 'Bediening & audio', exact: true }).click()
    await check(`wing-${width}`)
    await nav('2 · Ontwerp & repetitie')
    await page.getByRole('button', { name: /^Looks ·/ }).click()
    await check(`studio-${width}`)
    await page.getByRole('button', { name: 'Maak met AI', exact: true }).click()
    await check(`ai-${width}`)
    await nav('3 · Live')
    await check(`live-${width}`)
    await page.getByRole('button', { name: 'Blackout', exact: true }).click()
    assert(
      (await page.getByRole('button', { name: 'Blackout', exact: true }).getAttribute('aria-pressed')) === 'true',
      'Blackout exposes active state',
    )
    await page.getByRole('button', { name: 'Show afspelen', exact: true }).click()
    await page.locator('.show-menu > summary').click()
    assert(
      await page.getByRole('button', { name: 'Show exporteren', exact: true }).isVisible(),
      'File actions available in Live',
    )
  ...
  await page.evaluate(async () => {
    ...
    show.looks = Array.from({ length: 32 }, (_, i) => ({
      ...show.looks[0],
      id: `look-${i}`,
      name: `Look ${i + 1} — lange naam voor de liveset`,
    }))
  ...
  assert(
    (await page.locator('.live-look-library .look').getAttribute('aria-pressed')) === 'true',
    'Filtered Look activates',
  )
  await page.setViewportSize({ width: 390, height: 844 })
  await check('long-names-390')
  assert(errors.length === 0, `Runtime errors: ${errors.join('; ')}`)
  await page.evaluate(() => {
    window.__professionalUiQa = 'passed'
  })
