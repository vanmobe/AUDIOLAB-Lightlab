...
;async (page) => {
  ...
  const geometry = () =>
    page.locator('#stage-map').evaluate((el) => {
  ...
  for (const width of [1584, 1024, 800, 390]) {
    ...
    for (const name of ['Bandlid toevoegen', 'Selecteren', 'Weergave', 'Groepen beheren']) {
      ...
      if ((await page.locator('.stage-inspector > .stage-tool-panel').count()) !== 1)
        throw new Error('Tool is not inside sidebar')
      const after = await geometry()
      if (Object.keys(before).some((key) => Math.abs(before[key] - after[key]) > 1))
        throw new Error(`${width}/${name}: stage moved ${JSON.stringify({ before, after })}`)
