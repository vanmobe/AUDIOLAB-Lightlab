...
interface Props { children?: ReactNode; 'aria-label'?: string; onChange?: (event: { target: { value: string } }) => void; onClick?: () => void }
...
it('filters only the desktop selection list and dispatches selection without editing the saved show', () => {
  hooks.values = []
  const onChange = vi.fn(), onSelectLook = vi.fn()
  const show = { ...initialShow, looks: Array.from({ length: 7 }, (_, i) => ({ ...initialShow.looks[0], id: `look-${i}`, name: `Look ${i + 1}` })) }
  const render = () => { hooks.cursor = 0; return LookStudio({ show, onChange, onSelectLook, selectedLookId: 'look-0', bpm: 120, onBpmChange: vi.fn() }) }
