...
const proposal = (): DesignProposal => ({
  provider: 'test',
  summary: 'ideas',
  colorProfiles: [1, 2].map((n) => ({
    ...initialShow.colorProfiles[0],
    id: `idea-${n}`,
    name: `Idea ${n}`,
    primary: n === 1 ? '#123456' : '#abcdef',
  })),
  programs: [],
  looks: [],
})
describe('selective idea acceptance', () => {
  it('adds only selected palettes, preserving setup, bindings and existing collection', () => {
    const show = structuredClone(initialShow),
      before = fingerprint(show)
    const next = selectedProposalCandidate(show, proposal(), options, before, {
      ...emptySelection(),
      colorProfiles: ['idea-2'],
    })
  ...
  it('rejects empty/stale selections and still validates unselected provider data', () => {
    const p = proposal(),
      base = fingerprint(initialShow)
    expect(() => selectedProposalCandidate(initialShow, p, options, base, emptySelection())).toThrow('Selecteer')
    expect(() =>
      selectedProposalCandidate({ ...initialShow, name: 'changed' }, p, options, base, {
        ...emptySelection(),
        colorProfiles: ['idea-1'],
      }),
    ).toThrow('gewijzigd')
    p.colorProfiles[1].primary = 'invalid'
    expect(() =>
      selectedProposalCandidate(initialShow, p, options, base, { ...emptySelection(), colorProfiles: ['idea-1'] }),
    ).toThrow('kleuren')
  })
  it('derives transitive dependencies and releases them when the root is deselected', () => {
    ...
    expect(resolveProposalSelection(p, { ...emptySelection(), looks: ['look'] })).toEqual({
      looks: ['look'],
      programs: ['pattern'],
      colorProfiles: ['idea-1', 'idea-2'],
    })
    expect(resolveProposalSelection(p, emptySelection())).toEqual(emptySelection())
    expect(resolveProposalSelection(p, { ...emptySelection(), programs: ['pattern'] }).colorProfiles).toEqual([
      'idea-2',
    ])
  })
  it('revises only selected existing palettes', () => {
    ...
    const next = selectedProposalCandidate(initialShow, p, { ...options, revision: true }, fingerprint(initialShow), {
      ...emptySelection(),
      colorProfiles: [p.colorProfiles[0].id],
    })
  ...
  it('does not consume a version for an unchanged revision subset', () => {
    const p = proposal()
    p.colorProfiles = initialShow.colorProfiles
      .slice(0, 2)
      .map((item, index) => ({ ...item, primary: index ? '#123456' : item.primary }))
    expect(() =>
      selectedProposalCandidate(initialShow, p, { ...options, revision: true }, fingerprint(initialShow), {
        ...emptySelection(),
        colorProfiles: [p.colorProfiles[0].id],
      }),
    ).toThrow('wijzigen niets')
  })
  it('can append only a palette from a replacement proposal without replacing bindings or active Look', () => {
    ...
    p.programs = [
      {
        ...
        rateBeats: 1,
        defaultColorProfileId: 'idea-1',
        pattern: {
          version: 1,
          floor: 0.2,
          steps: [
            { selection: 'all', direction: 'forward', envelope: 'hold', width: 1, trail: 0, level: 1, weight: 1 },
          ],
        },
      },
    ]
    p.looks = [
      {
        id: 'replacement-look',
        name: 'Idea Look',
        programId: 'replacement-pattern',
        colorProfileId: 'idea-1',
        layers: initialShow.groups.map((group) => ({
          groupId: group.id,
          mode: 'static',
          programId: null,
          colorProfileId: null,
    ...
    const next = selectedProposalCandidate(
      initialShow,
      p,
      { ...designDefaults, replace: true, profileCount: 2, programCount: 1, lookCount: 1 },
      fingerprint(initialShow),
      { ...emptySelection(), colorProfiles: ['idea-2'] },
    )
  ...
  it('uses selection as the replacement set for scoped palette replacement', () => {
    const p = proposal()
    const next = selectedProposalCandidate(initialShow, p, { ...options, replace: true }, fingerprint(initialShow), {
      ...emptySelection(),
      colorProfiles: ['idea-2'],
    })
