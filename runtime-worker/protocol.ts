...
const identifier = (value: unknown): value is string => typeof value === 'string' && value.trim().length > 0 && value.length <= 120
const beat = (value: unknown): value is number => typeof value === 'number' && Number.isFinite(value) && value >= 0 && value <= 1e9
  ...
  return (input: unknown) => {
    ...
    try {
      ...
      if (request.op === 'load') {
        ...
        show = next
        player.reset(); analysis = undefined; previewKey = undefined; previewShow = undefined
        return { version: 1, requestId, ok: true }
      }
      if (!show) throw new Error()
      if (request.op === 'audio') {
        keys(request, ['version', 'requestId', 'op', 'analysis'])
        if (request.analysis === null) { analysis = undefined; player.reset(); return { version: 1, requestId, ok: true } }
        const input = object(request.analysis)
        keys(input, ['duration', 'kicks', 'confidence'], ['bpm'])
        const finite = (v: unknown, min: number, max: number): v is number => typeof v === 'number' && Number.isFinite(v) && v >= min && v <= max
        if (!finite(input.duration, .001, 600) || !finite(input.confidence, 0, 1)
          || (input.bpm != null && !finite(input.bpm, 1, 1000)) || !Array.isArray(input.kicks) || input.kicks.length > 7500) throw new Error()
        let previous = -1
        const kicks = input.kicks.map(value => {
          const kick = object(value); keys(kick, ['time', 'strength'])
          if (!finite(kick.time, 0, input.duration as number) || kick.time <= previous || !finite(kick.strength, 0, 1)) throw new Error()
          previous = kick.time
          return { time: kick.time, strength: kick.strength }
        })
        analysis = { duration: input.duration, confidence: input.confidence, bpm: input.bpm as number | null ?? null, kicks, waveform: [] }
        player.reset()
        return { version: 1, requestId, ok: true }
      }
      ...
      if (!['automation', 'static', 'safety', 'blackout'].includes(state.mode as string)
        ...
        || (Object.hasOwn(state, 'colorLockId') && !show.colorProfiles.some(profile => profile.id === state.colorLockId))) throw new Error()
      ...
      if (Object.hasOwn(request, 'live')) {
        ...
        const key = JSON.stringify([state.activeLookId, request.live.controls.overrides, request.live.groupIntensities, request.live.colorLockId])
        if (key !== previewKey) {
          previewShow = playbackLivePreview(show, preview.state, request.live).show
          previewKey = key
        }
        preview = { show: previewShow!, state: { ...preview.state, colorLockId: request.live.colorLockId ?? undefined } }
      } else {
        previewKey = undefined; previewShow = undefined
      }
      ...
      if (Object.hasOwn(request, 'audio')) {
        ...
        if (!analysis || typeof input.playing !== 'boolean' || !beat(input.seconds) || input.seconds > analysis.duration
          || !['tempo', 'kicks'].includes(input.mode as string) || typeof input.bpm !== 'number' || !Number.isFinite(input.bpm) || input.bpm < 30 || input.bpm > 240
          || typeof input.decayMs !== 'number' || !Number.isFinite(input.decayMs) || input.decayMs < 100 || input.decayMs > 1000
          || typeof input.floor !== 'number' || !Number.isFinite(input.floor) || input.floor < 0 || input.floor > 1
          || Math.abs(request.atBeats - input.seconds * input.bpm / 60) > 1e-9) throw new Error()
        const reactions = object(input.reactions)
        if (Object.entries(reactions).some(([id, value]) => !show!.groups.some(group => group.id === id) || !['look', 'pulse', 'step', 'static'].includes(value as string))) throw new Error()
