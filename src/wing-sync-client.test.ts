...
import { parseWingApply, parseWingPlan, parseWingProbe, requestWing, wingAddress, wingPlanRequest } from './wing-sync-client'

function request() {
  let show = assignControlBinding(initialShow, initialShow.controlSurface.bindings[0], { bank: 2, kind: 'button', index: 3 })
  show = assignControlBinding(show, { id: 'master', label: 'Wash', action: 'group-intensity', targetId: 'wash' }, { bank: 2, kind: 'rotary', index: 1 })
...
function plan() { const { body } = request(); return { body, value: { version: 1, address: body.address, planId: 'abc', device, expiresAt: new Date(Date.now() + 60_000).toISOString(), changes: body.bindings.map(binding => ({ ...binding, before: { type: 'OFF' }, after: { type: 'MIDI', ch: 2 } })), warnings: [] } } }
...
describe('WING synchronization boundary', () => {
  ...
  it('rejects empty banks, duplicates, unsupported consoles and nonprivate hosts before networking', () => {
    const { show } = request()
    for (const banks of [[], [0], [17], [2, 2], [1.5]]) expect(() => wingPlanRequest(show, '10.0.0.10', banks)).toThrow()
    expect(() => wingPlanRequest({ ...show, controlSurface: { ...show.controlSurface, profileId: 'wing-compact' } }, '10.0.0.10', [2])).toThrow('Compact')
    for (const address of ['localhost', '127.0.0.1', '8.8.8.8', '10.0.0.256', '10.00.0.10', '10.0.0.10:2223']) expect(() => wingAddress(address)).toThrow()
  ...
  it('fences stale addresses, out-of-scope and duplicate slots and malformed verification', () => {
    ...
    expect(() => parseWingApply({ version: 1, state: 'applied', verifiedSlots: 1, totalSlots: 2, backup: [] }, 2)).toThrow()
    expect(parseWingApply({ version: 1, state: 'partial', verifiedSlots: 1, totalSlots: 2, backup: [] }, 2).state).toBe('partial')
  })
  ...
  it('does not retry uncertain writes and retains actionable bounded errors', async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: 'De bank is intussen gewijzigd.' }), { status: 409 }))
    vi.stubGlobal('fetch', fetcher)
    await expect(requestWing('apply', { version: 1, planId: 'abc', confirm: true }, new AbortController().signal)).rejects.toThrow('De bank is intussen gewijzigd.')
  ...
  it('rejects oversized replies before parsing', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { headers: { 'content-length': String(512 * 1024 + 1) } })))
