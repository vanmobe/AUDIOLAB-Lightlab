...
export const aimPresets = [
  ...
  { id: 'left', label: 'Links rechtdoor', detail: 'Naar links, gezien vanuit de zaal', icon: '←', direction: [-1, 0, 0] },
  { id: 'left-up', label: 'Links omhoog', detail: 'Schuin omhoog naar de linkerzijde', icon: '↖', direction: [-1, 1, 0] },
  { id: 'left-down', label: 'Links omlaag', detail: 'Schuin omlaag naar de linkerzijde', icon: '↙', direction: [-1, -1, 0] },
  { id: 'right', label: 'Rechts rechtdoor', detail: 'Naar rechts, gezien vanuit de zaal', icon: '→', direction: [1, 0, 0] },
  { id: 'right-up', label: 'Rechts omhoog', detail: 'Schuin omhoog naar de rechterzijde', icon: '↗', direction: [1, 1, 0] },
  { id: 'right-down', label: 'Rechts omlaag', detail: 'Schuin omlaag naar de rechterzijde', icon: '↘', direction: [1, -1, 0] },
] as const
...
export function aimFixtures(fixtures: FixtureDeployment[], ids: string[], preset: AimPreset) {
  ...
  return fixtures.map(f => ids.includes(f.id) ? { ...f, aimMode: 'direction' as const,
    aim: f.position.map((value, axis) => value + preset.direction[axis] / length * 6) as [number, number, number],
  } : f)
}
...
export function matchesAimPreset(fixture: FixtureDeployment, preset: AimPreset) {
  ...
  return length > 0 && direction.every((value, axis) => Math.abs(value / length - preset.direction[axis] / expectedLength) < .001)
}
...
export function moveFixtures(fixtures: FixtureDeployment[], ids: string[], dx: number, dz: number) {
  ...
  dx = Math.max(-6.7 - Math.min(...selected.map(f => f.position[0])), Math.min(6.7 - Math.max(...selected.map(f => f.position[0])), dx))
  dz = Math.max(-4.7 - Math.min(...selected.map(f => f.position[2])), Math.min(4.7 - Math.max(...selected.map(f => f.position[2])), dz))
  return fixtures.map(f => ids.includes(f.id) ? { ...f,
    position: [f.position[0] + dx, f.position[1], f.position[2] + dz] as [number, number, number],
    aim: f.aimMode === 'direction' ? [f.aim[0] + dx, f.aim[1], f.aim[2] + dz] as [number, number, number] : f.aim,
  } : f)
}
...
export function setFixtureHeight(fixtures: FixtureDeployment[], ids: string[], height: number) {
  return fixtures.map(f => ids.includes(f.id) ? { ...f,
    ...
    aim: f.aimMode === 'direction' ? [f.aim[0], f.aim[1] + height - f.position[1], f.aim[2]] as [number, number, number] : f.aim,
  } : f)
}
...
export function alignFixtures(fixtures: FixtureDeployment[], ids: string[], axis: 'x' | 'z') {
  ...
  return selected.reduce((result, f) => moveFixtures(result, [f.id], axis === 'x' ? centre - f.position[0] : 0, axis === 'z' ? centre - f.position[2] : 0), fixtures)
}
...
export function distributeFixtures(fixtures: FixtureDeployment[], ids: string[], axis: 'x' | 'z' = 'x') {
  const index = axis === 'x' ? 0 : 2
  const selected = fixtures.filter(f => ids.includes(f.id)).sort((a, b) => a.position[index] - b.position[index] || a.id.localeCompare(b.id))
