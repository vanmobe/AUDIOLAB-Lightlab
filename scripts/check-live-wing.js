;async (page) => {
  ...
  await page.evaluate(async () => {
    ...
    show.controlSurface.bindings = [
      ...
      },
      {
        ...
        label: 'Kleur',
        action: 'color-lock',
        targetId: show.colorProfiles[1].id,
  ...
  assert(
    (await page.getByLabel('Live knop 2: Donker').getAttribute('aria-pressed')) === 'true',
    'Color retains blackout',
  )
  ...
  assert(
    (await page.getByLabel('Front spots intensity', { exact: true }).inputValue()) === '0.64',
    'Linked master follows rotary',
  )
  await page.locator('.live-group-panel > summary').click()
  ...
  assert(
    (await page.evaluate(
      () => JSON.parse(localStorage.getItem('lightflow-show-v1')).groups.find((group) => group.id === 'wash').intensity,
    )) === 0.64,
    'Master persisted',
  )
