...
vi.mock('./playback-client', async (original) => ({
  ...(await original<typeof import('./playback-client')>()),
  getPlaybackStatus: hooks.status,
  startPlayback: hooks.start,
  commandPlayback: hooks.command,
}))
vi.mock('./runtime-live-client', async (original) => ({
  ...(await original<typeof import('./runtime-live-client')>()),
  getRuntimeShow: hooks.show,
  getRuntimePreview: hooks.preview,
  setRuntimeLive: hooks.live,
}))
...
const frame = (revision = 0) => ({
  version: 1,
  sessionId: 'session',
  status,
  frame: evaluateFrame(initialShow, fixtureProfiles, { mode: 'automation', activeLookId: initialShow.activeLookId }, 2),
  controls: { overrides: {}, links: [] },
  groupIntensities: masters,
  colorLockId: null,
  revision,
})
...
beforeEach(() => {
  ...
  hooks.status.mockResolvedValue(status)
  hooks.start.mockResolvedValue(status)
  hooks.command.mockResolvedValue(status)
  hooks.show.mockResolvedValue(initialShow)
  hooks.preview.mockResolvedValue(frame())
  hooks.live.mockResolvedValue(status)
})
afterEach(() => {
  cleanup?.()
  cleanup = undefined
...
it('attaches to server snapshot and consumes real sequential frames without overlapping reads', async () => {
  ...
  await tick()
  await render().attach()
  expect(render().show).toBe(initialShow)
  expect(render().preview).toEqual(frame())
  expect(render().connected).toBe(true)
  let resolve!: (value: unknown) => void
  hooks.preview.mockImplementationOnce(
    () =>
      new Promise((done) => {
        resolve = done
      }),
...
it('clears the last frame on disconnect and reconnects only on explicit attach', async () => {
  ...
  await vi.advanceTimersByTimeAsync(50)
  expect(render().preview).toBeUndefined()
  expect(render().connected).toBe(false)
  const calls = hooks.preview.mock.calls.length
  await vi.advanceTimersByTimeAsync(1000)
  expect(hooks.preview).toHaveBeenCalledTimes(calls)
  await render().attach()
  expect(render().connected).toBe(true)
})
it('stop supersedes a pending mutation and ignores its late reply', async () => {
  ...
  hooks.command
    ...
    .mockResolvedValueOnce({ ...status, status: 'stopped' })
...
it('fences live settings with the rendered revision, not a newer unpublished poll revision', async () => {
  ...
  await rendered.setLive({ overrides: { wash: { intensity: 0.5 } }, links: [] }, masters, null)
...
it('serializes live mutations without queuing slider changes and disconnects on conflict', async () => {
  ...
  hooks.live.mockImplementationOnce(
    () =>
  ...
  await render().setLive({ overrides: {}, links: [] }, { ...masters, wash: 0.2 }, null)
  expect(hooks.live).toHaveBeenCalledOnce()
  expect(hooks.live.mock.calls[0][1]).toBe(0)
  reject(new PlaybackConflict('Changed revision'))
  await action
  expect(render().preview).toBeUndefined()
...
it('validates replacement schema, size and BPM before reading status or stopping the current show', async () => {
  const render = setup()
  await tick()
  hooks.status.mockClear()
  const cases: [typeof initialShow, number][] = [
    [initialShow, 0],
    [{ ...initialShow, activeLookId: 'missing' }, 120],
    [{ ...initialShow, padding: 'x'.repeat(2 * 1024 * 1024) } as typeof initialShow, 120],
  ]
...
it('rejects deterministic patch errors and runtime head limits before stopping the old show', async () => {
  ...
  const unknown = structuredClone(initialShow)
  unknown.fixtures[0].modeId = 'unknown'
  const crowded = structuredClone(initialShow)
  crowded.fixtures = Array.from({ length: 5 }, (_, index) => ({
    ...crowded.fixtures[0],
    id: `fixture-${index}`,
    patch: undefined,
    visualSegments: 64,
...
it('replaces only the known session and publishes fetched new server snapshot and frame', async () => {
  ...
  hooks.start.mockResolvedValueOnce(nextStatus)
  hooks.show.mockResolvedValueOnce(loaded)
  hooks.preview.mockResolvedValueOnce(nextFrame)
  ...
  expect(render().show).toBe(loaded)
  expect(render().preview).toEqual(nextFrame)
  expect(render().connected).toBe(true)
  expect(render().error).toBe('')
})
...
it('keeps attached frame and connection during replacement preflight and after discovery failure', async () => {
  ...
  let reject!: (reason: unknown) => void
  hooks.status.mockImplementationOnce(
    () =>
  ...
  expect(render().preview).toBe(previous)
  expect(render().connected).toBe(true)
  expect(render().status?.status).toBe('running')
...
it('failed or unconfirmed old-session stop never starts replacement', async () => {
  ...
  await render().loadEditorShow(initialShow, 120)
  expect(hooks.start).not.toHaveBeenCalled()
  expect(render().error).toContain('Stop failed')
  hooks.command.mockResolvedValueOnce(status)
  await render().loadEditorShow(initialShow, 120)
  expect(hooks.start).not.toHaveBeenCalled()
  expect(render().error).toContain('niet bevestigd')
})
...
it('replacement is singleflight and later editor changes cannot alter the validated start payload', async () => {
  ...
  await tick()
  let resolve!: (value: unknown) => void
  hooks.command.mockImplementationOnce(
    () =>
      new Promise((done) => {
        resolve = done
      }),
...
it('Stop supersedes replacement during pending stop and prevents a late start', async () => {
  ...
  hooks.command
    ...
    .mockResolvedValueOnce({ ...status, status: 'stopped' })
  const replacement = render().loadEditorShow(initialShow, 100)
  await tick()
  await render().stop()
  resolve({ ...status, status: 'stopped' })
  await replacement
...
it('unmount cancels replacement before start without sending an extra stop', async () => {
  ...
  const pending = render().loadEditorShow(initialShow, 100)
  await tick()
  cleanup!()
  cleanup = undefined
  resolve({ ...status, status: 'stopped' })
...
it('loading into an idle runtime skips stop and Stop still cancels a pending idle preflight', async () => {
  ...
  const pending = render().loadEditorShow(initialShow, 120)
  await render().stop()
  resolve({ ...status, status: 'idle', sessionId: null, lookId: null })
  await pending
  expect(hooks.start).not.toHaveBeenCalled()
  expect(hooks.command).not.toHaveBeenCalled()
  await render().loadEditorShow(initialShow, 120)
