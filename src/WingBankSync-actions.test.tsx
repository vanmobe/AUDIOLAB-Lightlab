...
vi.mock('./wing-sync-client', async original => ({ ...await original<typeof import('./wing-sync-client')>(),
  requestWing: hooks.request, parseWingPlan: (value: unknown) => value, parseWingApply: (value: unknown) => value,
}))
...
interface Props { children?: ReactNode; disabled?: boolean; onClick?: () => void; onChange?: (event: { target: { checked: boolean } }) => void; onBusy?: (busy: boolean) => void; onUncertain?: (uncertain: boolean) => void }
function elements(node: ReactNode): { type: unknown; props: Props }[] { return Children.toArray(node).flatMap(child => isValidElement<Props>(child) ? [child, ...elements(child.props.children)] : []) }
function text(node: ReactNode): string { return Children.toArray(node).map(child => isValidElement<Props>(child) ? text(child.props.children) : String(child)).join('') }
const flush = async () => { for (let i = 0; i < 10; i++) await Promise.resolve() }
const plan = { version: 1, planId: 'plan', address: '10.0.0.10', expiresAt: '2099-01-01T00:00:00Z', device: { name: 'WING', model: 'Full', firmware: '3.1' }, warnings: [], changes: [{ bank: 1, kind: 'button', index: 1, label: 'Look', before: { name: 'Before' }, after: { name: 'After' } }] }
beforeEach(() => { hooks.values = []; hooks.cursor = 0; hooks.request.mockReset(); vi.stubGlobal('sessionStorage', { setItem: vi.fn(), removeItem: vi.fn(), getItem: vi.fn() }) })
...
function setup() {
  ...
  hooks.values = []
  const render = () => { hooks.cursor = 0; return (child.type as (props: Props) => ReactNode)({ ...child.props, onBusy: vi.fn(), onUncertain: vi.fn() }) }
  const button = (label: string) => elements(render()).find(element => element.props.onClick && text(element.props.children) === label)!.props
  return { render, button }
}
it('requires explicit confirmation but permits applying a valid plan without downloading a backup', async () => {
  hooks.request.mockResolvedValueOnce(plan).mockResolvedValueOnce({ state: 'applied', verifiedSlots: 1, totalSlots: 1, error: null })
  const view = setup()
  view.button('Lees banken uit & vergelijk').onClick!(); await flush()
  ...
  view.button('Verstuur bevestigde wijzigingen').onClick!(); await flush()
