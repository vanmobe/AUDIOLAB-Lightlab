...
export function matchesStageCamera(camera: SimulationCamera, preset: SimulationCamera) {
  return (
    ...
    camera.position.every((value, index) => value === preset.position[index]) &&
    camera.target.every((value, index) => value === preset.target[index])
  )
}
...
export function stageCameraMarker(camera: SimulationCamera) {
  ...
  const dx = camera.target[0] - x,
    dz = camera.target[2] - z
  const distance = Math.hypot(dx, dz)
  const direction = distance > 0.000001 ? { x: dx / distance, y: dz / distance } : undefined
  // Shorten outward-facing arrows at the edge rather than drawing outside the stage map.
  const length = direction
    ? Math.min(
        18,
        direction.x > 0
          ? (138 - position.x) / direction.x
          : direction.x < 0
            ? (2 - position.x) / direction.x
            : Infinity,
        direction.y > 0 ? (98 - position.y) / direction.y : direction.y < 0 ? (2 - position.y) / direction.y : Infinity,
      )
    : 0
  ...
  const label =
    stageCameraPresets.find((preset) => matchesStageCamera(camera, preset.camera))?.label ?? 'Eigen standpunt'
