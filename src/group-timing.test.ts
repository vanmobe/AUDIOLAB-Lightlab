...
function timingShow(effect: AnimationEffect = 'pulse') {
  ...
  show.looks[0].layers = ['wash', 'back'].map((groupId) => ({
    groupId,
    mode: 'animation',
...
describe('per-group animation timing', () => {
  it.each(['pulse', 'chase', 'wave', 'random'] as const)(
    '%s uses independent duration/offset while sharing spatial targets',
    (effect) => {
      const base = timingShow(effect),
        adjusted = structuredClone(base)
      Object.assign(adjusted.looks[0].layers![0], { rateBeats: 4, offsetBeats: 1 })
      Object.assign(adjusted.looks[0].layers![1], { rateBeats: 0.5, offsetBeats: -0.5 })
      for (const at of [0, 0.37, 1.7, 5.3]) {
        const frame = render(adjusted, at)
        const slow = render(base, ((at - 1) / 4) * 2)
        const fast = render(base, ((at + 0.5) / 0.5) * 2)
  ...
  it('positive offset delays an ongoing pattern without clamping negative phase to startup', () => {
    ...
    show.looks[0].layers![0].offsetBeats = 0.5
    expect(render(show, 0).fixtures[0].intensity).toBeLessThan(render(timingShow(), 0).fixtures[0].intensity)
    expect(render(show, 0).fixtures[0]).toEqual(render(timingShow(), -0.5).fixtures[0])
  })
  it('null duration and zero offset preserve inherited behavior exactly', () => {
    const base = timingShow('random'),
      explicit = structuredClone(base)
  ...
  it('freeze applies timing to the held beat; safety and blackout bypass it', () => {
    const base = timingShow('wave'),
      show = structuredClone(base)
    Object.assign(show.looks[0].layers![0], { rateBeats: 8, offsetBeats: 3 })
    const frozen = { ...state, mode: 'static' as const, heldAtBeats: 0.75 }
    expect(render(show, 10, frozen).fixtures).toEqual(render(show, 100, frozen).fixtures)
    expect(render(show, 10, frozen).fixtures).toEqual(render(show, 0.75).fixtures)
    for (const mode of ['safety', 'blackout'] as const)
      expect(render(show, 3, { ...state, mode })).toEqual(render(base, 3, { ...state, mode }))
  })
  it.each(['static', 'off'] as const)('%s layers retain but ignore timing', (mode) => {
    const base = timingShow(),
      changed = structuredClone(base)
    base.looks[0].layers![0] = { ...base.looks[0].layers![0], mode, programId: null }
    changed.looks[0].layers![0] = { ...base.looks[0].layers![0], rateBeats: 0.125, offsetBeats: 64 }
...
describe('live timing edits', () => {
  it('updates linked timing fields independently and resets only their overrides', () => {
    ...
    let controls = updateLiveGroups(show, emptyLiveControls(), 'wash', {
      colorProfileId: 'neon',
      intensity: 0.4,
      rateBeats: 8,
    })
    ...
    expect(controls.overrides.wash).toEqual({ colorProfileId: 'neon', intensity: 0.4, rateBeats: 8, offsetBeats: 2 })
  ...
  it('legacy null live reset uses neutral one-beat duration', () => {
    ...
    expect(render(preview.show, 0.3)).toEqual(render(expected, 0.3))
  })
  ...
  it.each([
    ...
    { offsetBeats: -65 },
    { offsetBeats: 65 },
  ])('ignores invalid live timing %j', (invalid) => {
    const show = timingShow()
...
describe('persisted timing validation', () => {
  ...
  it.each([{}, { rateBeats: null }, { rateBeats: 0.125, offsetBeats: -64 }, { rateBeats: 64, offsetBeats: 64 }])(
    'round trips supported timing %j',
    (timing) => {
      const show = timingShow()
  ...
  it.each([
    ...
    { offsetBeats: null },
    { offsetBeats: '2' },
  ])('rejects invalid saved timing %j', (timing) => {
    const show = timingShow()
    Object.assign(show.looks[0].layers![0], timing)
