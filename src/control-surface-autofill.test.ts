...
const options: AutoFillOptions = { banks: [1, 2, 3, 4], grouping: 'order', mode: 'replace', rotaryGroupIds: ['front', 'wash', 'back', null] }
const clean = () => ({ ...structuredClone(initialShow), controlSurface: { profileId: 'wing-rack', bindings: [] as typeof initialShow.controlSurface.bindings } })

describe('automatic logical bank planning', () => {
  it('fills all Looks once, with fixed group encoders in non-contiguous sorted banks', () => {
    const show = clean(), before = structuredClone(show)
    ...
    expect(result.banks.flatMap(bank => bank.looks.map(look => look.lookId))).toEqual(show.looks.map(look => look.id))
    expect(result.banks.every(bank => bank.rotaries.map(rotary => rotary.groupId).join() === 'front,wash,back')).toBe(true)
  ...
  it('reuses unassigned controls and repeated replacement is idempotent', () => {
    const show = clean()
    show.controlSurface.bindings.push({ id: 'retained-look', action: 'look', targetId: show.looks[0].id, label: 'Eigen naam' })
    const first = planControlSurfaceFill(show, options)
    expect(first.surface.bindings.find(binding => binding.id === 'retained-look')?.slot).toEqual({ bank: 1, kind: 'button', index: 1 })
  ...
  it('preserves outside assignments, unplaces displaced actions and leaves null rotaries alone', () => {
    ...
    show.controlSurface.bindings.push(
      { id: 'out', action: 'look', targetId: show.looks[0].id, label: 'Outside', slot: { bank: 16, kind: 'button', index: 2 } },
      { id: 'stop', action: 'mode', targetId: 'blackout', label: 'Stop', slot: { bank: 1, kind: 'button', index: 1 } },
      { id: 'keep', action: 'group-intensity', targetId: 'effects', label: 'Haze', slot: { bank: 1, kind: 'rotary', index: 4 } },
    )
  ...
  it('keeps occupied buttons and skips Looks already in selected banks in empty mode', () => {
    const show = clean()
    show.controlSurface.bindings.push({ id: 'placed', action: 'look', targetId: show.looks[0].id, label: 'Placed', slot: { bank: 1, kind: 'button', index: 5 } })
    ...
    expect(result.surface.bindings.filter(binding => binding.action === 'look' && binding.targetId === show.looks[0].id)).toHaveLength(1)
    expect(planControlSurfaceFill({ ...show, controlSurface: result.surface }, { ...options, mode: 'empty' }).surface).toEqual(result.surface)
  })
  it('rejects an occupied conflicting rotary atomically', () => {
    const show = clean()
    show.controlSurface.bindings.push({ id: 'rotary', action: 'group-intensity', targetId: 'effects', label: 'Haze', slot: { bank: 1, kind: 'rotary', index: 1 } })
    const result = planControlSurfaceFill(show, { ...options, mode: 'empty' })
  ...
  it('starts a new bank per character and reports grouping overflow without partial assignments', () => {
    ...
    const characterOverrides = Object.fromEntries(show.looks.map((look, index) => [look.id, (['calm', 'movement', 'energetic'] as const)[index]]))
    const result = planControlSurfaceFill(show, { ...options, grouping: 'character', banks: [1, 3, 8], characterOverrides })
    expect(result.errors).toEqual([])
    ...
    const failed = planControlSurfaceFill(show, { ...options, grouping: 'character', banks: [1, 3], characterOverrides })
  ...
  it('rejects invalid banks, missing groups, unsupported profiles and capacity overflow', () => {
    const show = clean()
    for (const banks of [[], [0], [17], [1.5], [NaN]]) expect(planControlSurfaceFill(show, { ...options, banks }).errors.length).toBeGreaterThan(0)
    expect(planControlSurfaceFill(show, { ...options, rotaryGroupIds: ['missing'] }).errors.length).toBeGreaterThan(0)
    expect(planControlSurfaceFill({ ...show, controlSurface: { ...show.controlSurface, profileId: 'unknown' } }, options).errors.length).toBeGreaterThan(0)
    show.controlSurface.bindings = Array.from({ length: 512 }, (_, index) => ({ id: `mode-${index}`, label: 'Stop', action: 'mode', targetId: 'blackout' }))
  ...
  it('fills 32 Looks exactly and never silently truncates an undersized selection', () => {
    const show = clean()
    show.looks = Array.from({ length: 32 }, (_, index) => ({ ...show.looks[0], id: `look-${index}`, name: `Look ${index}` }))
  ...
  it('does not mix a newly filled character with a preserved contrary Look', () => {
    ...
    show.controlSurface.bindings.push({ id: 'kept', action: 'look', label: 'Fast', targetId: show.looks[0].id, slot: { bank: 1, kind: 'button', index: 1 } })
    const result = planControlSurfaceFill(show, { ...options, mode: 'empty', grouping: 'character', banks: [1, 2], characterOverrides })
    expect(result.errors).toEqual([])
    expect(result.warnings.join()).toContain('niet opnieuw op karakter')
    expect(result.banks.map(bank => bank.looks.map(look => look.lookId))).toEqual([[show.looks[0].id], [show.looks[1].id]])
  })
  it('keeps custom names in empty mode and unused bank names in replacement mode', () => {
    const show = { ...clean(), controlSurface: { ...clean().controlSurface, bankNames: { '1': 'Mijn bank', '4': 'Reserve', '16': 'Buiten selectie' } } }
  ...
  it('preserves hidden stored slots beyond the current profile and does not count them as placed', () => {
    const show = clean()
    show.controlSurface.profileId = 'wing-compact'
    show.controlSurface.bindings.push({ id: 'hidden', action: 'look', targetId: show.looks[0].id, label: 'Other profile', slot: { bank: 1, kind: 'button', index: 40 } })
    for (const mode of ['empty', 'replace'] as const) {
      const result = planControlSurfaceFill(show, { ...options, mode, banks: [1], rotaryGroupIds: [] })
      expect(result.errors).toEqual([])
      expect(result.surface.bindings.find(binding => binding.id === 'hidden')).toEqual(show.controlSurface.bindings[0])
  ...
  it('does not describe a moved Look as displaced to the unassigned list', () => {
    const show = clean()
    show.controlSurface.bindings.push({ id: 'moved', action: 'look', targetId: show.looks[0].id, label: 'Move', slot: { bank: 1, kind: 'button', index: 8 } })
    const result = planControlSurfaceFill(show, options)
...
describe('Look character suggestions', () => {
  it('classifies the actual layers and group durations, never the Look or animation name', () => {
    const show = clean(), look = show.looks[0]
    const program = { ...show.programs[0], id: 'test', name: 'Very calm', effect: 'pulse' as const, rateBeats: 1, pattern: undefined }
    show.programs.push(program)
    look.layers = resolveLookLayers(show, look).map(layer => ({ ...layer, mode: 'animation', programId: program.id, rateBeats: 1 }))
    look.name = 'Quiet'
    ...
    look.layers = look.layers.map(layer => ({ ...layer, mode: layer.groupId === 'effects' ? 'animation' : 'off', rateBeats: .25 }))
    expect(classifyLookCharacter(show, look)).toBe('calm')
  })
  it('ignores muted group masters', () => {
    const show = clean(), look = show.looks[0]
    show.groups = show.groups.map(group => ({ ...group, intensity: 0 }))
    look.layers = resolveLookLayers(show, look).map(layer => ({ ...layer, mode: 'animation', programId: show.programs.find(program => program.effect === 'pulse')?.id ?? show.programs[1].id, rateBeats: .125 }))
    expect(classifyLookCharacter(show, look)).toBe('calm')
  })
  it('recognizes constant composed patterns despite animated fallback names', () => {
    const show = clean(), look = show.looks[0], program = show.programs[0]
    program.effect = 'sparkle'
    program.pattern = { version: 1, floor: .3, steps: [{ selection: 'all', direction: 'forward', envelope: 'hold', width: 1, trail: 0, level: 1, weight: 1 }] }
    look.layers = resolveLookLayers(show, look).map(layer => ({ ...layer, mode: 'animation', programId: program.id, rateBeats: .125 }))
    expect(classifyLookCharacter(show, look)).toBe('calm')
  })
})
