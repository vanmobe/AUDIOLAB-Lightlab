...
export function assertSimulationBudget(fixtures: FixtureDeployment[]) {
  if (fixtures.length > 1024)
    throw new SimulationBudgetError(
      'Simulatie gepauzeerd: maximaal 1024 fixtures. Je show blijft beschikbaar om te bewerken en exporteren.',
    )
  let heads = 0
  for (const fixture of fixtures) {
    ...
    if (!Number.isInteger(segments) || segments < 1 || segments > 64)
      throw new SimulationBudgetError('Simulatie gepauzeerd: ongeldig aantal lichtpunten bij een fixture.')
    heads += segments
    if (heads > 256)
      throw new SimulationBudgetError(
        'Simulatie gepauzeerd: maximaal 256 lichtpunten. Je show blijft beschikbaar om te bewerken en exporteren.',
      )
  }
}
...
export function beamThrow(origin: THREE.Vector3, direction: THREE.Vector3) {
  ...
  for (const [axis, value] of [
    ...
    if (t <= 0.05 || t >= distance) continue
    const hit = origin.clone().addScaledVector(direction, t)
    const hitsSurface =
      axis === 'y'
        ? Math.abs(hit.x) <= 7 && hit.z >= -5 && hit.z <= 5
        : axis === 'z'
          ? Math.abs(hit.x) <= 7.3 && hit.y >= 0 && hit.y <= 5
...
export function createFixtureView(fixture: FixtureDeployment, segment: number, smokeTime = { value: 0 }) {
  ...
  const lamp = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: '#000000', side: THREE.DoubleSide, fog: false }),
  )
  lamp.position.set(segmentX, y, z)
  const direction = new THREE.Vector3(...fixture.aim).sub(
    fixture.aimMode === 'direction' ? new THREE.Vector3(...fixture.position) : lamp.position,
  )
  ...
  const beam = new THREE.Mesh(
    new THREE.ConeGeometry(length * Math.tan(beamAngle), length, 48, 1, true),
    beamMaterial(lamp.position, direction, length, beamAngle, smokeTime),
  )
  lamp.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), direction)
  const housing = new THREE.Group()
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(0.17, 0.14, 0.32, 16),
    new THREE.MeshStandardMaterial({ color: '#232830', roughness: 0.55, metalness: 0.65 }),
  )
  body.position.copy(lamp.position).addScaledVector(direction, -0.17)
...
export function updateFixtureView(
  view: ReturnType<typeof createFixtureView>,
...
export function createBandMemberView(member: BandMember) {
  ...
  const clothing = new THREE.MeshStandardMaterial({ color: '#93979c', roughness: 0.85, metalness: 0 })
  const skin = new THREE.MeshStandardMaterial({ color: '#c8bcae', roughness: 0.8, metalness: 0 })
  ...
  add(new THREE.SphereGeometry(0.15, 16, 12), skin, 0, 1.62, 0)
  add(new THREE.CapsuleGeometry(0.2, 0.3, 6, 16), clothing, 0, 1.15, 0)
  for (const side of [-1, 1]) {
    add(new THREE.CylinderGeometry(0.08, 0.065, 0.77, 10), clothing, side * 0.11, 0.445, 0)
    add(new THREE.BoxGeometry(0.17, 0.09, 0.29), clothing, side * 0.11, 0.045, 0.055)
    const arm = add(new THREE.CapsuleGeometry(0.06, 0.48, 4, 12), clothing, side * 0.285, 1.09, 0)
...
export class StageSimulator {
  ...
  constructor(
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.22, 0.45, 1.15)
    ...
    const ordered = [...fixtures].sort(
      (a, b) =>
        Number(fixtureProfiles.find((p) => p.id === b.profileId)?.kind === 'theatre-spot') -
        Number(fixtureProfiles.find((p) => p.id === a.profileId)?.kind === 'theatre-spot'),
    )
  ...
  private addFixture(fixture: FixtureDeployment) {
    if (fixtureProfiles.find((profile) => profile.id === fixture.profileId)?.kind === 'hazer') {
      const housing = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: '#333b48' }),
      )
  ...
  setCamera(preset: SimulationCamera) {
    ...
    const vertical = Math.hypot(preset.target[0] - preset.position[0], preset.target[2] - preset.position[2]) < 0.000001
  ...
  private render = (now = performance.now()) => {
    ...
    const elapsed =
      this.previousRenderTime === undefined ? 0 : Math.min(0.1, Math.max(0, (now - this.previousRenderTime) / 1000))
