...
const adj = Array.from({ length: 12 }, (_, index) => ({
  id: `adj-${index + 1}`,
  name: `ADJ TriPar ${index + 1}`,
  profileId: 'adj-mega-tripar-profile-plus',
  modeId: '4ch',
  groupId: index < 4 ? 'wash' : 'back',
  patch: { universe: 1, address: 1 + index * 4 },
  position: [-5 + (index % 6) * 2, 1.2, index < 6 ? -2.5 : 2.5] as [number, number, number],
  aim: [0, 1.2, 0] as [number, number, number],
}))
const bars = Array.from({ length: 2 }, (_, index) => ({
  id: `tri-bar-${index + 1}`,
  name: `Stage TRI bar ${index + 1}`,
  profileId: 'stairville-stage-tri',
  modeId: '14ch',
  groupId: 'back',
  visualSegments: 4,
  patch: { universe: 1, address: 49 + index * 14 },
  position: [-3.8 + index * 7.6, 3.2, -3] as [number, number, number],
  aim: [0, 1.2, 0] as [number, number, number],
}))
const fronts = Array.from({ length: 4 }, (_, index) => ({
  id: `front-${index + 1}`,
  name: `Varytec Front ${index + 1}`,
  profileId: 'varytec-theater-spot-100',
  modeId: '2ch',
  groupId: 'front',
  patch: { universe: 1, address: 161 + index * 2 },
  position: [-4.5 + index * 3, 4.5, 3] as [number, number, number],
  aim: [-1.5 + index, 1.2, 0] as [number, number, number],
}))
...
export const initialShow: ShowDocument = {
  ...
  fixtures: [
    ...
    ...fronts,
    {
      ...
      modeId: '2ch',
      groupId: 'effects',
      patch: { universe: 1, address: 77 },
      position: [0, 0, -3],
      aim: [0, 1, 0],
    },
  ],
  groups: [
    { id: 'front', name: 'Front spots', intensity: 0.8 },
    { id: 'wash', name: 'Wash', intensity: 0.75 },
    { id: 'back', name: 'Back & bars', intensity: 0.8 },
    { id: 'effects', name: 'Effects / haze', intensity: 0 },
  ],
  ...
  programs: [
    {
      ...
      name: 'Ambient',
      effect: 'static',
      targetGroupIds: ['front', 'wash', 'back'],
      rateBeats: 1,
    ...
    },
    {
      ...
      targetGroupIds: ['front', 'wash', 'back'],
      rateBeats: 0.5,
      defaultColorProfileId: 'neon',
    },
    {
      id: 'pulse',
      name: 'Pulse',
      effect: 'pulse',
      targetGroupIds: ['wash', 'back'],
      rateBeats: 1,
      defaultColorProfileId: 'neon',
    },
  ],
