...
describe('advisory design quality checks', () => {
  it('flags movement names in palettes without banning valid mood names', () => {
    const p = proposal()
    p.colorProfiles = [
      { ...initialShow.colorProfiles[0], name: 'Purple Chase' },
      { ...initialShow.colorProfiles[1], name: 'Warm amber' },
    ]
  ...
  it('finds identical patterns even if group ordering and names differ', () => {
    ...
    p.programs = [
      initialShow.programs[0],
      {
        ...
        id: 'copy',
        name: 'Anders',
        targetGroupIds: [...initialShow.programs[0].targetGroupIds].reverse(),
      },
    ]
  ...
  it('warns on low diversity in a collection, not a single requested pattern', () => {
    ...
    p.programs = Array.from({ length: 4 }, (_, i) => ({
      ...initialShow.programs[0],
      effect: 'pulse',
      id: String(i),
      rateBeats: i + 1,
    ...
    expect(designQualityWarnings(p, initialShow)).toEqual([
      expect.stringContaining('herhaalt'),
      expect.stringContaining('herhaalt'),
      expect.stringContaining('herhaalt'),
      expect.stringContaining('Weinig patroonvariatie'),
    ])
  })
  ...
  it('does not count ignored recipe directions as creative diversity', () => {
    ...
    p.programs = (['forward', 'reverse', 'bounce', 'inward'] as const).map((direction, i) => ({
      ...
      pattern: {
        version: 1,
        floor: 0.2,
        steps: [{ selection: 'random', direction, envelope: 'fade-out', width: 1, trail: 0, level: 1, weight: 1 }],
      },
    }))
  ...
  it('does not collapse different layered Looks sharing the same legacy pair', () => {
    ...
    p.looks = [
      { ...initialShow.looks[0], layers: [layer] },
      { ...initialShow.looks[0], id: 'other', layers: [{ ...layer, intensity: 0.5 }] },
    ]
    expect(designQualityWarnings(p, initialShow)).toEqual([])
  })
    ...
    p.looks = [
      {
        ...
        layers: initialShow.groups.map((group) => ({
          groupId: group.id,
          mode: group.id === 'effects' ? ('static' as const) : ('animation' as const),
          programId: group.id === 'effects' ? null : initialShow.programs[1].id,
          colorProfileId: null,
          intensity: group.id === 'front' ? 0.2 : group.id === 'effects' ? 0.7 : 1,
          rateBeats: 1,
          offsetBeats: 0,
        })),
      },
