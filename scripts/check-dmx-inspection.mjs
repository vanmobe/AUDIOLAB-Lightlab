...
const frame = (fixtureId, extra = {}) => ({ fixtureId, intensity: 0.5, color: '#804020', haze: 0, ...extra })
const request = {
  version: 1,
  requestId: 'http-golden-six-personalities',
  patch: {
    fixtures: [
    ...
    fixtures: [
      ...
      frame('tri14', {
        segments: [
          ...
          { intensity: 0.5, color: '#00ff00' },
          { intensity: 0.25, color: '#0000ff' },
          { intensity: 0, color: '#ffffff' },
        ],
      }),
      frame('white', { color: '#0000ff' }),
      frame('haze', { haze: 0.25 }),
    ],
  },
}
...
const reject = async (changed, code) => {
  ...
  assert.ok(
    result.issues.some((issue) => issue.severity === 'error' && issue.code === code),
...
try {
  ...
  assert.deepEqual(
    initial.universes.map((universe) => universe.routeEnabled),
    [true, false],
  )
  ...
  assert.deepEqual(
    (await inspect(singleHeads)).universes,
    initial.universes,
    'Engine single-head recipe frames must encode normally',
  )
  ...
  console.log(
    'DMX HTTP checks passed: six golden maps, two universes, disabled empty route, atomic failures, blackout, repeatability, strict JSON, 415/413 and chunked limit; no output sent.',
  )
} finally {
