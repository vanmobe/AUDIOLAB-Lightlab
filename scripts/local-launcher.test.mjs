...
test('local built app serves assets but rejects methods, foreign hosts and escaped/symlink paths', async () => {
  ...
  const dist = join(temp, 'dist'); await mkdir(dist); await mkdir(join(dist, 'assets'))
  await writeFile(join(dist, 'index.html'), '<h1>Lightlab</h1>'); await writeFile(join(dist, 'assets/app.js'), 'export {}')
  ...
  try {
    ...
    const index = await get('/'); assert.equal(index.status, 200); assert.equal(index.headers.get('cache-control'), 'no-store')
    ...
    const foreign = await new Promise(resolve => { const req = request(url, { headers: { Host: 'foreign.test' } }, response => { response.resume(); resolve(response.statusCode) }); req.end() })
    assert.equal(foreign, 403)
    assert.equal((await get('/%2e%2e%2foutside.txt')).status, 404)
    assert.equal((await get('/.env')).status, 404)
    if (process.platform !== 'win32') { await symlink(join(temp, 'outside.txt'), join(dist, 'escape.txt')); assert.equal((await get('/escape.txt')).status, 404) }
