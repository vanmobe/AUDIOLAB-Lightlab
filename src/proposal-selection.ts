...
export function resolveProposalSelection(proposal: DesignProposal, selection: ProposalSelection): ProposalSelection {
  ...
  for (const key of collectionKeys) result[key] = proposal[key].filter(item => selection[key].includes(item.id)).map(item => item.id)
  ...
  for (const look of proposal.looks.filter(item => result.looks.includes(item.id))) {
    add('programs', look.programId); add('colorProfiles', look.colorProfileId)
    for (const layer of look.layers ?? []) { add('programs', layer.programId); add('colorProfiles', layer.colorProfileId) }
  }
  for (const program of proposal.programs.filter(item => result.programs.includes(item.id))) add('colorProfiles', program.defaultColorProfileId)
  return result
}

export function selectedProposalCandidate(show: ShowDocument, proposal: DesignProposal, options: DesignOptions, base: string, selection: ProposalSelection): ShowDocument {
  ...
  if (options.replace && options.scope !== 'all') {
    ...
    const filtered: DesignProposal = { ...proposal,
  ...
  if (options.revision && !collectionKeys.some(key => validated[key].some(item => selected[key].includes(item.id) && JSON.stringify(item) !== JSON.stringify(show[key].find(old => old.id === item.id))))) throw new Error('De geselecteerde ideeën wijzigen niets. Kies een gewijzigd item.')
  ...
  for (const key of collectionKeys) {
    ...
    ;(next[key] as Array<ShowDocument[typeof key][number]>) = [...next[key].map(item => items.find(updated => updated.id === item.id) ?? item), ...items.filter(item => !next[key].some(old => old.id === item.id))]
    if (next[key].length > 32) throw new Error(`Maximum 32 ${key}. Selecteer minder ideeën of maak eerst plaats in de collectie.`)
  }
  ...
  for (const program of next.programs.filter(item => selected.programs.includes(item.id))) {
    ...
    if (signatures.has(signature)) throw new Error('Een geselecteerd patroon bestaat al. Deselecteer het patroon en de Looks die het nodig hebben.')
