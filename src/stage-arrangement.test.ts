...
it('aligns only selected fixtures and preserves beam direction, height and patch', () => {
  ...
  after.slice(0, 3).forEach((f, i) => {
    ...
    expect(f.aim.map((v, axis) => v - f.position[axis])).toEqual(before[i].aim.map((v, axis) => v - before[i].position[axis]))
  })
...
it('distributes unevenly spaced fixtures while keeping both endpoints and world targets', () => {
  ...
  before.forEach((f, i) => { f.position[0] = [-4, -3, 4][i]; f.aimMode = 'target' })
  const after = distributeFixtures(before, before.map(f => f.id))
...
it('leaves undersized and empty selections unchanged', () => {
  ...
  expect(distributeFixtures(initialShow.fixtures, initialShow.fixtures.slice(0, 2).map(f => f.id))).toBe(initialShow.fixtures)
})
...
it('aligns the top-view Y position without changing X, height or world targets', () => {
  ...
  before.forEach((f, i) => { f.position[2] = [-4, -2, 3, 4][i]; f.aimMode = 'target' })
  const after = alignFixtures(before, before.slice(0, 3).map(f => f.id), 'z')
  after.slice(0, 3).forEach((f, i) => {
...
it('distributes front to back without moving sideways or changing height and beam direction', () => {
  const before = structuredClone(initialShow.fixtures.slice(0, 4))
  before.forEach((f, i) => { f.position[2] = [-4, -3, 4, 2][i] })
  const ids = before.slice(0, 3).map(f => f.id)
  const aimed = aimFixtures(before, ids, aimPresets.find(p => p.id === 'left-up')!)
...
it.each(['x', 'z'] as const)('keeps zero-span and unknown selections unchanged on %s', axis => {
  const before = structuredClone(initialShow.fixtures.slice(0, 3))
  before.forEach(f => { f.position[axis === 'x' ? 0 : 2] = 0 })
  expect(distributeFixtures(before, before.map(f => f.id), axis)).toEqual(before)
