...
export function validSimulationHaze(value: number) {
  return Number.isFinite(value) ? Math.min(100, Math.max(0, value)) : defaultSimulationHaze
}
export function validSimulationBrightness(value: number) {
  return Number.isFinite(value) ? Math.min(200, Math.max(10, value)) : 100
}
...
export function simulationFixtureOutput(fixture: EvaluatedFixture, hidden: boolean): EvaluatedFixture {
  return hidden
    ? {
        ...
        intensity: 0,
        haze: 0,
        segments: fixture.segments?.map((segment) => ({ ...segment, intensity: 0 })),
      }
    : fixture
}
