async (page) => {
  ...
  const bounds = () => page.locator('.stage-map').evaluate(node => { const r = node.getBoundingClientRect(); return { top: r.top + scrollY, height: r.height } })
  const before = await bounds()
  await marker.focus(); await marker.press('Enter')
  assert(await page.getByRole('button', { name: 'Zaal', exact: true }).getAttribute('aria-pressed') === 'true', 'Camera opens view controls')
  assert(await page.locator('.map-fixture[aria-pressed=true]').count() === selected, 'Camera retains selection')
  assert(saved === await page.evaluate(() => localStorage.getItem('lightflow-show-v1')), 'Opening camera does not mutate show')
  ...
  for (const label of ['Dichtbij', 'Overzicht', 'Zaal']) {
    ...
    assert(await page.getByRole('button', { name: label, exact: true }).getAttribute('aria-pressed') === 'true', 'Preset selected')
  }
  ...
  await marker.click()
  assert(beforeAim === await page.evaluate(() => localStorage.getItem('lightflow-show-v1')), 'Camera click does not aim fixtures at the marker')
  assert(await page.getByRole('button', { name: 'Annuleren', exact: true }).count() === 0, 'Camera cancels targeting tool')
