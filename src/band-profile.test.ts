...
describe('band profile contract', () => {
  it('starts empty and automatic without inventing the band identity or colors', () => {
    expect(defaultBandProfile).toEqual({
      name: '',
      genres: '',
      character: '',
      colorMood: 'auto',
  ...
  it('accepts exact text/color boundaries and all explicit enum values', () => {
    const boundary = {
      ...
      name: 'n'.repeat(120),
      genres: 'g'.repeat(240),
      character: 'c'.repeat(1200),
      preferredColors: ['#AaBbCc', '#000000', '#FFFFFF', '#2f7cff'],
    }
    expect(() => validateBandProfile(boundary)).not.toThrow()
    for (const [key, values] of Object.entries({
      colorMood: ['warm', 'cool', 'bold', 'restrained'],
      energy: ['calm', 'balanced', 'high'],
      complexity: ['simple', 'layered', 'rich'],
  ...
  it.each([
    ...
    [],
    {},
    { ...defaultBandProfile, script: 'run()' },
    { ...defaultBandProfile, name: 1 },
    { ...defaultBandProfile, name: 'n'.repeat(121) },
    { ...defaultBandProfile, genres: 'g'.repeat(241) },
    { ...defaultBandProfile, character: 'c'.repeat(1201) },
    { ...defaultBandProfile, energy: 'extreme' },
    { ...defaultBandProfile, complexity: 3 },
    { ...defaultBandProfile, colorMood: 'blue' },
    { ...defaultBandProfile, motion: 'stop' },
    { ...defaultBandProfile, preferredColors: null },
    { ...defaultBandProfile, preferredColors: ['blue'] },
    { ...defaultBandProfile, preferredColors: ['#fff'] },
    { ...defaultBandProfile, preferredColors: ['#12345678'] },
    { ...defaultBandProfile, preferredColors: [123456] },
    { ...defaultBandProfile, preferredColors: Array(5).fill('#ffffff') },
  ])('rejects malformed or unsupported profile %j', (value) => {
  ...
  it.each([
    ...
    ['layered', 4],
    ['rich', 8],
  ] as const)('bounds %s patterns at %i relative steps', (complexity, maxSteps) => {
  ...
  it.each([
    ...
    ['medium', 2, 8],
    ['fast', 0.25, 2],
  ] as const)('bounds %s group durations without changing pattern tempo', (motion, minRateBeats, maxRateBeats) => {
