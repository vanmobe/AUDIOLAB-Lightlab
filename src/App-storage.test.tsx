...
it('fences corrupt canonical storage and does not silently load or overwrite stale legacy data', () => {
  const setItem = vi.fn(), getItem = vi.fn((key: string) => key === activePackageKey ? '{broken' : key === 'lightflow-show-v1' ? JSON.stringify({ ...initialShow, name: 'Stale legacy show' }) : null)
  vi.stubGlobal('localStorage', { getItem, setItem })
  const html = renderToStaticMarkup(<App />)
  expect(html).toContain('automatisch bewaren is geblokkeerd')
  expect(html).not.toContain('Stale legacy show'); expect(setItem).not.toHaveBeenCalled()
...
it('reads canonical show and history together without touching legacy keys', () => {
  ...
  const setItem = vi.fn(), getItem = vi.fn((key: string) => key === activePackageKey ? JSON.stringify(bundle) : null)
  vi.stubGlobal('localStorage', { getItem, setItem })
  expect(renderToStaticMarkup(<App />)).toContain('Canonical show')
  expect(getItem).toHaveBeenCalledWith(activePackageKey); expect(getItem).not.toHaveBeenCalledWith('lightflow-show-v1')
  expect(setItem).not.toHaveBeenCalled()
})
