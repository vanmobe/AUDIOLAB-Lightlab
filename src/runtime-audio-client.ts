...
export interface RuntimeAudioStatus {
  version: 1; sessionId: string; audioId: string | null; state: 'detached' | 'following' | 'lost'; sequence: number; error: string | null
}
export function parseRuntimeAudioStatus(value: unknown, sessionId: string): RuntimeAudioStatus {
  ...
  if (item.version !== 1 || item.sessionId !== sessionId || !['detached', 'following', 'lost'].includes(item.state)
    || !(item.audioId === null || (typeof item.audioId === 'string' && /^[a-f0-9]{32}$/.test(item.audioId)))
    || !Number.isSafeInteger(item.sequence) || item.sequence < 0 || !(item.error === null || (typeof item.error === 'string' && item.error.length <= 4096))
    || (item.state === 'following' && !item.audioId)) throw new Error('Ongeldige of verouderde audiostatus. Verbind opnieuw.')
  return item
}
export function audioPosition(input: AudioLiveSnapshot) {
  return { seconds: input.seconds, playing: input.playing ?? false, mode: input.mode, bpm: input.bpm,
    reactions: input.reactions, decayMs: input.decayMs, floor: input.floor }
}
export async function sendRuntimeAudio(sessionId: string, audioId: string, command: 'attach' | 'sync' | 'detach',
  signal: AbortSignal, input?: AudioLiveSnapshot, sequence = 0): Promise<RuntimeAudioStatus> {
  if (!sessionId || !/^[a-f0-9]{32}$/.test(audioId) || (command !== 'detach' && !input)) throw new Error('Geen geldige audioverbinding of WAV-analyse.')
  const body = { version: 1, sessionId, audioId, command,
    ...(command !== 'detach' ? { sequence, position: audioPosition(input!) } : {}),
    ...(command === 'attach' ? { analysis: { duration: input!.analysis.duration, kicks: input!.analysis.kicks, bpm: input!.analysis.bpm, confidence: input!.analysis.confidence } } : {}),
  }
...
export class RuntimeAudioLink {
  ...
  constructor(private sessionId: string, private send: Send = sendRuntimeAudio) {}
  get connected() { return this.audioId !== undefined && this.analysis !== undefined }
  async connect(input: AudioLiveSnapshot): Promise<RuntimeAudioStatus> {
    ...
    const audioId = crypto.randomUUID().replaceAll('-', ''), epoch = ++this.epoch
    this.audioId = audioId; this.sequence = 0
    ...
    try {
      ...
      if (result.state !== 'following' || result.audioId !== audioId || result.sequence !== 0) throw new Error('De runtime volgt deze audiobron niet.')
      this.analysis = input.analysis
      return result
    } catch (error) { if (epoch === this.epoch) void this.disconnect(); throw error }
  ...
  async sync(input: AudioLiveSnapshot | undefined): Promise<RuntimeAudioStatus | undefined> {
    ...
    const audioId = this.audioId!, sequence = ++this.sequence, epoch = this.epoch
    const controller = new AbortController(); this.pending = controller
    ...
    try {
      ...
      if (result.state !== 'following' || result.audioId !== audioId || result.sequence !== sequence) throw new Error('De audioverbinding is gestopt. Koppel opnieuw en bevestig uitvoer opnieuw.')
      return result
    } catch (error) { if (epoch === this.epoch) void this.disconnect(); throw error }
    finally { clearTimeout(timer); if (this.pending === controller) this.pending = undefined }
  }
  async disconnect(): Promise<void> {
    const id = this.audioId
    this.epoch++; this.audioId = undefined; this.analysis = undefined
    ...
    if (id) {
      ...
      try { await this.send(this.sessionId, id, 'detach', controller.signal) } catch { /* No automatic reattach or re-arm. */ }
