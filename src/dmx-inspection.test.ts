  ...
  universes: [
    {
      ...
      protocol: 'artnet',
      routeEnabled: false,
      channels: [0, 1, 2, 3, ...Array(508).fill(0)],
      fixtures: [{ fixtureId: 'adj-1', address: 1, channels: [0, 1, 2, 3], channelLabels: ['R', 'G', 'B', 'Dimmer'] }],
    },
  ],
})
afterEach(() => {
...
describe('stateless DMX inspection contract', () => {
  it('sends only patch and actual evaluated output, not creative/context/presentation data', () => {
    ...
    expect(request.frame).toEqual(
      evaluateFrame(initialShow, fixtureProfiles, { mode: 'automation', activeLookId: initialShow.activeLookId }, 2),
    )
    ...
    expect(
      inspectionRequest(show, show.activeLookId, 'blackout', 0, 'test').frame.fixtures.every(
  ...
  it('accepts bounded data and rejects unsafe or mismatched response fields', () => {
    ...
    for (const change of [
      ...
      { dryRun: false },
      { requestId: 'stale' },
      { issues: [{ severity: 'error', code: 'patch', message: 'Fix patch' }] },
      { issuesTruncated: 'yes' },
    ])
      expect(() => parseInspection({ ...response(), ...change }, 'test')).toThrow()
    for (const value of [-1, 256, 0.5, NaN]) {
      const bad = response()
      bad.universes[0].channels[0] = value
      expect(() => parseInspection(bad, 'test')).toThrow()
    }
    const mismatched = response()
    mismatched.universes[0].fixtures[0].channelLabels = []
    expect(() => parseInspection(mismatched, 'test')).toThrow()
    const byteMismatch = response()
    byteMismatch.universes[0].fixtures[0].channels[1] = 99
    expect(() => parseInspection(byteMismatch, 'test')).toThrow()
    const overlap = response()
    overlap.universes[0].fixtures.push({ ...overlap.universes[0].fixtures[0], fixtureId: 'another' })
      ...
      parseInspection(
        {
          ...response(),
          issues: [
            { severity: 'warning', code: 'unverified-personality', message: 'Not physically checked', fixtureId: null },
          ],
        },
        'test',
      ).issues[0].fixtureId,
    ...
    expect(() =>
      parseInspection({ ...response(), universes: Array(257).fill(response().universes[0]) }, 'test'),
    ).toThrow()
  })
  ...
  it('rejects excessive declared and streamed bodies before JSON parsing', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(new Response('{}', { headers: { 'content-length': String(4 * 1024 * 1024) } })),
    )
