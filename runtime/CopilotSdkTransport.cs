...
#pragma warning disable GHCP001 // Pinned SDK 1.0.13: deny-only permission and disabled MCP capability.
    public static SessionConfig SafeSession(string model, string system, string directory) => new() {
        Model = model, ClientName = "Lightlab", WorkingDirectory = directory, ConfigDirectory = directory,
        SystemMessage = new() { Mode = SystemMessageMode.Replace, Content = system },
        AvailableTools = [], Tools = [], McpServers = new Dictionary<string, McpServerConfig>(),
        OnPermissionRequest = (_, _) => Task.FromResult(PermissionDecision.Reject("Lightlab heeft geen tooltoegang.")),
        EnableConfigDiscovery = false, EnableOnDemandInstructionDiscovery = false, EnableFileHooks = false,
        EnableHostGitOperations = false, EnableSessionStore = false, EnableSkills = false,
        EnableSessionTelemetry = false, EnableExperimentalMode = false, EnableMcpApps = false,
        EnableFileChangeTracking = false, SkipCustomInstructions = true, CustomAgentsLocalOnly = true,
        CoauthorEnabled = false, ManageScheduleEnabled = false, SkipEmbeddingRetrieval = true,
        EmbeddingCacheStorage = EmbeddingCacheStorageMode.InMemory, McpOAuthTokenStorage = McpOAuthTokenStorageMode.InMemory,
        Memory = new() { Enabled = false }, InfiniteSessions = new() { Enabled = false },
        SkillDirectories = [], PluginDirectories = [], InstructionDirectories = [], IncludedBuiltinSkills = [],
        CustomAgents = [], AdditionalDirectories = [], Streaming = true, IncludeSubAgentStreamingEvents = false
    };
#pragma warning restore GHCP001
    ...
    static CopilotClient CreateClient(string directory) => new(new CopilotClientOptions {
        ...
        Mode = CopilotClientMode.CopilotCli, Connection = RuntimeConnection.ForStdio("copilot", ["--disable-builtin-mcps", "--no-custom-instructions"]),
        Environment = SafeEnvironment(),
        WorkingDirectory = directory, UseLoggedInUser = true, LogLevel = CopilotLogLevel.None,
        EnableRemoteSessions = false, BuiltinPluginDirectories = []
    });
