...
const frame = (show: typeof initialShow, beat: number, runtime = state) =>
  evaluateFrame(show, fixtureProfiles, runtime, beat)

describe('pattern-independent group timing migration', () => {
  it.each(animationEffects)(
    'preserves legacy $id frames, IDs and metadata across all supported timing limits',
    (effect) => {
      for (const rateBeats of [0.01, 0.12, 0.5, 8, 65, 1024]) {
        ...
        for (const mode of ['automation', 'static', 'safety', 'blackout'] as const) {
          for (const beat of [0, 0.17, 3.21])
            expect(frame(migrated, beat, { ...state, mode, heldAtBeats: 0.31 })).toEqual(
              frame(original, beat, { ...state, mode, heldAtBeats: 0.31 }),
            )
        }
      }
    },
  ...
  it('captures layer-specific legacy duration once, without inheriting later program metadata edits', () => {
    ...
    const migrated = materializeGroupTiming(original)
    expect(migrated.looks[0].layers?.find((layer) => layer.groupId === 'wash')).toMatchObject({
      rateBeats: 0.5,
      offsetBeats: -2,
    })
    const changed = structuredClone(migrated)
    changed.programs.forEach((program) => {
      program.rateBeats = 1
    })
    for (const beat of [0, 0.3, 1.7]) expect(frame(changed, beat)).toEqual(frame(migrated, beat))
    changed.programs[1].effect = 'wave'
    expect(resolveLookLayers(changed, changed.looks[0]).find((layer) => layer.groupId === 'wash')).toMatchObject({
      programId: 'chorus',
      rateBeats: 0.5,
      offsetBeats: -2,
    })
  ...
  it('preserves current group duration when switching patterns in live controls', () => {
    ...
    expect(
      resolveLookLayers(preview.show, preview.show.looks[1]).find((layer) => layer.groupId === 'wash'),
    ).toMatchObject({ programId: 'pulse', rateBeats: 0.5 })
  })
