;async (page) => {
  ...
  assert(
    await controls
      .locator('summary')
      .innerText()
      .then((text) => text.includes('50%') && text.includes('1 groep verborgen')),
    'Collapsed summary reflects settings',
  )
  assert(
    saved === (await page.evaluate(() => localStorage.getItem('lightflow-show-v1'))),
    'Brightness and group visibility leave show unchanged',
  )
  ...
  assert(
    saved === (await page.evaluate(() => localStorage.getItem('lightflow-show-v1'))),
    'Returning to Zaal preserves all show values',
  )
  ...
  assert(
    await page.locator('.look-studio-preview').evaluate((node) => getComputedStyle(node).position === 'static'),
    'Expanded controls can scroll at limited desktop height',
  )
  await page.getByRole('spinbutton', { name: 'Repetitietempo in BPM', exact: true }).scrollIntoViewIfNeeded()
  assert(
    !(await controls.getByRole('checkbox', { name: 'Wash', exact: true }).isChecked()),
    'Group visibility shared with Look studio',
  )
  ...
  assert(
    (await controls.locator('summary').innerText()).includes('alle groepen zichtbaar'),
    'Group hiding is temporary',
  )
  ...
  return {
    status: 'UI, persistence, group recovery, camera presets, layout, mobile and real GPU brightness passed',
    pixels,
  }
}
