...
const audio: AudioLiveSnapshot = {
  seconds: 1, analysis: { duration: 3, kicks: [{ time: 1, strength: 1 }], waveform: [], bpm: 120, confidence: 1 },
  mode: 'kicks', bpm: 120, reactions: Object.fromEntries(show.groups.map(group => [group.id, 'pulse'])), decayMs: 300, floor: .2,
}
describe('Live audio bridge', () => {
  ...
  it.each(['blackout', 'safety'] as const)('%s wins over kick reactions', mode => {
    expect(createAudioLivePlayer().evaluate(show, { ...state, mode }, 700, audio).frame)
      .toEqual(evaluateFrame(show, fixtureProfiles, { ...state, mode }, 2))
  })
  it.each(['tempo', 'kicks'] as const)('holds %s despite advancing audio and BPM edits', mode => {
    const player = createAudioLivePlayer(), input = { ...audio, mode }
    player.evaluate(show, state, 700, input)
    const held = player.evaluate(show, { ...state, mode: 'static', heldAtBeats: 700 }, 701, { ...input, seconds: 1.1 }).frame
    expect(player.evaluate(show, { ...state, mode: 'static' }, 900, { ...input, seconds: 2.5, bpm: 60 }).frame.fixtures).toEqual(held.fixtures)
  ...
  it('preserves color lock, zero masters and live off overrides', () => {
    ...
    expect(frame.fixtures.filter(item => show.fixtures.find(fixture => fixture.id === item.fixtureId)?.groupId === id).every(item => item.intensity === 0)).toBe(true)
    ...
    expect(createAudioLivePlayer().evaluate(muted, state, 9, audio).frame.fixtures.every(item => item.intensity === 0)).toBe(true)
  })
  it('uses the original engine without an active source', () => {
    expect(createAudioLivePlayer().evaluate(show, state, 7).frame).toEqual(evaluateFrame(show, fixtureProfiles, state, 7))
  })
  it('captures the incoming media position when enabling follow while held', () => {
    const player = createAudioLivePlayer(), held = { ...state, mode: 'static' as const }
  ...
  it('retains musical Look transitions when following stable tempo', () => {
    ...
    fixture.regie = { ...fixture.regie!, minimumCoverage: { percent: 0, threshold: .1 }, transition: { quantizeBeats: 0, fadeBeats: 2 } }
    const player = createAudioLivePlayer(), input = { ...audio, mode: 'tempo' as const }
    ...
    expect(player.evaluate(fixture, next, 300, { ...input, seconds: 1.5 }).transition?.progress).toBeCloseTo(.5)
  })
})
