...
function content(node: ReactNode): string {
  return Children.toArray(node)
    .map((child) => (isValidElement<NodeProps>(child) ? content(child.props.children) : String(child)))
    .join('')
}
const flush = async () => {
...
async function setup() {
  ...
  const store = {
    list: vi.fn(async (): Promise<LibraryListing> => ({ entries: [entry], damagedCount: 0 })),
    load: vi.fn(async () => bundle),
    save: vi.fn(async () => entry),
    saveRecovery: vi.fn(async () => entry),
  ...
  const props = {
    ...
    onDeleteVersion: vi.fn(async () => true),
  }
  const render = () => {
    hooks.cursor = 0
    hooks.effects = []
    return ShowLibrary(props)
  }
  render()
  const cleanup = hooks.effects[0]()
  await flush()
  const button = (label: string) =>
    nodes(render()).find((node) => node.type === 'button' && content(node.props.children) === label)!.props
...
it('only reads on mount and opens a validated package through the parent callback', async () => {
  ...
  expect(view.store.save).not.toHaveBeenCalled()
  expect(view.props.onOpen).not.toHaveBeenCalled()
  view.button('Openen').onClick!()
  await flush()
...
it('preserves the editor and dialog when storage or parent backup fails', async () => {
  ...
  await flush()
  expect(view.props.onOpen).not.toHaveBeenCalled()
  expect(content(view.render())).toContain('Beschadigde show')
  view.props.onOpen.mockResolvedValueOnce(false)
  view.button('Openen').onClick!()
  await flush()
  expect(view.props.onClose).not.toHaveBeenCalled()
  expect(content(view.render())).toContain('huidige editor blijft behouden')
})
it('discards a late loaded package after unmount and never opens it', async () => {
  ...
  view.store.load.mockImplementationOnce(
    () =>
      new Promise((done) => {
        resolve = done
      }),
  ...
  view.button('Openen').onClick!()
  view.cleanup!()
  resolve(createShowPackage(initialShow, []))
  await flush()
  expect(view.props.onOpen).not.toHaveBeenCalled()
  expect(view.props.onClose).not.toHaveBeenCalled()
})
it('serializes same-tick save submissions and exposes quota failure without replacing the editor', async () => {
  const view = await setup()
  view.store.save.mockRejectedValueOnce(new Error('Lokale opslag is vol'))
  const submit = nodes(view.render()).find((node) => node.type === 'form')!.props.onSubmit!
  submit({ preventDefault() {} })
  submit({ preventDefault() {} })
  await flush()
  expect(view.store.save).toHaveBeenCalledOnce()
  expect(content(view.render())).toContain('Lokale opslag is vol')
  expect(view.props.onOpen).not.toHaveBeenCalled()
})
it('requires confirmation for deletion and routes version restore through the parent', async () => {
  const view = await setup()
  view.button('Verwijderen').onClick!()
  expect(view.store.remove).not.toHaveBeenCalled()
  vi.mocked(window.confirm).mockReturnValueOnce(true)
  view.button('Verwijderen').onClick!()
  await flush()
  expect(view.store.remove).toHaveBeenCalledExactlyOnceWith('saved')
  view.button('Versies van huidige show · 1/100').onClick!()
  view.button('Herstellen').onClick!()
  await flush()
...
it('shows reserved recovery export-only without opening or overwriting its sole durable copy', async () => {
  ...
  view.button('Bibliotheek vernieuwen').onClick!()
  await flush()
  ...
  expect(
    nodes(view.render()).filter((node) => node.type === 'button' && content(node.props.children) === 'Openen'),
  ).toHaveLength(0)
