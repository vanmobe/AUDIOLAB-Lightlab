...
const child = spawn('dotnet', [resolve('runtime/bin/Debug/net10.0/Lightflow.Runtime.dll')], {
  env: {
    ...process.env,
    LIGHTLAB_RUNTIME_PORT: String(port),
    LIGHTLAB_WORKER_PATH: resolve('runtime-worker/dist/engine.mjs'),
    LIGHTFLOW_AI_PROVIDER: 'offline-templates',
  },
...
const output = (command) =>
  post('/playback/output', { version: 1, sessionId, command, ...(command === 'arm' ? { confirmed: true } : {}) })
try {
  await wait(async () => {
    try {
      return (await read('/health')).status === 'ready'
    } catch {
      return false
    }
  })
  ...
  await delay(100)
  assert.equal(packets.length, 0, 'Even enabled show routes do not auto-arm')
  assert.equal((await post('/playback/output', { version: 1, sessionId, command: 'arm' })).status, 400)
  assert.equal(
    (await post('/playback/output', { version: 1, sessionId: 'stale', command: 'arm', confirmed: true })).status,
    409,
  )
  assert.equal(
    (
  ...
  for (const universe of [1, 2]) {
    ...
    assert.ok(
      frames.some((p) => p.subarray(126).some((value) => value > 0)),
      'Real evaluator light reaches the wire',
    )
    assert.ok(frames.every((p) => p.length === 638 && p[125] === 0 && p[112] === 0))
    assert.equal(frames[1][111], (frames[0][111] + 1) % 256)
  }
  assert.equal(
    (await post('/output/arm', { confirmed: true, host: '127.0.0.1', universe: 1, protocol: 'sacn' })).status,
    409,
    'Legacy sender cannot compete',
  )
  ...
  await delay(100)
  assert.ok(
    packets.slice(-4).every((p) => p.subarray(126).every((value) => value === 0)),
    'Continuous blackout covers both universes',
  )
  ...
  await delay(100)
  assert.ok(
    packets.slice(-4).some((p) => p.subarray(126).some((value) => value > 0)),
    'Look change resumes actual output',
  )
  assert.equal((await output('disarm')).status, 200)
  await delay(50)
  for (const universe of [1, 2])
    assert.equal(packets.filter((p) => p.readUInt16BE(113) === universe && p[112] === 0x40).length, 3)
  const count = packets.length
  await delay(120)
  assert.equal(packets.length, count, 'No sends after acknowledged disarm')
  ...
  const wire = () =>
    packets
      .filter((p) => p.readUInt16BE(113) === 2 && p[112] === 0)
      .at(-1)
      .subarray(126)
  const peak = Buffer.from(wire())
  assert.ok(
    peak.some((value) => value > 0),
    'Kick peak reaches physical DMX encoder',
  )
  assert.equal((await audio('sync', { sequence: 1, position: { ...position, seconds: 1.5 } })).status, 200)
  await delay(100)
  const trough = Buffer.from(wire())
  assert.ok(
    trough.some((value, i) => value < peak[i]),
    'Kick tail reduces real universe channel levels',
  )
  ...
  assert.equal(
    (await post('/playback/command', { version: 1, sessionId, command: 'mode', mode: 'blackout' })).status,
    200,
  )
  assert.equal((await audio('sync', { sequence: 3, position })).status, 200)
  await delay(70)
  assert.ok(
    wire().every((value) => value === 0),
    'Blackout overrides audio kicks on the wire',
  )
  await wait(async () => (await read(`/playback/audio?sessionId=${sessionId}`)).state === 'lost')
  assert.equal(
    (await read(`/playback/output?sessionId=${sessionId}`)).state,
    'disarmed',
    'Missing clock disarms physical output',
  )
  const expiredCount = packets.length
  await delay(100)
  assert.equal(packets.length, expiredCount, 'No indefinite output after heartbeat loss')
  ...
  console.log(
    'PASS: isolated HTTP + real evaluator + two sACN universes; opt-in, fencing, Look changes, WAV kick peak/tail/seek, blackout priority, audio watchdog/disarm, no automatic resume, Stop.',
  )
} finally {
