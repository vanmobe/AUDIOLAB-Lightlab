...
;async (page) => {
  ...
  await page.route('**/ai/propose', async (route) => {
    ...
    assert(
      result.code === 'invalid_ai_request' && result.stage === 'request' && !result.trace,
      'Failure is explicitly pre-provider',
    )
  ...
  try {
    ...
    assert(
      (await page.getByRole('alert').textContent()).includes('niet naar het AI-model'),
      'User can locate the problem and knows AI was not called',
    )
    ...
    assert(
      (await page.getByRole('spinbutton', { name: 'Aantal kleurprofielen', exact: true }).inputValue()) === '1.5',
      'Draft remains available to correct',
    )
    ...
    assert(
      ...
      'Error never mutates stored show',
    )
