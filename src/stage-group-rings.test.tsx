...
interface Props {
  children?: ReactNode; id?: string; className?: string; style?: CSSProperties & { '--group-ring'?: string }
  'aria-label'?: string; 'aria-pressed'?: boolean; 'data-group-highlighted'?: boolean; 'data-group-rings'?: boolean
  placeholder?: string; onSubmit?: (event: { preventDefault: () => void }) => void
  disabled?: boolean; checked?: boolean; onClick?: () => void; onMouseEnter?: () => void; onMouseLeave?: () => void; onFocus?: () => void; onBlur?: () => void; onChange?: (event: { target: { checked?: boolean; value?: string } }) => void
}
...
function setup() {
  ...
  const changed = vi.fn()
  const render = () => { hooks.cursor = 0; return StageEditor({ show, onChange: changed }) }
  const label = (name: string) => find(render(), props => props['aria-label'] === name)
  const group = (id: string) => label(`Selecteer groep ${show.groups.find(g => g.id === id)!.name} (${show.fixtures.filter(f => f.groupId === id).length} lampen)`)
  return { render, label, group, changed, show: () => show, replace: (next: typeof show) => { show = next } }
}
describe('stage group identification', () => {
  ...
  it('highlights on hover/focus without selecting and selects exactly the clicked group', () => {
    ...
    for (const fixture of view.show().fixtures) expect(view.label(fixture.name)['aria-pressed']).toBe(fixture.groupId === 'front')
    view.group('front').onBlur!()
    view.group('wash').onClick!()
    for (const fixture of view.show().fixtures) expect(view.label(fixture.name)['aria-pressed']).toBe(fixture.groupId === 'wash')
    expect(view.changed).not.toHaveBeenCalled()
  })
  ...
  it('keeps colors when groups are renamed/appended and follows reassignment', () => {
    const view = setup(), previousColor = view.group('wash').style?.['--group-ring']
    view.replace({ ...view.show(), groups: [...view.show().groups.map(g => g.id === 'wash' ? { ...g, name: 'Wash links' } : g), { id: 'new', name: 'Nieuw', intensity: 1 }] })
    ...
    view.replace({ ...view.show(), fixtures: view.show().fixtures.map(f => f.id === moved.id ? { ...f, groupId: 'new' } : f) })
  ...
  it.each([255, 256])('enforces the saved-document group limit from %i groups', count => {
    const view = setup()
    view.replace({ ...view.show(), groups: [...view.show().groups, ...Array.from({ length: count - view.show().groups.length }, (_, i) => ({ id: `extra-${i}`, name: `Extra ${i}`, intensity: 1 }))] })
    find(view.render(), props => !!props.onClick && text(props.children) === 'Groepen beheren').onClick!()
    find(view.render(), props => props.placeholder === 'Bijvoorbeeld wash links').onChange!({ target: { value: 'New group' } })
    expect(find(view.render(), props => props.disabled !== undefined && text(props.children) === 'Maak lege groep').disabled).toBe(count === 256)
