...
const layers = (programId: string) =>
  initialShow.groups.map((group) => ({
    groupId: group.id,
...
function proposal(): DesignProposal {
  return {
    provider: 'test',
    summary: 'nieuw',
    colorProfiles: [{ ...initialShow.colorProfiles[0], id: 'generated', name: 'Generated' }],
    programs: [],
    looks: [],
  }
}
const pattern = (variant = 0): Pattern => ({
  version: 1,
  floor: 0.2,
  steps: [
    {
      selection: 'moving',
      direction: (['forward', 'reverse', 'bounce', 'inward'] as const)[Math.floor(variant / 8) % 4],
      envelope: 'hold',
      width: (variant % 8) + 1,
      trail: 0.4,
      level: 1,
...
describe('design proposal acceptance boundary', () => {
  it('allows complete-collection requests to leave individual counts at zero', () => {
    expect(
      designConfigurationError(initialShow, { ...designDefaults, profileCount: 0, programCount: 2, lookCount: 0 }),
    ).toBe('')
    expect(
      designConfigurationError(initialShow, { ...designDefaults, profileCount: 0, programCount: 0, lookCount: 0 }),
    ).toContain('minstens')
    expect(
      designConfigurationError(initialShow, {
        ...designDefaults,
        scope: 'programs',
        profileCount: 0,
        programCount: 0,
    ...
    const p = proposal()
    p.colorProfiles = []
    p.programs = [{ ...initialShow.programs[0], id: 'only-program', rateBeats: 1, pattern: pattern() }]
    const next = proposalCandidate(
      initialShow,
      p,
      { ...designDefaults, profileCount: 0, programCount: 1, lookCount: 0 },
      fingerprint(initialShow),
    )
  ...
  it('allows up to 32 palette suggestions even when existing slots are occupied', () => {
    const p = proposal()
    p.colorProfiles = Array.from({ length: 32 }, (_, i) => ({
      ...initialShow.colorProfiles[0],
      id: `idea-${i}`,
      name: `Idea ${i}`,
      primary: `#${(0x100000 + i).toString(16).slice(-6)}`,
    }))
    ...
    const candidate = proposalCandidate(
      initialShow,
      p,
      { ...designDefaults, scope: 'colorProfiles', profileCount: 32 },
      fingerprint(initialShow),
    )
  ...
  it('replaces only scoped palettes and keeps existing Looks valid', () => {
    ...
    const next = proposalCandidate(
      initialShow,
      p,
      { ...designDefaults, scope: 'colorProfiles', replace: true, profileCount: 2, programCount: 0, lookCount: 0 },
      fingerprint(initialShow),
    )
    expect(next.colorProfiles.map((item) => item.id)).toEqual(['new-warm', 'new-cool'])
    expect(
      next.programs
        .map((item) => item.defaultColorProfileId)
        .every((id) => next.colorProfiles.some((color) => color.id === id)),
    ).toBe(true)
    expect(
      next.looks.map((item) => item.colorProfileId).every((id) => next.colorProfiles.some((color) => color.id === id)),
    ).toBe(true)
    ...
    p.programs = [
      {
        ...
        id: 'band-recipe',
        pattern: { ...pattern(), steps: Array.from({ length: 3 }, () => pattern().steps[0]) },
      },
    ]
  ...
  it('checks animated group speed for new Looks, while retaining existing Look timing', () => {
    ...
    expect(() =>
      proposalCandidate(
        { ...show, bandProfile: { ...show.bandProfile!, motion: 'fast' } },
        p,
        options,
        fingerprint(show),
      ),
  ...
  it('accepts distinct recipes with the same fallback effect and preserves their data', () => {
    ...
    const next = proposalCandidate(
      initialShow,
      p,
      { ...designDefaults, scope: 'programs', programCount: 2 },
      fingerprint(initialShow),
    )
  ...
  it('rejects recipes disguised by ignored fields or proportional weights', () => {
    ...
    p.programs = [first, second].map((recipe, index) => ({
      ...initialShow.programs[0],
      id: `disguised-${index}`,
      pattern: recipe,
    }))
  ...
  it('snapshots old Look timing before an AI pattern revision changes legacy metadata', () => {
    ...
    const next = proposalCandidate(
      initialShow,
      p,
      { ...designDefaults, scope: 'programs', revision: true, programCount: 1 },
      fingerprint(initialShow),
    )
    expect(next.looks[1].layers?.find((layer) => layer.groupId === 'wash')?.rateBeats).toBe(0.5)
  ...
  it('rejects new animation-owned timing and inherited timing in newly proposed Looks', () => {
    ...
    expect(() =>
      proposalCandidate(initialShow, p, { ...opts, scope: 'programs', programCount: 1 }, fingerprint(initialShow)),
    ).toThrow('beatduur per groep')
    p.programs = []
    p.looks = [
      {
        ...initialShow.looks[0],
        id: 'bad-look',
        layers: layers(initialShow.programs[0].id).map((layer) => ({ ...layer, rateBeats: null })),
      },
    ]
    expect(() =>
      proposalCandidate(initialShow, p, { ...opts, scope: 'looks', lookCount: 1 }, fingerprint(initialShow)),
    ).toThrow('eigen beatduur')
  })
  it('replaces the entire collection at requested counts, preserving setup and removing obsolete control targets', () => {
    const p = proposal()
    p.programs = [
      { ...initialShow.programs[0], id: 'replacement-program', defaultColorProfileId: 'generated', pattern: pattern() },
    ]
    p.looks = [
      {
        id: 'replacement-look',
        ...
        colorProfileId: 'generated',
        layers: layers('replacement-program'),
      },
    ]
    const options = { ...designDefaults, replace: true, profileCount: 1, programCount: 1, lookCount: 1 }
    const show = structuredClone(initialShow)
    show.controlSurface.bindings.push(
      { id: 'locked-color', label: 'Oude kleur', action: 'color-lock', targetId: show.colorProfiles[0].id },
      { id: 'free-color-lock', label: 'Kleur vasthouden', action: 'color-lock' },
    )
    ...
    for (const key of ['fixtures', 'groups', 'routes', 'camera', 'sync', 'bandMembers'] as const)
      expect(next[key]).toEqual(initialShow[key])
    expect(next.controlSurface.bindings).toEqual(
      show.controlSurface.bindings.filter((binding) => binding.action !== 'look' && binding.id !== 'locked-color'),
    )
  ...
  it('allows replacement up to 32 regardless of existing counts and supports scoped replacement', () => {
    ...
    const full: DesignProposal = {
      provider: 'test',
      summary: 'replace',
      colorProfiles: Array.from({ length: 32 }, (_, i) => ({ ...initialShow.colorProfiles[0], id: `c-${i}` })),
      programs: Array.from({ length: 32 }, (_, i) => ({
        ...initialShow.programs[0],
        name: `Patroon ${i}`,
        rateBeats: 1,
        id: `p-${i}`,
        defaultColorProfileId: `c-${i}`,
        pattern: pattern(i),
      })),
      looks: Array.from({ length: 32 }, (_, i) => ({
        id: `l-${i}`,
        name: `Look ${i}`,
        programId: `p-${i}`,
        colorProfileId: `c-${i}`,
  ...
  it('rejects stale proposals', () =>
    expect(() =>
      proposalCandidate({ ...initialShow, name: 'changed' }, proposal(), opts, fingerprint(initialShow)),
    ).toThrow('gewijzigd'))
  it('rejects wrong counts, overwrite, malformed summary and invalid colors', () => {
    expect(() =>
      proposalCandidate(initialShow, proposal(), { ...opts, profileCount: 2 }, fingerprint(initialShow)),
    ).toThrow('aantal')
    const p = proposal()
    p.colorProfiles[0].id = initialShow.colorProfiles[0].id
    expect(() => proposalCandidate(initialShow, p, opts, fingerprint(initialShow))).toThrow('overschrijft')
    expect(() =>
      proposalCandidate(initialShow, { ...proposal(), summary: {} }, opts, fingerprint(initialShow)),
    ).toThrow('uitleg')
    const color = proposal()
    color.colorProfiles[0].primary = 'red'
  ...
  it('rejects locked collection edits and dangling references', () => {
    const p = proposal()
    p.looks = [{ id: 'newlook', name: 'x', programId: 'absent', colorProfileId: 'generated' }]
    expect(() => proposalCandidate(initialShow, p, opts, fingerprint(initialShow))).toThrow('aantal')
    expect(() =>
      proposalCandidate(
        initialShow,
        { ...p, colorProfiles: [] },
        { ...opts, scope: 'looks', lookCount: 1 },
        fingerprint(initialShow),
      ),
    ).toThrow('onbekende')
  })
  it('rejects no-op revisions and applies scoped real revisions', () => {
    const p = proposal()
    p.colorProfiles = [structuredClone(initialShow.colorProfiles[0])]
    expect(() => proposalCandidate(initialShow, p, { ...opts, revision: true }, fingerprint(initialShow))).toThrow(
      'niets',
    )
  ...
  it('rejects a recipe revision that changes nothing', () => {
    ...
    const p = proposal()
    p.colorProfiles = []
    p.programs = [structuredClone(show.programs[0])]
    expect(() =>
      proposalCandidate(show, p, { ...opts, scope: 'programs', programCount: 1, revision: true }, fingerprint(show)),
    ).toThrow('niets')
  })
  it('allows oversized idea lists but selection enforces the persisted 32 item total', () => {
    const p = proposal()
    p.colorProfiles = Array.from({ length: 31 }, (_, i) => ({ ...p.colorProfiles[0], id: 'new-' + i }))
    expect(
      proposalCandidate(initialShow, p, { ...opts, profileCount: 31 }, fingerprint(initialShow)).colorProfiles,
    ).toHaveLength(initialShow.colorProfiles.length + 31)
  })
    ...
    p.looks = [
      {
        ...initialShow.looks[0],
        id: 'timed-look',
        layers: layers(initialShow.programs[1].id).map((layer) => ({ ...layer, rateBeats: 8, offsetBeats: 2 })),
      },
    ]
    const options = { ...opts, scope: 'looks' as const, lookCount: 1 }
  ...
  it('accepts distinct fixed layer palettes and persists custom recipe names', () => {
    ...
    p.programs = [
      {
        ...initialShow.programs[0],
        id: 'new-program',
        name: 'Dubbele slinger met lichtstaart',
        effect: 'wave',
        rateBeats: 1,
        pattern: pattern(),
      },
    ...
    const next = proposalCandidate(
      initialShow,
      p,
      { ...opts, scope: 'programs', programCount: 1 },
      fingerprint(initialShow),
    )
    expect(next.programs.at(-1)?.name).toBe('Dubbele slinger met lichtstaart')
    p.programs = []
    p.looks = [{ ...initialShow.looks[0], id: 'new-look', layers: layers(initialShow.programs[0].id) }]
    p.looks[0].layers![0].colorProfileId = initialShow.colorProfiles[1].id
    expect(
      proposalCandidate(initialShow, p, { ...opts, scope: 'looks', lookCount: 1 }, fingerprint(initialShow)).looks.at(
        -1,
      )?.layers?.[0].colorProfileId,
    ).toBe(initialShow.colorProfiles[1].id)
  })
})
