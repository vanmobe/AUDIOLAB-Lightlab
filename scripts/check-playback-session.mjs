...
const post = (path, value) =>
  fetch(base + path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(value) })
...
try {
  ...
  assert.deepEqual(
    heldLater.inspection.universes.map((u) => u.channels),
    held.inspection.universes.map((u) => u.channels),
    'Static holds emitted values',
  )
  await command({ command: 'mode', mode: 'blackout' })
  const black = await waitFor(framePath(), (value) => value.frame.mode === 'blackout')
  assert.ok(
    black.inspection.universes.every((u) => u.channels.every((value) => value === 0)),
    'Blackout zeros every universe',
  )
  ...
  const warm = await waitFor(
    framePath(),
    (value) =>
      value.frame.mode === 'automation' &&
      value.inspection.universes.some((u) => u.channels.some((value) => value > 0)),
  )
  ...
  assert.equal(
    (await post('/playback/command', { version: 1, sessionId: oldId, command: 'stop' })).status,
    409,
    'Old tab cannot stop replacement session',
  )
  assert.equal((await read('/playback/status')).status, 'running')
  console.log(
    'Autonomous real-process playback passed: two universes, browser-independent clock, BPM continuity, static, blackout, invalid command, explicit stop and session fencing. No physical output.',
  )
} finally {
