...
function rig(quantizeBeats: 0 | 1 | 2 | 4 | 8 = 0, fadeBeats = 4) {
  ...
  for (const look of show.looks) look.layers = show.groups.map(group => ({ groupId: group.id, mode: group.id === 'effects' ? 'off' : 'static', programId: null, colorProfileId: null, intensity: 1, rateBeats: 1, offsetBeats: 0 }))
  return show
}
const state = (activeLookId: string, mode: RuntimeMode = 'automation', heldAtBeats?: number) => ({ activeLookId, mode, ...(heldAtBeats === undefined ? {} : { heldAtBeats }) })
const light = (result: ReturnType<ReturnType<typeof createLookTransitionPlayer>['evaluate']>) => result.frame.fixtures[0]
describe('shared Look transitions', () => {
  it('preserves instant legacy frames, including static/haze, when transitions are absent', () => {
    ...
    for (const at of [0, .4, 10]) for (const mode of ['automation', 'static', 'safety', 'blackout'] as const) {
      const input = state(initialShow.looks[at === 0 ? 0 : 1].id, mode, .2)
      expect(player.evaluate(initialShow, fixtureProfiles, input, at)).toEqual({ frame: evaluateFrame(initialShow, fixtureProfiles, input, at) })
    }
  })
  it('waits for the beat boundary and blends emitted colors over a musical duration', () => {
    const show = rig(4), player = createLookTransitionPlayer()
    ...
    expect(halfway.transition?.progress).toBe(.5)
  ...
  it('interruption captures the current mixed frame without a jump or stale source', () => {
    const show = rig(), player = createLookTransitionPlayer()
    player.evaluate(show, fixtureProfiles, state('warm-static'), 0)
  ...
  it('captures an in-flight mix on static, and applies later operator edits at the held beat', () => {
    ...
    expect(player.evaluate(edited, fixtureProfiles, staticState, 11).frame.fixtures.every(fixture => fixture.intensity === 0)).toBe(true)
  })
  it('safety, blackout and explicit master/group edits cancel immediately', () => {
    for (const action of ['safety', 'blackout', 'edit'] as const) {
      ...
      player.evaluate(show, fixtureProfiles, state('warm-static'), 0)
      player.evaluate(show, fixtureProfiles, state('neon-chorus'), 1)
      const next: ShowDocument = action === 'edit' ? { ...show, groups: show.groups.map(group => ({ ...group, intensity: 0 })) } : show
  ...
  it('supports quantized cuts, reset/seek, and never fades edits of the same Look', () => {
    const show = rig(2, 0), player = createLookTransitionPlayer()
    player.evaluate(show, fixtureProfiles, state('warm-static'), 0)
    expect(player.evaluate(show, fixtureProfiles, state('neon-chorus'), .5).transition?.phase).toBe('queued')
  ...
  it('does not cancel a fade for equivalent derived inputs, linking or presentation changes', () => {
    ...
    expect(result.transition?.progress).toBe(.5)
  ...
  it('rejects unknown fields and invalid saved transition bounds', () => {
    ...
    for (const transition of [{ quantizeBeats: 3, fadeBeats: 1 }, { quantizeBeats: 0, fadeBeats: -1 }, { quantizeBeats: 0, fadeBeats: 33 }, { quantizeBeats: 0, fadeBeats: NaN }, { quantizeBeats: 0, fadeBeats: 1, script: '' }]) expect(() => assertShowDocument({ ...show, regie: { ...show.regie, transition } })).toThrow()
  })
})
