...
it('exports only referenced built-in profiles including references exclusively in history', () => {
  const show = { ...initialShow, fixtures: [] },
    version = createVersion(initialShow, 'Historical rig')
  const bundle = createShowPackage(show, [version])
  expect(bundle.fixtureCatalog?.profiles.map((profile) => profile.id).sort()).toEqual(
    [...new Set(initialShow.fixtures.map((fixture) => fixture.profileId))].sort(),
  )
  ...
  expect(createPackageFixtureCatalog([initialShow]).fingerprint).toBe(
    createPackageFixtureCatalog([initialShow, initialShow]).fingerprint,
  )
})
...
it.each(['fingerprint', 'channel', 'color', 'extra', 'missing', 'huge'])(
  'rejects incompatible catalog %s instead of interpreting arbitrary fixture definitions',
  (kind) => {
    const bundle = createShowPackage(initialShow, []),
      catalog = bundle.fixtureCatalog!
...
it('does not mutate installed definitions or show/history while making a portable snapshot', () => {
  const before = JSON.stringify(fixtureProfiles),
    bundle = createShowPackage(initialShow, [])
