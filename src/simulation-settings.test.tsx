...
describe('simulation-only controls', () => {
  ...
  it('applies exposure, masks haze and restores group light on the actual renderer update path', () => {
    ...
    const simulator = Object.assign(Object.create(StageSimulator.prototype), {
      renderer,
      atmosphere,
      ...
      lights: new Map([[fixture.id, [view]]]),
    }) as StageSimulator
    const frame = {
      atBeats: 1,
      mode: 'automation',
      fixtures: [
        { fixtureId: fixture.id, intensity: 1, color: '#ffffff', haze: 0 },
        { fixtureId: 'hazer', intensity: 0, color: '#000000', haze: 1 },
      ],
    } as Parameters<StageSimulator['update']>[0]
    const saved = structuredClone(frame)
    simulator.update(frame, { brightness: 50, hiddenGroupIds: ['haze'] })
    expect(renderer.toneMappingExposure).toBe(0.55)
    ...
    expect(view.beam.material.opacity).toBeCloseTo(0.08)
    ...
    expect(view.beam.material.opacity).toBeCloseTo(0.08)
    expect(atmosphere.density).toBeCloseTo(0.022)
    simulator.update({ ...frame, fixtures: [frame.fixtures[0]] }, { ...defaultSimulationSettings, haze: 35 })
    expect(view.beam.material.opacity).toBeCloseTo(0.028)
  ...
  it('keeps hidden-state feedback visible and offers recovery without live controls', () => {
    const html = renderToStaticMarkup(
      <SimulationControls
        show={initialShow}
        simulationSettings={{ brightness: 50, hiddenGroupIds: ['wash'] }}
        onSimulationSettingsChange={vi.fn()}
