...
export interface AudioLiveSource { read(): AudioLiveSnapshot | undefined }
...
export function createAudioLivePlayer() {
  ...
  const kickTransitions = createLookTransitionPlayer({
    ...
    evaluateFrame(show, _profiles, state, beat) {
      ...
      const frame = audioPreviewFrame(show, state.activeLookId, seconds, audio.analysis,
        audio.reactions, audio.bpm, audio.decayMs, audio.floor,
        state.mode === 'static' ? { ...state, mode: 'automation' } : state)
      return state.mode === 'static' ? { ...frame, mode: 'static', fixtures: frame.fixtures.map(item => ({ ...item, haze: 0 })) } : frame
    },
  })
  ...
  let previousBpm = 120
  return {
    reset() { transitions.reset(); kickTransitions.reset(); kickAudio = undefined; clock = undefined; heldSeconds = undefined; heldBpm = undefined; previousSeconds = 0 },
    evaluate(show: ShowDocument, state: RuntimeState, freeBeats: number, audio?: AudioLiveSnapshot): TransitionResult {
      ...
      if (clock !== nextClock) {
        transitions.reset(); kickTransitions.reset(); kickAudio = undefined; heldSeconds = undefined; heldBpm = undefined; clock = nextClock
        previousSeconds = audio?.seconds ?? 0; previousBpm = audio?.bpm ?? 120
      }
      ...
      if (state.mode === 'static') { heldSeconds ??= previousSeconds; heldBpm ??= previousBpm }
      else { heldSeconds = undefined; heldBpm = undefined }
      ...
      if (audio.mode === 'tempo') return transitions.evaluate(show, fixtureProfiles,
        state.mode === 'static' ? { ...state, heldAtBeats: beat } : state, beat)
      ...
      return kickTransitions.evaluate(show, fixtureProfiles,
        state.mode === 'static' ? { ...state, heldAtBeats: beat } : state, beat)
    },
  }
}
