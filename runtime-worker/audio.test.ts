...
const audio = {
  ...
  mode: 'kicks' as const,
  bpm: 60,
  reactions: { [initialShow.groups[0].id]: 'pulse' as const },
  decayMs: 300,
  floor: 0.1,
}
...
describe('runtime media follower', () => {
  it('returns the same queued, fading, interrupted and held kick transitions as the browser', () => {
    ...
    for (const [seconds, lookIndex, mode, phase] of [
      [0, 0, 'automation', undefined],
      [1.1, 1, 'automation', 'queued'],
      [2, 1, 'automation', 'fading'],
      [3, 1, 'automation', 'fading'],
      [3, 0, 'automation', 'queued'],
      [4.5, 0, 'automation', 'fading'],
      [4.6, 0, 'static', undefined],
      [5, 0, 'blackout', undefined],
      [0.5, 1, 'automation', 'queued'],
    ] as const) {
      ...
      expect(expected.transition?.phase).toBe(seconds === 0.5 ? undefined : phase)
      expect(
        handle({ version: 1, requestId: 'frame', op: 'evaluate', atBeats: seconds, state, audio: position }),
  ...
  it('matches shared browser audio frames including hold, color lock, safety, blackout, seek and tempo', () => {
    ...
    for (const [mode, seconds, follow] of [
      ['automation', 1.1, 'kicks'],
      ['static', 1.5, 'kicks'],
      ['static', 1.8, 'kicks'],
      ['automation', 0.5, 'kicks'],
      ['safety', 2.1, 'kicks'],
      ['blackout', 2.3, 'kicks'],
      ['automation', 3, 'tempo'],
      ['static', 3.2, 'tempo'],
      ['automation', 0.2, 'tempo'],
    ] as const) {
      ...
      const expected = player.evaluate(show, state, seconds, {
        ...position,
        analysis: { ...analysis, waveform: [] },
      } as AudioLiveSnapshot)
      expected.frame.atBeats = seconds
      expect(
        handle({ version: 1, requestId: 'evaluate', op: 'evaluate', atBeats: seconds, state, audio: position }),
      ).toEqual({ version: 1, requestId: 'evaluate', ok: true, ...expected })
    }
  })
  it('rejects invalid analysis and clocks without discarding valid audio, then clears on detach', () => {
    ...
    for (const invalid of [
      ...
      { ...analysis, confidence: NaN },
      { ...analysis, waveform: [] },
    ])
      expect(handle({ ...configure, analysis: invalid }).ok).toBe(false)
    expect(handle(frame).ok).toBe(true)
    for (const invalid of [
      { ...audio, seconds: 11 },
      { ...audio, bpm: 0 },
      { ...audio, reactions: { missing: 'pulse' } },
      { ...audio, floor: -1 },
      { ...audio, script: true },
    ])
