...
import { evaluatePattern, patternDescription, patternSignature, validatePattern, type Pattern, type PatternStep } from './pattern-language'
...
const step = (change: Partial<PatternStep> = {}): PatternStep => ({ selection: 'moving', direction: 'forward', envelope: 'hold', width: 1, trail: 0, level: 1, weight: 1, ...change })
const recipe = (...steps: PatternStep[]): Pattern => ({ version: 1, floor: 0, steps: steps.length ? steps : [step()] })
const values = (pattern: Pattern, phase: number, count = 4) => Array.from({ length: count }, (_, index) => evaluatePattern(pattern, phase, index, count))
    ...
    for (const selection of ['all', 'alternate', 'random', 'moving'] as const) {
      for (const direction of ['forward', 'reverse', 'bounce', 'inward', 'outward'] as const) {
        for (const envelope of ['hold', 'fade-in', 'fade-out', 'pulse'] as const) {
          expect(() => validatePattern({ version: 1, floor: 1, steps: Array.from({ length: 16 }, () => step({ selection, direction, envelope, width: 8, trail: 1, level: 0, weight: 8 })) })).not.toThrow()
        }
      }
    }
  })
  it.each([null, [], {}, { ...recipe(), code: 'alert(1)' }, { ...recipe(), version: 2 }, { ...recipe(), floor: NaN }, { ...recipe(), floor: -1 }, { ...recipe(), floor: 1.1 }, { ...recipe(), steps: [] }, { ...recipe(), steps: Array.from({ length: 17 }, () => step()) }])('rejects malformed recipe %j', value => {
    expect(() => validatePattern(value)).toThrow()
  })
  it.each([{ selection: 'script' }, { direction: 'sideways' }, { envelope: 'custom' }, { width: 0 }, { width: 9 }, { width: 1.5 }, { weight: 0 }, { weight: 9 }, { weight: .5 }, { trail: -1 }, { trail: Infinity }, { level: -1 }, { level: 2 }, { level: '1' }, { duration: 4 }])('rejects malformed step %j', invalid => {
...
describe('relative pattern evaluation', () => {
  it('partitions a cycle using relative step weights and repeats including negative phases', () => {
    const pattern = recipe(step({ selection: 'all', level: .25 }), step({ selection: 'all', level: .75, weight: 3 }))
    expect(values(pattern, .249)).toEqual([.25, .25, .25, .25])
    expect(values(pattern, .25)).toEqual([.75, .75, .75, .75])
    expect(values(pattern, -.5)).toEqual(values(pattern, .5))
  ...
  it('moves forwards, backwards and bounces over group-local points', () => {
    ...
    expect(values(recipe(), .99)).toEqual([0, 0, 0, 1])
    expect(values(recipe(step({ direction: 'reverse' })), 0)).toEqual([0, 0, 0, 1])
    const bounce = recipe(step({ direction: 'bounce' }))
    expect(values(bounce, .5)).toEqual([0, 0, 0, 1])
    expect(values(bounce, .99)).toEqual([1, 0, 0, 0])
  })
  it('mirrors inward/outward heads for even and odd counts', () => {
    const inward = recipe(step({ direction: 'inward' })), outward = recipe(step({ direction: 'outward' }))
    expect(values(inward, 0)).toEqual([1, 0, 0, 1])
    expect(values(inward, .9)).toEqual([0, 1, 1, 0])
    expect(values(outward, 0, 5)).toEqual([0, 0, 1, 0, 0])
    expect(values(outward, .9, 5)).toEqual([1, 0, 0, 0, 1])
  })
  it('uses width for moving windows and keeps a fading tail behind travel only', () => {
    expect(values(recipe(step({ width: 2 })), .5, 6)).toEqual([0, 0, 1, 1, 0, 0])
    const forward = values(recipe(step({ trail: 1 })), .5, 6)
    ...
    expect(values(recipe(step({ direction: 'reverse', trail: 1 })), .5, 6)).toEqual([...forward].reverse())
  })
  it('applies envelopes and a nonzero baseline without exceeding 1', () => {
    expect(values(recipe(step({ selection: 'all', envelope: 'fade-in' })), .25)).toEqual([.25, .25, .25, .25])
    expect(values(recipe(step({ selection: 'all', envelope: 'fade-out' })), .25)).toEqual([.75, .75, .75, .75])
    expect(values(recipe(step({ selection: 'all', envelope: 'pulse' })), .5)).toEqual([1, 1, 1, 1])
    expect(values({ ...recipe(step({ level: .8 })), floor: .4 }, .1)).toEqual([.8, .4, .4, .4])
  })
  it('alternates sets each cycle independently of ignored fields', () => {
    ...
    expect(values(pattern, -1)).toEqual([0, 1, 0, 1])
    expect(values(recipe(step({ selection: 'alternate', width: 8, trail: 1, direction: 'reverse' })), 0)).toEqual(values(pattern, 0))
  })
  it('random selects exact bounded width reproducibly and changes with cycles', () => {
    ...
    for (const count of [1, 2, 5, 11, 32]) {
      for (const phase of [-3.2, 0, .7, 1, 19]) {
...
describe('recipe identity and labels', () => {
  it('deduplicates provably constant recipes without collapsing genuine changes', () => {
    for (const level of [0, .4, 1]) {
      ...
      const hiddenMotion = { ...recipe(step({ selection: 'moving', direction: 'inward', envelope: 'pulse', level, weight: 3 }), step({ selection: 'random', level: level / 2, weight: 7 })), floor: level }
      ...
      for (const phase of [-1.8, 0, .2, .8, 7.1]) for (const count of [1, 4, 9]) expect(values(hiddenMotion, phase, count)).toEqual(values(steady, phase, count))
    }
    const constant = recipe(step({ selection: 'all', level: .4 }), step({ selection: 'all', level: .4, weight: 8 }))
    expect(patternSignature({ effect: 'pulse', pattern: constant })).toBe(patternSignature({ effect: 'pulse', pattern: recipe(step({ selection: 'all', level: .4 })) }))
    const varying = recipe(step({ selection: 'all', level: .4 }), step({ selection: 'all', level: .5 }))
    expect(patternSignature({ effect: 'pulse', pattern: varying })).not.toBe(patternSignature({ effect: 'pulse', pattern: constant }))
  })
  it('ignores name, IDs, fallback effect, ignored fields and proportional step weights', () => {
    const original = { effect: 'pulse', pattern: recipe(step({ selection: 'all', weight: 2 }), step({ selection: 'random', width: 2, weight: 4 })) }
    const equal = { effect: 'static', pattern: recipe(step({ selection: 'all', width: 8, trail: 1, direction: 'reverse' }), step({ selection: 'random', direction: 'bounce', trail: .5, width: 2, weight: 2 })) }
    expect(patternSignature(original)).toBe(patternSignature(equal))
    for (const phase of [0, .2, .5, 1.7]) expect(values(original.pattern, phase)).toEqual(values(equal.pattern, phase))
  ...
  it('keeps genuine motion, envelope and step-order differences', () => {
    ...
    for (const change of [{ direction: 'reverse' }, { envelope: 'pulse' }, { width: 2 }, { trail: .5 }, { level: .5 }] as Partial<PatternStep>[]) {
      expect(patternSignature({ effect: 'pulse', pattern: recipe(step(change)) })).not.toBe(base)
    }
    expect(patternSignature({ effect: 'pulse', pattern: recipe(step({ selection: 'all' }), step()) }))
      .not.toBe(patternSignature({ effect: 'pulse', pattern: recipe(step(), step({ selection: 'all' })) }))
  })
  it('describes movement without timing and preserves custom recipe names', () => {
    const program = { ...initialShow.programs[0], name: 'Dubbele golvende staart', pattern: recipe(step({ direction: 'bounce', trail: .5 })) }
...
describe('recipe frame integration', () => {
  function showWithPattern() {
    ...
    show.programs[0].pattern = recipe()
    show.looks[0].layers = ['front', 'wash', 'back'].map(groupId => ({ groupId, mode: 'animation', programId: show.programs[0].id, colorProfileId: null, intensity: 1, rateBeats: 4, offsetBeats: 0 }))
    return show
  }
  ...
  it('animates every group independently, overrides static fallback and preserves warm-white fixtures', () => {
    ...
    for (const group of ['front', 'wash', 'back']) {
      ...
      const points = frame.fixtures.filter(fixture => ids.has(fixture.fixtureId)).flatMap(fixture => fixture.segments ?? [fixture])
  ...
  it('is independent of program IDs and supports group timing, freeze, blackout and safety', () => {
    ...
    renamed.looks[0].layers!.forEach(layer => { layer.programId = 'new-id' })
    expect(evaluateFrame(renamed, fixtureProfiles, state, 1.3)).toEqual(evaluateFrame(show, fixtureProfiles, state, 1.3))
    const frozen = { ...state, mode: 'static' as const, heldAtBeats: 1 }
    expect(evaluateFrame(show, fixtureProfiles, frozen, 5).fixtures).toEqual(evaluateFrame(show, fixtureProfiles, frozen, 10).fixtures)
    expect(evaluateFrame(show, fixtureProfiles, { ...state, mode: 'blackout' }, 1).fixtures.every(fixture => fixture.intensity === 0)).toBe(true)
    const withoutRecipe = structuredClone(show)
    delete withoutRecipe.programs[0].pattern
    expect(evaluateFrame(show, fixtureProfiles, { ...state, mode: 'safety' }, 1)).toEqual(evaluateFrame(withoutRecipe, fixtureProfiles, { ...state, mode: 'safety' }, 1))
  })
  ...
  it('applies independent group duration and signed offset to relative step progress', () => {
    const base = showWithPattern(), changed = structuredClone(base)
    ...
    for (const beat of [0, 1.5, 5, 9]) {
      ...
      for (const fixture of changed.fixtures.filter(fixture => fixture.groupId === 'wash')) {
        expect(actual.fixtures.find(item => item.fixtureId === fixture.id)).toEqual(expected.fixtures.find(item => item.fixtureId === fixture.id))
      }
    }
  })
})
