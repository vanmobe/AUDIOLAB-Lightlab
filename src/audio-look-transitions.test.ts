...
function rig(quantizeBeats: 0 | 2 = 0, fadeBeats = 2) {
  ...
  show.regie = {
    ...defaultShowRegie(show),
    minimumCoverage: { percent: 0, threshold: 0.1 },
    transition: { quantizeBeats, fadeBeats },
  }
  ...
  const at = (seconds: number, look = 1, mode: RuntimeMode = 'automation') =>
    player.evaluate(show, state(look, mode), 900, { ...audio, seconds })
  at(0, 0)
...
describe('individual-kick Look transitions', () => {
  it('fades both live pulse endpoints and reacts to irregular kicks, not inferred beats', () => {
    ...
    const tail = at(1.25)
    expect(tail.frame.fixtures[0].intensity).toBeCloseTo(0.2)
    const kick = at(1.4)
    expect(kick.frame.fixtures[0].intensity).toBeCloseTo(1)
    expect(kick.transition?.progress).toBeCloseTo(0.4)
    const endpoints = [0, 1].map((index) =>
      audioPreviewFrame(
        show,
        state(index).activeLookId,
        1.4,
        audio.analysis,
        audio.reactions,
        audio.bpm,
    ...
    expect(kick.frame.fixtures).toEqual(
      mixTransitionFrames(endpoints[0], endpoints[1], kick.transition!.progress).fixtures,
    )
  ...
  it('queues on the BPM grid while old Look reacts, and supports zero-duration cuts', () => {
    const { at } = rig(2, 0)
    expect(at(0.6).transition).toMatchObject({ phase: 'queued', startAtBeats: 2 })
    expect(at(0.9).frame.fixtures[0].color).toBe('#ff0000')
    const cut = at(1)
  ...
  it('holds the actual mixed frame, turns haze off and abandons the fade when resuming', () => {
    ...
    expect(player.evaluate(show, state(1, 'static'), 900, { ...audio, seconds: 2.3, bpm: 60 }).frame.fixtures).toEqual(
      held.frame.fixtures,
    )
  ...
  it.each(['blackout', 'safety', 'master'] as const)('%s takes priority over both fading endpoints', (action) => {
    ...
    at(1)
    const input =
      action === 'master' ? { ...show, groups: show.groups.map((group) => ({ ...group, intensity: 0 })) } : show
  ...
  it('preserves fixed-white color throughout the fade and never edits stored Looks', () => {
    ...
    const whiteIds = show.fixtures
      .filter((fixture) => fixture.profileId.includes('varytec'))
  ...
  it('does not lose an exact kick through a BPM conversion roundtrip', () => {
    ...
    const input = {
      ...
      seconds: 0.7,
      bpm: 90,
      analysis: { ...audio.analysis, kicks: [{ time: 0.7, strength: 1 }] },
    }
