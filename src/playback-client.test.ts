...
it('strictly validates phase, counters, status enums and output flag', () => {
  ...
  for (const change of [
    ...
    expect(() => parsePlaybackStatus({ ...status(), ...change })).toThrow()
  expect(parsePlaybackStatus({ ...status(), status: 'idle', sessionId: null, lookId: null })).toMatchObject({
    status: 'idle',
  })
})
it('uses only explicit playback routes with bounded snapshot and session commands', async () => {
  const fetch = vi.fn().mockImplementation(async () => new Response(JSON.stringify(status())))
  ...
  expect(fetch.mock.calls.map((call) => call[0])).toEqual([
    'http://127.0.0.1:5188/playback/status',
    'http://127.0.0.1:5188/playback/start',
...
it('never retries conflicting mutations and rejects mismatched session replies', async () => {
  ...
  vi.stubGlobal('fetch', fetch)
  await expect(commandPlayback('session-1', { command: 'stop' }, new AbortController().signal)).rejects.toBeInstanceOf(
    PlaybackConflict,
  )
...
it('bounds request duration and warns that a timed-out mutation may have executed', async () => {
  ...
  vi.stubGlobal(
    ...
    vi.fn(
      (_url, options) =>
        new Promise((_resolve, reject) => options.signal.addEventListener('abort', () => reject(new Error('abort')))),
    ),
  )
  const failed = expect(
    commandPlayback('session-1', { command: 'stop' }, new AbortController().signal),
  ).rejects.toThrow('opdracht kan wel ontvangen zijn')
...
it('prepares a detached frozen snapshot and validates the entire start envelope before any request', () => {
  ...
  expect(Object.isFrozen(prepared.looks[0])).toBe(true)
  ...
  expect(() =>
    preparePlaybackSnapshot(
      { ...initialShow, diagnosticPadding: 'x'.repeat(2 * 1024 * 1024) } as typeof initialShow,
      120,
    ),
  ).toThrow('2 MiB')
  expect(fetch).not.toHaveBeenCalled()
