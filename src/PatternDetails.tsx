...
export function PatternDetails({ pattern }: { pattern: Pattern }) {
  ...
  return <details className="pattern-details">
    <summary>Samengesteld patroon · {pattern.steps.length} {pattern.steps.length === 1 ? 'stap' : 'stappen'} · basisniveau {Math.round(pattern.floor * 100)}%</summary>
    <ol>{pattern.steps.map((step, index) => <li key={index}>
    ...
    <p className="muted">Deze stappen herhalen binnen de beatduur van elke groep. Geen eigen tempo of kleuren in het patroon.</p>
  </details>
}
