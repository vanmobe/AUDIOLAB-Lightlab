...
export function validateBandProfile(value: unknown): asserts value is BandProfile {
  ...
  if (Object.keys(profile).length !== keys.length || keys.some((key) => !Object.hasOwn(profile, key)))
    throw new Error('Bandprofiel bevat ontbrekende of onbekende velden.')
  for (const [key, limit] of [
    ...
    if (typeof profile[key] !== 'string' || profile[key].length > limit)
      throw new Error(`Bandprofiel ${key} mag maximaal ${limit} tekens bevatten.`)
  }
  for (const [key, choices] of [
    ...
    if (typeof profile[key] !== 'string' || !(choices as readonly string[]).includes(profile[key]))
      throw new Error(`Ongeldige bandvoorkeur: ${key}.`)
  }
  if (
    !Array.isArray(profile.preferredColors) ||
    profile.preferredColors.length > 4 ||
    Array.from(profile.preferredColors).some((color) => typeof color !== 'string' || !/^#[0-9a-f]{6}$/i.test(color))
  )
    throw new Error('Kies maximaal vier geldige voorkeurskleuren.')
}
...
export function bandDesignLimits(profile?: BandProfile) {
  ...
  const [minRateBeats, maxRateBeats] = { auto: [0.125, 64], slow: [8, 32], medium: [2, 8], fast: [0.25, 2] }[
    profile?.motion ?? 'auto'
  ]
...
export function bandProfileSummary(profile?: BandProfile): string {
  ...
  const count =
    [profile.colorMood, profile.energy, profile.complexity, profile.motion].filter((value) => value !== 'auto').length +
    (profile.preferredColors.length ? 1 : 0)
