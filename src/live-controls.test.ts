...
const render = (controls = emptyLiveControls(), runtime = state, show = initialShow, at = 0.25) => {
...
describe('live group links', () => {
  ...
  it('linking and unlinking never copy settings or change current output', () => {
    const original = updateLiveGroups(initialShow, emptyLiveControls(), 'wash', {
      colorProfileId: 'neon',
      intensity: 0.4,
    })
...
describe('live group overrides', () => {
  it('changes only the requested fields of linked targets and resets only those targets', () => {
    let controls = updateLiveGroups(initialShow, emptyLiveControls(), 'back', {
      colorProfileId: 'warm',
      intensity: 0.3,
    })
    controls = updateLiveGroups(initialShow, controls, 'front', { intensity: 0.8 })
    controls = linkLiveGroups(initialShow, controls, ['wash', 'back'])
    controls = updateLiveGroups(initialShow, controls, 'wash', { mode: 'animation', programId: 'pulse' })
    expect(controls.overrides.back).toEqual({
      colorProfileId: 'warm',
      intensity: 0.3,
      mode: 'animation',
      programId: 'pulse',
    ...
    expect(resetLiveGroups(initialShow, controls, 'back').overrides).toEqual({ front: { intensity: 0.8 } })
  })
  it('copies each linked group’s own source Look layer and captures following palettes', () => {
    ...
    show.looks[1].layers = [
      { groupId: 'front', mode: 'static', programId: null, colorProfileId: 'warm', intensity: 0.6 },
      { groupId: 'wash', mode: 'animation', programId: 'pulse', colorProfileId: null, intensity: 0.9 },
    ]
    ...
    expect(controls.overrides.front).toEqual({
      mode: 'static',
      programId: null,
      colorProfileId: 'warm',
      intensity: 0.6,
    ...
    expect(controls.overrides.wash).toEqual({
      mode: 'animation',
      programId: 'pulse',
      colorProfileId: 'neon',
      intensity: 0.9,
  ...
  it('preserves original references without effective overrides, even with links or stale entries', () => {
    for (const controls of [
      emptyLiveControls(),
      { overrides: {}, links: [['wash', 'back']] },
      { overrides: { gone: { intensity: 0.2 } }, links: [] },
    ]) {
  ...
  it('sanitizes deleted references and invalid fields without discarding independent valid fields', () => {
    const controls = {
      overrides: {
        wash: { mode: 'animation' as const, programId: 'deleted', colorProfileId: 'deleted', intensity: 0.3 },
        back: { intensity: NaN },
      },
      links: [],
    }
    ...
    expect(wash).toMatchObject({ mode: 'static', programId: null, colorProfileId: null, intensity: 0.3 })
  ...
  it('static/off modes clear program reference in the ephemeral layer, not the independent override', () => {
    ...
    expect(
      livePreview(initialShow, state, controls).show.looks[0].layers!.find((l) => l.groupId === 'wash'),
    ).toMatchObject({ mode: 'off', programId: null })
  ...
  it.each(['blackout', 'safety', 'static'] as const)('changes never resume %s or erase held beat', (mode) => {
    const controls = updateLiveGroups(initialShow, emptyLiveControls(), 'wash', {
      mode: 'animation',
      programId: 'pulse',
      colorProfileId: 'neon',
    })
    const runtime = { ...state, mode, heldAtBeats: 0.3 }
    ...
    if (mode !== 'static') expect(render(controls, runtime)).toEqual(render(emptyLiveControls(), runtime))
      ...
      expect(render(controls, runtime, initialShow, 10).fixtures).toEqual(
        render(controls, runtime, initialShow, 20).fixtures,
      )
  })
  it('falls back to the same Look as the domain for a deleted active Look and handles no Looks', () => {
    const controls = updateLiveGroups(initialShow, emptyLiveControls(), 'wash', { intensity: 0.2 })
  ...
  it('does not mutate stored show, links, existing overrides or source Look arrays', () => {
    ...
    updateLiveGroups(show, changed, 'wash', { intensity: 0.2 })
  ...
  it('treats imported prototype-like IDs as ordinary own group keys', () => {
    ...
    const controls = updateLiveGroups(show, emptyLiveControls(), '__proto__', { mode: 'static', intensity: 0.5 })
