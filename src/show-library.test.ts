...
function database() {
    ...
    transaction(_names: string[], mode: string) {
      ...
      const tx = {
        ...
        objectStore(name: string) {
          ...
          function request(action: () => unknown, write = false) {
            ...
            queueMicrotask(() => {
              ...
              if (!pending)
                queueMicrotask(() => {
                  if (!aborted && !pending) {
                    if (mode === 'readwrite') for (const [name, values] of working) stores.set(name, values)
                    tx.oncomplete?.()
                  }
                })
            })
          ...
          return {
            get: (id: string) => request(() => structuredClone(entries.get(id))),
            getAll: (_query: unknown, count: number) =>
              request(() => structuredClone([...entries.values()].slice(0, count))),
            count: () => request(() => entries.size),
            put: (value: { id: string }) => request(() => entries.set(value.id, structuredClone(value)), true),
            delete: (id: string) => request(() => entries.delete(id), true),
          }
        },
      }
      return tx
  ...
  return {
    library: createShowLibrary(factory),
    stores,
    fail: () => {
      failWrites = true
    },
...
it('saves, reloads, renames and removes complete packages without altering version snapshots', async () => {
  ...
  expect(loaded.show.name).toBe('Concert A')
  ...
  expect((await library.list()).entries).toEqual([])
...
it('rolls back metadata and payload together on quota failure', async () => {
  const { library, fail } = database(),
    saved = await library.save('Saved', bundle())
  fail()
...
it('reserves one replaceable atomic recovery slot outside all 32 named shows', async () => {
  ...
  expect(listing.entries).toHaveLength(32)
  expect(listing.recovery?.name).toBe('Latest editor')
  expect((await library.load(recovery.id)).show.name).toBe('Latest editor')
  fail()
  await expect(library.saveRecovery('Failed replacement', bundle())).rejects.toThrow('opslag is vol')
  expect((await library.load(recovery.id)).show.name).toBe('Latest editor')
  expect((await library.list()).entries).toHaveLength(32)
})
it('isolates damaged metadata and selected payloads rather than breaking all shows', async () => {
  const { library, stores } = database(),
    first = await library.save('Good', bundle()),
    second = await library.save('Bad payload', bundle())
  ...
  await library.remove('bad')
  expect((await library.list()).damagedCount).toBe(0)
...
it('bounds names, versions and bytes before writing', () => {
  ...
  expect(() =>
    libraryPayload('Large', { ...bundle(), unused: 'x'.repeat(20_000_001) } as ReturnType<typeof bundle>),
  ).toThrow('te groot')
  expect(() =>
    libraryPayload('Versions', {
      ...bundle(),
      versions: Array.from({ length: 101 }, () => createVersion(initialShow, 'x')),
    }),
  ).toThrow()
