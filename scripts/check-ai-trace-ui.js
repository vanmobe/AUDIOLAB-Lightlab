...
;async (page) => {
  ...
  await page
    .getByLabel('Beschrijf kleur, energie en ritme')
    .fill(
      'Maak één warm aards kleurprofiel voor een rustige akoestische folkband, met koperen hoofdkleur, zachte groene accentkleur, amber nevenskleur en warmwit. Geen animaties of looks.',
    )
  ...
  assert(
    body.trace?.provider === 'ollama' && body.trace.attempts.length > 0,
    'Real provider capture present on success or failure',
  )
  const wire = body.trace.attempts[0]
  assert(
    wire.requestBody.includes('minimumCoverage') &&
      wire.requestBody.includes('colorRoles') &&
      wire.requestBody.includes('fixtures'),
    'Actual request includes settings and setup',
  )
  assert(wire.responseBody && wire.responseStatus, 'Raw real model response captured')
  if (!(await page.locator('.ai-diagnostics').evaluate((el) => el.open)))
  ...
  assert(
    await page.locator('.ai-diagnostics').evaluate((el) => el.scrollWidth <= el.clientWidth + 1),
    'Trace wraps at mobile width',
  )
  ...
  assert(
    ...
    'Diagnosis/generation does not change active show',
  )
  return {
    status: reply.status(),
    attempts: body.trace.attempts.length,
    requestCharacters: wire.requestBody.length,
    responseCharacters: wire.responseBody.length,
    success: reply.ok(),
    note: 'Actual local Ollama wire capture/render/clear and mobile wrapping verified. Proposal not accepted.',
  }
}
