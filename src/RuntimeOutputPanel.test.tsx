...
const render = () => {
  hooks.cursor = 0
  return RuntimeOutputPanel({ sessionId: 'test', running: true })
}
...
beforeEach(async () => {
  ...
  hooks.values = []
  hooks.cursor = 0
  client.read.mockReset().mockResolvedValue(status)
  client.write.mockReset().mockResolvedValue({ ...status, state: 'armed' })
  render()
...
it('blocks invalid and missing routes, including direct handler invocation', async () => {
  for (const invalid of [
    { ...status, armError: 'Ongeldige patch' },
    { ...status, routes: [] },
  ]) {
