;async (page) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    show.programs[0].pattern = {
      ...
      floor: 0.2,
      steps: [
        { selection: 'moving', direction: 'bounce', envelope: 'hold', width: 1, trail: 0.4, level: 1, weight: 1 },
      ],
    }
    assertShowDocument(show)
    const body = inspectionRequest(show, show.looks[0].id, 'automation', 0.375, 'browser-real-recipe')
    ...
    const rows = new Map(
      encoded.universes.flatMap((universe) => universe.fixtures).map((fixture) => [fixture.fixtureId, fixture]),
    )
    const rgb = (output) =>
      [1, 3, 5].map((start) => Math.round(parseInt(output.color.slice(start, start + 2), 16) * output.intensity))
    for (const output of [...one, ...four]) {
      ...
      const expected =
        physical.profileId === 'varytec-theater-spot-100'
          ? [Math.round(output.intensity * 255), 0]
          : output.segments.length === 4
            ? [...output.segments.flatMap(rgb), 0, 255]
            : [...rgb(output), 0]
      if (JSON.stringify(channels) !== JSON.stringify(expected)) throw new Error(`Recipe mismatch: ${output.fixtureId}`)
    }
    return {
      oneHeadFixtures: one.length,
      fourHeadFixtures: four.length,
      universes: encoded.universes.length,
      outputSent: encoded.outputSent,
    }
  })
