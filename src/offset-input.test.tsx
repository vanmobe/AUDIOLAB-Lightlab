...
describe('exact offset input commit', () => {
  it.each(['1.25', '-0.5', '0', '64', '-64'])('writes %s into the Look without leaving the field', (value) => {
    ...
    const control = GroupTimingControls({
      show,
      layers: [layer],
      onChange: (change) => {
        saved = editLookLayer(show, saved, 'wash', change)
      },
    })
    exactInput(control)!.onChange({ target: { value } })
    expect(
      JSON.parse(JSON.stringify(saved)).layers.find((item: { groupId: string }) => item.groupId === 'wash').offsetBeats,
    ).toBe(Number(value))
  })
