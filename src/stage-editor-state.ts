...
export function stageSnapshot(show: ShowDocument) {
  return {
    fixtures: show.fixtures.map(({ id, name, position, aim, aimMode, groupId }) => ({ id, name, position, aim, aimMode, groupId })),
...
export function restoreStage(show: ShowDocument, snapshot: StageSnapshot): ShowDocument {
  ...
  return {
    ...
    groups: [
      ...snapshot.groups.map(g => ({ ...g, intensity: show.groups.find(item => item.id === g.id)?.intensity ?? 1 })),
