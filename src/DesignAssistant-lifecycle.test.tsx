...
const visibleText = (node: ReactNode): string =>
  Children.toArray(node)
    .map((child) =>
      isValidElement<Props>(child) ? (child.props.hidden ? '' : visibleText(child.props.children)) : String(child),
    )
    .join('')
function button(node: ReactNode, label: string): Props {
  for (const child of Children.toArray(node)) {
    ...
    try {
      return button(child.props.children, label)
    } catch {
      /* next sibling */
    }
  }
...
describe('AI request lifecycle', () => {
  ...
  it('guides scope to brief without generating or losing count drafts', () => {
    ...
    requestInputs(render()).find((input) => input['aria-label'] === 'Aantal kleurprofielen')!.onChange!({
      target: { value: '3' },
    })
  ...
  it('normalizes hidden count drafts on the wire while preserving their editor values', async () => {
    const fetch = vi.fn().mockResolvedValue(Response.json({ error: 'test' }, { status: 502 }))
    vi.stubGlobal('fetch', fetch)
    requestInputs(render()).find((input) => input['aria-label'] === 'Aantal kleurprofielen')!.onChange!({
      target: { value: '1.5' },
    })
    ...
    expect(JSON.parse(fetch.mock.calls[0][1].body).options).toMatchObject({
      scope: 'programs',
      profileCount: 0,
      programCount: 8,
      lookCount: 0,
  ...
  it('allows zero counts inside a complete request when at least one collection is requested', async () => {
    ...
    requestInputs(render()).find((input) => input['aria-label'] === 'Aantal animaties')!.onChange!({
      target: { value: '2' },
    })
    requestInputs(render()).find((input) => input['aria-label'] === 'Aantal Looks')!.onChange!({
      target: { value: '0' },
    })
    expect(button(render(), 'Maak voorstel').disabled).toBe(false)
    await button(render(), 'Maak voorstel').onClick!()
  ...
  it('explains generic legacy input errors without asserting a runtime mismatch', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          Response.json({ error: 'Ontwerpaanvraag heeft ongeldige of ontbrekende velden.' }, { status: 400 }),
        ),
    )
    await button(render(), 'Maak voorstel').onClick!()
  ...
  it('reads a bounded local timeout preference and captures it in the request outside show data', async () => {
    vi.stubGlobal('localStorage', {
      getItem: (key: string) => (key === 'lightlab-ollama-timeout-minutes' ? '45' : null),
    })
    const fetch = vi.fn().mockImplementation(async () => Response.json({ error: 'test' }, { status: 502 }))
    vi.stubGlobal('fetch', fetch)
    await button(render(), 'Maak voorstel').onClick!()
  ...
  it('ignores stale JSON and does not clear the newer request pending state', async () => {
    const oldJson = deferred<string>(),
      newerResponse = deferred<unknown>()
    const oldResponse = new Response(
      new ReadableStream({
        async start(controller) {
          controller.enqueue(new TextEncoder().encode(await oldJson.promise))
          controller.close()
        },
      }),
    )
    ...
    oldJson.resolve(
      JSON.stringify({
        summary: 'OLD RESULT',
        trace: { version: 1, provider: 'ollama', model: null, attempts: [], truncated: false },
      }),
    )
    await oldRequest
  ...
  it('retains raw trace for rejection and clears it on demand and on a new request', async () => {
    const trace = {
      ...
      attempts: [
        {
          requestBody: '<script>private request</script>',
          responseBody: 'RAW REJECTION',
          responseStatus: 200,
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation(async () => Response.json({ error: 'Rejected', trace }, { status: 502 })),
    )
    await button(render(), 'Maak voorstel').onClick!()
...
describe('candidate WebGL recovery', () => {
  it('localizes initialization failure and retries without discarding its input show', () => {
    ...
    const render = () => {
      harness.cursor = 0
      harness.effects = []
      return CandidatePreview({ show, lookId: show.activeLookId })
    }
    render()
