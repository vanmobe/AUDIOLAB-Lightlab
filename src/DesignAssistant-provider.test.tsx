...
vi.mock('react', async original => ({
  ...
  useMemo: (fn: () => unknown) => fn(), useEffect: (fn: () => unknown) => { hooks.effects.push(fn) },
}))
interface Props { children?: ReactNode; disabled?: boolean; value?: string; type?: string; 'aria-label'?: string; onClick?: () => Promise<void>; onChange?: (event: { target: { value: string; checked?: boolean } }) => void }
...
function find(node: ReactNode, predicate: (props: Props) => boolean): Props {
  for (const child of Children.toArray(node)) {
    ...
    try { return find(child.props.children, predicate) } catch { /* next */ }
  }
  throw new Error('Control not found')
}
const render = () => { hooks.cursor = 0; hooks.effects = []; return DesignAssistant({ show: initialShow, onAccept: vi.fn() }) }
...
let models: string[]
beforeEach(() => {
  hooks.values = []; models = ['model-a', 'model-b']
  vi.stubGlobal('fetch', vi.fn(async (url: string) => url.endsWith('/status') ? Response.json({ provider: 'ollama', providers: ['ollama', 'copilot'], model: 'local', aiConfigured: true, runtimeVersion: '0.1.0', aiContractVersion: 'test-contract' }) : url.includes('/models') ? Response.json({ models, defaultModel: null }) : Response.json({ error: 'test-stop' }, { status: 502 })))
})
