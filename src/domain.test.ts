...
const show: ShowDocument = {
  ...
  name: 'Test show',
  groups: [
    { id: 'front', name: 'Front', intensity: 1 },
    { id: 'wash', name: 'Wash', intensity: 1 },
  ],
  fixtures: [
    {
      id: 'front-1',
    ...
    },
    {
      id: 'wash-1',
      name: 'Wash 1',
      profileId: 'adj-mega-tripar-profile-plus',
      modeId: '4ch',
      groupId: 'wash',
      patch: { universe: 1, address: 3 },
      position: [1, 3, 0],
      aim: [0, 0, 0],
    },
  ],
  ...
  programs: [
    {
      ...
      name: 'Steady',
      effect: 'static',
      targetGroupIds: ['front', 'wash'],
      rateBeats: 1,
      defaultColorProfileId: 'warm',
    },
  ],
...
describe('lighting domain', () => {
  it.each(['automation', 'static'] as const)(
    'keeps a valid empty design dark in %s while retaining front-only mode',
    (mode) => {
      ...
      expect(
        evaluateFrame(empty, fixtureProfiles, state, 1).fixtures.every(
          (fixture) => fixture.intensity === 0 && fixture.haze === 0,
        ),
      ).toBe(true)
      expect(evaluateFrame(empty, fixtureProfiles, { ...state, mode: 'safety' }, 1).fixtures[0].intensity).toBe(0.8)
    },
  )
  it('keeps front-only mode independent of a dark creative palette or color lock', () => {
    ...
    edited.groups[0].intensity = 0.5
    const state = { mode: 'safety' as const, activeLookId: edited.activeLookId, colorLockId: 'dark' }
    expect(evaluateFrame(edited, fixtureProfiles, state, 1).fixtures[0].intensity).toBe(0.4)
  ...
  it('does not attenuate static and pulse programs with the inactive chase level', () => {
    ...
    expect(evaluateFrame(edited, fixtureProfiles, state, 0.25).fixtures[0].intensity).toBeCloseTo(0.8)
    edited.programs[0].effect = 'chase'
    expect(evaluateFrame(edited, fixtureProfiles, state, 0).fixtures[1].intensity).toBeCloseTo(0.28)
  })
  ...
  it.each(['static', 'pulse', 'chase'] as const)(
    'keeps the Varytec emitter fixed throughout %s and color locks',
    (effect) => {
      const edited = structuredClone(show)
      edited.programs[0].effect = effect
      edited.colorProfiles.push({
        ...edited.colorProfiles[0],
        id: 'blue',
        primary: '#0000ff',
        accent: '#ff00ff',
        white: '#00ff00',
      })
      for (const beat of [0, 0.25, 1, 2.5, 16]) {
        const frame = evaluateFrame(
          edited,
          fixtureProfiles,
          { mode: 'automation', activeLookId: edited.activeLookId, colorLockId: 'blue' },
          beat,
        )
  ...
  it('blocks two armed protocols on the same universe', () => {
    ...
    expect(
      validateShow(invalid, fixtureProfiles).some((issue) =>
        issue.message.includes('meer dan één actieve outputroute'),
      ),
    ).toBe(true)
  })
...
describe('expanded spatial patterns', () => {
  ...
  const state = { mode: 'automation' as const, activeLookId: 'warm-look' }
  const effects = animationEffects
    .filter((item) => !['static', 'pulse', 'chase'].includes(item.id))
    .map((item) => item.id)
  ...
  it.each(effects)('%s is seek-deterministic, freezes and respects targets, group levels and blackout', (effect) => {
    ...
    expect(levels.every((level) => level >= 0 && level <= 0.5)).toBe(true)
    expect(new Set(levels).size).toBeGreaterThan(1)
    expect(evaluateFrame(edited, fixtureProfiles, { ...state, mode: 'static', heldAtBeats: 1.4 }, 99).fixtures).toEqual(
      frame.fixtures,
    )
    expect(
      evaluateFrame(edited, fixtureProfiles, { ...state, mode: 'blackout' }, 1.4).fixtures.every(
        (item) => item.intensity === 0 && item.haze === 0 && !item.segments,
      ),
    ).toBe(true)
  })
  it('runs a full four-head sequence in rateBeats, then repeats', () => {
    ...
    for (let beat = 0; beat <= 4; beat++) {
      ...
      expect(output.segments!.map((item) => item.intensity)).toEqual(
        [0, 1, 2, 3].map((index) => (index === beat % 4 ? 1 : 0)),
      )
      expect(output.intensity).toBe(0.25)
    }
  })
  ...
  it('builds progressively and resets at the complete cycle boundary', () => {
    ...
    expect(
      [0, 1, 2, 3, 4].map((beat) => evaluateFrame(edited, fixtureProfiles, state, beat).fixtures[1].intensity),
    ).toEqual([0.25, 0.5, 0.75, 1, 0.25])
  })
  it('orders target fixtures by x, then z and stable id, not document order', () => {
    ...
    expect(
      [0, 4 / 3, 8 / 3].map(
        (beat) =>
          evaluateFrame(edited, fixtureProfiles, state, beat).fixtures.find((item) => item.intensity === 1)!.fixtureId,
      ),
    ).toEqual(['a', 'b', 'c'])
  })
  it('twinkles fade within their interval and waves move smoothly', () => {
    ...
    const nearby = evaluateFrame(edited, fixtureProfiles, state, 0.001).fixtures[1].segments!
    expect(first.every((segment, index) => Math.abs(segment.intensity - nearby[index].intensity) < 0.01)).toBe(true)
  ...
  it.each(effects)('%s ignores unrelated fixtures and storage order and limits RGB to two palette colors', (effect) => {
    ...
    expect(
      evaluateFrame(edited, fixtureProfiles, state, 1.4).fixtures.find((item) => item.fixtureId === before.fixtureId),
    ).toEqual(before)
  ...
  it.each(effects)('%s preserves fixed warm white and does not treat hazers as light points', (effect) => {
    ...
    edited.fixtures.push({
      ...edited.fixtures[0],
      id: 'haze',
      profileId: 'stairville-hz-200',
      modeId: '1ch',
      position: [-10, 0, 0],
    })
  ...
  it('random selections change across intervals without changing mid-interval', () => {
    ...
    expect(evaluateFrame(edited, fixtureProfiles, state, 0.1).fixtures).toEqual(
      evaluateFrame(edited, fixtureProfiles, state, 3.9).fixtures,
    )
    const patterns = [0, 4, 8, 12, 16, 20].map((beat) =>
      JSON.stringify(evaluateFrame(edited, fixtureProfiles, state, beat).fixtures[1].segments),
    )
