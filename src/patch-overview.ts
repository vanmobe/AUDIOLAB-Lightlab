  ...
  const conflicts =
    !fixture.patch || !end
      ...
      : show.fixtures.filter((other) => {
          ...
          return (
            otherMode &&
            other.patch.address <= end &&
            other.patch.address + otherMode.channels - 1 >= fixture.patch!.address
          )
        })
...
export function patchInputError(universe: string, address: string, channels: number) {
  if (!/^\d+$/.test(universe) || Number(universe) < 1 || Number(universe) > 63999)
    return 'Kies een universe van 1 tot en met 63999.'
  if (!/^\d+$/.test(address) || Number(address) < 1 || Number(address) + channels - 1 > 512)
    return `Kies een startadres van 1 tot en met ${513 - channels}; deze modus gebruikt ${channels} kanalen.`
  return ''
}
...
export function routeUniverseError(universe: string, protocol: string) {
  ...
  return (
    patchInputError(universe, '1', 1) ||
    (protocol === 'artnet' && Number(universe) > 32768
      ? 'Art-Net ondersteunt in Lightlab universe 1 tot en met 32768.'
      : '')
  )
}
