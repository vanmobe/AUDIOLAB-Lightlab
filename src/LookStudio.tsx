...
export function LookStudio({
  ...
  useEffect(() => {
    inputs.current = { show, bpm, lookId: selectedLook?.id }
  }, [show, bpm, selectedLook?.id])
  useEffect(() => {
    ...
    try {
      ...
      setPreviewError(
        simulationErrorMessage(
          error,
          'De 3D-preview kon niet starten. Controleer of WebGL beschikbaar is in je browser. Je kunt de Look wel blijven bewerken.',
        ),
      )
      return
    }
    ...
    const update = (now: number) => {
      ...
      if (current.lookId) {
        const result = player.evaluate(
          current.show,
          fixtureProfiles,
          { mode: 'automation', activeLookId: current.lookId },
          beat.current,
        )
  ...
  if (!selectedLook)
    return (
      <section className="look-studio-empty">
        <h2>Looks & repetitie</h2>
        <p>Maak eerst een Look bij AI-ontwerp om deze te bewerken en meteen op het podium te bekijken.</p>
      </section>
    )

  const invalidBpm = validPreviewBpm(bpmDraft) === undefined
  return (
    <section className="look-studio" aria-label="Look bewerken en repeteren">
      <header className="look-studio-heading">
        <div>
          <h2>Looks & repetitie</h2>
          <p className="muted">Bewerk een groep en zie meteen wat er op het podium verandert.</p>
        </div>
        <label className="look-studio-mobile-picker">
          Look
          <select
            aria-label="Look in de studio"
      ...
      <div className="look-studio-layout">
        <nav className="look-studio-library" aria-label="Looks in de studio">
          <h3>
            Looks <small>{show.looks.length}</small>
          </h3>
          {show.looks.length > 6 && (
            <input
              type="search"
              aria-label="Zoek studio-Look"
              placeholder="Zoek een Look…"
              value={lookSearch}
              onChange={(event) => setLookSearch(event.target.value)}
            />
          )}
          <div className="look-studio-look-list">
            {show.looks
              .filter(
                (look) =>
                  show.looks.length <= 6 ||
                  look.name.toLocaleLowerCase().includes(lookSearch.trim().toLocaleLowerCase()),
              )
              .map((look) => (
                <button
                  key={look.id}
            ...
            {show.looks.length > 6 &&
              !show.looks.some((look) =>
                look.name.toLocaleLowerCase().includes(lookSearch.trim().toLocaleLowerCase()),
              ) && <p className="muted">Geen Looks gevonden.</p>}
          </div>
        </nav>
        <aside className="look-studio-preview" aria-label="Repetitievoorbeeld">
          ...
          {previewError && (
            <p role="alert" className="look-studio-error">
              {previewError}
            </p>
          )}
          ...
          {diagnostic && (
            <>
              <TransitionStatus show={show} transition={diagnostic.transition} />
              <CoverageStatus
                show={show}
                frame={diagnostic.frame}
                transitioning={diagnostic.transition?.phase === 'fading'}
              />
            </>
          )}
          <div className="look-studio-tempo">
            <div className="look-studio-tempo-title">
              <strong>Repetitietempo</strong>
              <span>Alleen voor dit voorbeeld · geen live-uitvoer</span>
            </div>
            <div className="look-studio-tempo-inputs">
              <input
                type="range"
                aria-label="Repetitietempo"
                min={previewBpmRange.min}
                max={previewBpmRange.max}
                step="1"
                value={bpm}
                onChange={(event) => {
              ...
              <label>
                <input
                  type="number"
                  aria-label="Repetitietempo in BPM"
                  min={previewBpmRange.min}
                  max={previewBpmRange.max}
                  step="any"
                  value={bpmDraft}
                  aria-invalid={invalidBpm}
            ...
            <small id="studio-bpm-hint">
              {invalidBpm
                ? `Kies ${previewBpmRange.min}–${previewBpmRange.max} BPM. Het voorbeeld blijft op ${bpm} BPM lopen.`
                : 'Hoger BPM = sneller. Groepsduur en offsets blijven in beats.'}
            </small>
          </div>
        </aside>
        <div className="look-studio-editor">
          ...
          <LookEditor
            ...
            look={selectedLook}
            onChange={(look) =>
              onChange({ ...show, looks: show.looks.map((item) => (item.id === look.id ? look : item)) })
            }
          />
        </div>
      </div>
