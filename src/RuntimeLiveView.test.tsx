...
vi.mock('react', async original => ({ ...await original<typeof import('react')>(),
  useState: (value: unknown) => [value === 'looks' ? mocked.controller : typeof value === 'function' ? value() : value, vi.fn()],
  useRef: (value: unknown) => ({ current: value }), useMemo: (factory: () => unknown) => factory(), useEffect: vi.fn(),
}))
const status = { version: 1 as const, sessionId: 'test', status: 'running' as const, mode: 'automation' as const, lookId: initialShow.activeLookId, bpm: 90, atBeats: 17, frameCount: 80, universeCount: 1, outputSent: false as const, error: null }
const makePreview = (): RuntimeLivePreview => ({ version: 1, sessionId: 'test', status, frame: { atBeats: 17, mode: 'automation', fixtures: [] }, controls: { overrides: {}, links: [['front', 'wash']] }, groupIntensities: Object.fromEntries(initialShow.groups.map(group => [group.id, group.intensity])), colorLockId: null, revision: 8 })
beforeEach(() => { mocked.controller = 'looks'; mocked.value = { show: initialShow, preview: makePreview(), status, connected: true, pending: false, error: '', start: vi.fn(), loadEditorShow: vi.fn(), stop: vi.fn(), attach: vi.fn(), command: vi.fn(), setLive: vi.fn() } })
function componentProps<T>(node: ReactNode, type: unknown, match: (props: Record<string, unknown>) => boolean = () => true): T {
  for (const child of Children.toArray(node)) {
    if (!isValidElement<{ children?: ReactNode }>(child)) continue
    if (child.type === type && match(child.props)) return child.props as T
    try { return componentProps(child.props.children, type, match) } catch { /* inspect next sibling */ }
  }
...
it('shows actual runtime beat and never presents an unavailable frame as live', () => {
  ...
  mocked.value.connected = false; mocked.value.preview = undefined
...
it('routes linked rotary masters to transient runtime state without mutating the editor', () => {
  ...
  const surface = componentProps<{ onIntensity: (id: string, value: number) => void; runtimeSession: boolean }>(view, LiveControlSurface)
  surface.onIntensity('wash', .27)
  expect(surface.runtimeSession).toBe(true)
  expect(mocked.value.setLive).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ wash: .27, front: .27 }), null)
  expect(JSON.stringify(initialShow)).toBe(before)
})
it('routes group controls and WING color/look actions to the same runtime', () => {
  mocked.controller = 'wing'
  const view = RuntimeLiveView({ show: initialShow, onConfigure: () => {} })
  const surface = componentProps<{ onLook: (id: string) => void; onColor: (id?: string) => void }>(view, LiveControlSurface)
  surface.onLook('neon-chorus'); surface.onColor('neon')
...
it('shows one main controller at a time and keeps target/mode/output visible independently', () => {
  ...
  expect(componentProps<{ hidden: boolean }>(view, 'div', props => props.className === 'runtime-controller-view').hidden).toBe(true)
  const transport = componentProps<{ target: string; disabled: boolean; onMode: (mode: string) => void }>(view, LiveTransportBar)
  ...
  expect(componentProps<{ hidden: boolean }>(wing, 'div', props => props.className === 'runtime-controller-view').hidden).toBe(false)
...
it.each(['stopped', 'running', 'unavailable'] as const)('offers one deliberate preparation action for %s', phase => {
  ...
  const action = componentProps<{ onClick: () => void; children: string }>(view, 'button', props => props.className === 'primary')
...
it('loads the editor rather than the existing runtime snapshot and keeps attach separate', () => {
  ...
  componentProps<{ onClick: () => void }>(view, 'button', props => props.children === 'Laad editorshow in runtime').onClick()
  expect(mocked.value.loadEditorShow).toHaveBeenCalledWith(editor, 90)
  expect(mocked.value.attach).not.toHaveBeenCalled()
  componentProps<{ onClick: () => void }>(view, 'button', props => props.children === 'Verbind met bestaande sessie').onClick()
...
it('keeps loading available when already connected, but disables it while busy or without Looks', () => {
  ...
  const button = (show = editor) => componentProps<{ disabled: boolean }>(RuntimeLiveView({ show, onConfigure: () => {} }), 'button', props => props.children === 'Laad editorshow in runtime')
...
it('keeps DMX control mounted in its drawer and reachable during a pending show command', () => {
  ...
  expect(componentProps(transport.children, 'button', props => props.children === 'DMX-uitvoer')).not.toHaveProperty('disabled', true)
  expect(mocked.value.start).not.toHaveBeenCalled()
})
it('explains patch, session and confirmation when no DMX session exists', () => {
  mocked.value.status = undefined; mocked.value.preview = undefined; mocked.value.connected = false
  const view = RuntimeLiveView({ show: initialShow, onConfigure: () => {} })
