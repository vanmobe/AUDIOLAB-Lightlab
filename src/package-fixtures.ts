...
export interface PackageFixtureCatalog { version: 1; fingerprint: string; profiles: FixtureProfile[] }
...
export function validatePackageFixtureCatalog(value: unknown, shows: ShowDocument[]) {
  const mismatch = () => new Error('De fixturecatalogus van dit showbestand is ongeldig of wijkt af van deze Lightlab-versie. Open het met de oorspronkelijke compatibele versie; geïmporteerde DMX-profielen worden niet uitgevoerd.')
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw mismatch()
  const data = value as Partial<PackageFixtureCatalog>
  if (Object.keys(data).some(key => !['version', 'fingerprint', 'profiles'].includes(key)) || data.version !== 1 || typeof data.fingerprint !== 'string' || data.fingerprint.length > 64 || !Array.isArray(data.profiles) || data.profiles.length > 64) throw mismatch()
  ...
  function matches(actual: unknown, trusted: unknown): boolean {
    ...
    const expectedKeys = Object.keys(trusted), actualKeys = Object.keys(actual)
    return expectedKeys.length === actualKeys.length && expectedKeys.every(key => Object.hasOwn(actual, key) && matches((actual as Record<string, unknown>)[key], (trusted as Record<string, unknown>)[key]))
  }
