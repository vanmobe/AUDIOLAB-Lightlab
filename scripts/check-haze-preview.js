;async (page) => {
  ...
  assert(
    (await controls.getByRole('slider', { name: /^Rook \/ haze/ }).inputValue()) === '35',
    'Reset restores moderate haze',
  )
  assert(
    saved === (await page.evaluate(() => localStorage.getItem('lightlab-active-package-v1'))),
    'Viewer haze never changes show package',
  )
